// ==========================================
// 1. AYARLAR (Lütfen Burayı Doldur)
// ==========================================
const firebaseConfig = {
  apiKey: "AIzaSyCYoOAJshvUBvJhLW5PQTvz-bDfIobCwi8",
  authDomain: "kelimelik-5x5.firebaseapp.com",
  projectId: "kelimelik-5x5",
  storageBucket: "kelimelik-5x5.firebasestorage.app",
  messagingSenderId: "292210549026",
  appId: "1:292210549026:web:21f376ea9d0ee86555057b"
};

// Firebase Başlatma Kontrolü
if (!firebase.apps.length) {
    firebase.initializeApp(firebaseConfig);
}
const db = firebase.firestore();

// ==========================================
// NASIL OYNANIR PENCERE AYARLARI (GLOBAL)
// ==========================================

// Pencereyi AÇAN Fonksiyon
window.openInfoModal = function() {
    const modal = document.getElementById("howToPlayModal");
    if (modal) {
        modal.classList.remove("hidden");
        // CSS yüklenmese bile açılması için zorluyoruz:
        modal.style.display = "flex"; 
    }
};

// Pencereyi KAPATAN Fonksiyon
window.closeInfoModal = function() {
    const modal = document.getElementById("howToPlayModal");
    if (modal) {
        modal.classList.add("hidden");
        modal.style.display = "none";
    }
};

// Sayfa yüklendiğinde: Pencerenin dışına (siyah alana) tıklayınca kapatma
window.addEventListener('load', () => {
    const modal = document.getElementById("howToPlayModal");
    window.onclick = function(event) {
        if (event.target === modal) {
            window.closeInfoModal();
        }
    };
});

// ==========================================
// YENİ İSTATİSTİK VE SIRALAMA SİSTEMİ
// ==========================================

// 1. Kullanıcı ID'si Al
function getMyStatsId() {
    let id = localStorage.getItem('kelimelik_user_id');
    if (!id) {
        id = 'user_' + Math.random().toString(36).substr(2, 6);
        localStorage.setItem('kelimelik_user_id', id);
    }
    return id;
}

// 2. Yerel İstatistikleri Getir
function getLocalRandomStats() {
    const raw = localStore.getItem('kelimelik_random_history');
    // Veri yapısı: { allScores: [120, 90, ...], dates: [...] }
    if (!raw) return { allScores: [] };
    return JSON.parse(raw);
}

// 3. İstatistik Güncelle (Oyun Bittiğinde Çağrılır)
function updateRandomStats(score) {
    let data = getLocalRandomStats();
    
    // Puanı listeye ekle
    data.allScores.push(score);
    
    // LocalStorage'a kaydet
    localStorage.setItem('kelimelik_random_history', JSON.stringify(data));

    // --- FIREBASE GÜNCELLEME (AYLIK VERİ) ---
    saveMonthlyStatsToFirebase(data.allScores);
}

// 4. Firebase'e Aylık Veri Yazma
function saveMonthlyStatsToFirebase(allScores) {
    const userId = getMyStatsId();
    const date = new Date();
    // Anahtar Örneği: 'stats_2023_12' (Yıl_Ay)
    const monthKey = `stats_${date.getFullYear()}_${date.getMonth() + 1}`; 
    
    // Genel Ortalama
    const totalAvg = Math.round(allScores.reduce((a, b) => a + b, 0) / allScores.length);
    
    // Sadece bu ayın oyunlarını ayırt etmek zor olacağı için, 
    // Basitlik adına "Toplam Oyun Sayısı" üzerinden filtreleme yapacağız
    // Ancak kullanıcı "Bu ay en az 10 oyun" dediği için, Firebase'de o aya özel sayaç tutmalıyız.
    
    const userRef = db.collection('users').doc(userId);

    db.runTransaction(async (transaction) => {
        const doc = await transaction.get(userRef);
        let userData = doc.exists ? doc.data() : {};
        
        // O aya ait veriyi al veya oluştur
        let monthData = userData[monthKey] || { count: 0, totalScore: 0, avg: 0 };
        
        // Yeni puanı ekle
        // Not: Burada 'score' parametresini fonksiyon içinde globalden alamadığımız için
        // allScores dizisinin son elemanını alıyoruz.
        const lastScore = allScores[allScores.length - 1];
        
        monthData.count += 1;
        monthData.totalScore += lastScore;
        monthData.avg = Math.round(monthData.totalScore / monthData.count); // Tam sayı ortalama

        // Veriyi güncelle
        transaction.set(userRef, {
            [monthKey]: monthData, // Dinamik anahtar (Örn: stats_2025_12)
            lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
        }, { merge: true });
    });
}

// 5. İstatistik Penceresini Açma ve Hesaplama
window.openStatsModal = function() {
    const modal = document.getElementById("statsModal");
    if (!modal) return;

    // --- KİŞİSEL VERİLERİ HESAPLA ---
    const data = getLocalRandomStats();
    const scores = data.allScores;
    
    let lifeTimeAvg = "-";
    let last10Avg = "-";

    if (scores.length > 0) {
        // Genel Ortalama
        const total = scores.reduce((a, b) => a + b, 0);
        lifeTimeAvg = (total / scores.length).toFixed(1); // Virgülden sonra 1 hane

        // Son 10 Ortalama
        // slice(-10) son 10 elemanı alır
        const last10 = scores.slice(-10);
        const total10 = last10.reduce((a, b) => a + b, 0);
        last10Avg = (total10 / last10.length).toFixed(1);
    }

    document.getElementById('statLifeTimeAvg').textContent = lifeTimeAvg;
    document.getElementById('statLast10Avg').textContent = last10Avg;

    modal.classList.remove("hidden");
    modal.style.display = "flex";

    // --- LİDER TABLOSUNU ÇEK ---
    fetchLeaderboard();
};

// 6. Lider Tablosunu Getir
function fetchLeaderboard() {
    const tbody = document.getElementById('leaderboardBody');
    tbody.innerHTML = '<tr><td colspan="3">Yükleniyor...</td></tr>';

    const date = new Date();
    const monthKey = `stats_${date.getFullYear()}_${date.getMonth() + 1}`; // Örn: stats_2025_12
    
    // SORGULAMA: O ayın sayacı >= 10 olanları getir, Ortalamaya göre sırala
    // NOT: Bu sorgu için Firebase Console'da Index oluşturmanız gerekebilir!
    // Eğer console'da hata linki çıkarsa ona tıklayıp index oluşturun.
    
    db.collection('users')
        .orderBy(`${monthKey}.avg`, 'desc') // Ortalamaya göre azalan
        .limit(20) // İlk 20 kişi
        .get()
        .then(snapshot => {
            let html = '';
            let rank = 1;
            
            snapshot.forEach(doc => {
                const uData = doc.data();
                const mData = uData[monthKey];

                // Filtreleme: En az 10 oyun (Firestore where sorgusu index isteyeceği için
                // basitlik adına filtrelemeyi burada yapıyoruz, veri azsa sorun olmaz)
                if (mData && mData.count >= 10) {
                    // Kullanıcı ID'sinin son 4 hanesini göster (Gizlilik için)
                    const shortName = "Oyuncu-" + doc.id.substr(-4).toUpperCase();
                    
                    // Kendi satırımızı vurgulamak için
                    const isMe = (doc.id === getMyStatsId());
                    const rowStyle = isMe ? 'style="background-color:#e8f8f5; font-weight:bold;"' : '';

                    html += `
                        <tr ${rowStyle}>
                            <td>${rank++}.</td>
                            <td>${shortName} ${isMe ? '(Sen)' : ''}</td>
                            <td>${mData.avg} Puan</td>
                        </tr>
                    `;
                }
            });

            if (html === '') {
                html = '<tr><td colspan="3">Bu ay henüz yeterli oyun oynanmadı.</td></tr>';
            }
            tbody.innerHTML = html;
        })
        .catch(error => {
            console.error("Leaderboard error:", error);
            tbody.innerHTML = '<tr><td colspan="3">Sıralama yüklenemedi.</td></tr>';
        });
}

// Kapatma Fonksiyonu (Aynı kalabilir)
window.closeStatsModal = function() {
    document.getElementById("statsModal").classList.add("hidden");
    document.getElementById("statsModal").style.display = "none";
};

// ==========================================
// 1.1. PUANLAMA VE SÖZLÜK TANIMLARI
// ==========================================
const SCORE_RULES = { 3: 5, 4: 9, 5: 15 };

// DİKKAT: JSON dosyasının içeriği bu değişkenin içine yapıştırılacak.
// Lütfen burayı 3542 satırlık gerçek kelime listenizle doldurun.
const WORD_ARRAY = [
    // ... TÜM KELİMELERİNİZ BURAYA ...

"AB",
"ABA",
"ABACI",
"ABADİ",
"ABALI",
"ABANA",
"ABANİ",
"ABAŞO",
"ABAT",
"ABAZA",
"ABBAS",
"ABDAL",
"ABDAL",
"ABE",
"ABECE",
"ABES",
"ABHAZ",
"ABİDE",
"ABİS",
"ABİYE",
"ABLA",
"ABLAK",
"ABLİ",
"ABONE",
"ABOSA",
"ABRA",
"ABRAŞ",
"ABU",
"ABUK",
"ABULİ",
"ABUS",
"ACABA",
"ACAR",
"ACAR",
"ACARA",
"ACELE",
"ACEM",
"ACEM",
"ACEMİ",
"ACEP",
"ACEZE",
"ACI",
"ACICA",
"ACILI",
"ACIMA",
"ACIOT",
"ACISU",
"ACİBE",
"ACİL",
"ACİZ",
"ACUBE",
"ACUL",
"ACUN",
"ACUR",
"ACUZE",
"ACVE",
"ACYO",
"AÇ",
"AÇAN",
"AÇAR",
"AÇGÖZ",
"AÇI",
"AÇIK",
"AÇIM",
"AÇIŞ",
"AÇIT",
"AÇKI",
"AÇLIK",
"AÇMA",
"AÇMAK",
"AÇMAZ",
"AD",
"AD",
"ADA",
"ADAK",
"ADALE",
"ADALI",
"ADAM",
"ADAMA",
"ADANA",
"ADAP",
"ADAŞ",
"ADAY",
"ADCI",
"ADEDİ",
"ADEM",
"ADESE",
"ADET",
"ADETA",
"ADIL",
"ADIM",
"ADINA",
"ADİ",
"ADİL",
"ADLI",
"ADLİ",
"ADRES",
"ADSAL",
"ADSIZ",
"AF",
"AFAK",
"AFAKİ",
"AFAL",
"AFAT",
"AFAZİ",
"AFET",
"AFGAN",
"AFİ",
"AFİF",
"AFİFE",
"AFİLİ",
"AFİS",
"AFİŞ",
"AFİŞE",
"AFONİ",
"AFSUN",
"AFŞAR",
"AFŞİN",
"AFT",
"AFTOS",
"AFYON",
"AGAH",
"AGAMİ",
"AGEL",
"AGOP",
"AGORA",
"AGRAF",
"AGU",
"AGUŞ",
"AĞ",
"AĞA",
"AĞAÇ",
"AĞCI",
"AĞCIK",
"AĞDA",
"AĞI",
"AĞIL",
"AĞILI",
"AĞIM",
"AĞIN",
"AĞIR",
"AĞIŞ",
"AĞIT",
"AĞIZ",
"AĞIZ",
"AĞLAK",
"AĞLI",
"AĞLI",
"AĞMA",
"AĞMAK",
"AĞNAM",
"AĞRAZ",
"AĞRI",
"AĞRI",
"AĞSI",
"AĞYAR",
"AH",
"AHA",
"AHALİ",
"AHAR",
"AHBAP",
"AHCAR",
"AHDİ",
"AHENK",
"AHFAT",
"AHFEŞ",
"AHIR",
"AHİ",
"AHİ",
"AHİR",
"AHİT",
"AHİZ",
"AHİZE",
"AHKAM",
"AHLAF",
"AHLAK",
"AHLAT",
"AHLAT",
"AHMAK",
"AHRAZ",
"AHŞA",
"AHŞAP",
"AHU",
"AHVAH",
"AHVAL",
"AİDAT",
"AİLE",
"AİT",
"AJAN",
"AJANS",
"AJİTE",
"AJUR",
"AK",
"AKA",
"AKABE",
"AKAÇ",
"AKAĞA",
"AKAİT",
"AKAJU",
"AKAK",
"AKALA",
"AKAR",
"AKBAŞ",
"AKÇA",
"AKÇE",
"AKÇIL",
"AKDUT",
"AKEMİ",
"AKI",
"AKICI",
"AKIL",
"AKIM",
"AKIN",
"AKIŞ",
"AKİDE",
"AKİK",
"AKİL",
"AKİM",
"AKİS",
"AKİT",
"AKİT",
"AKKAN",
"AKKOR",
"AKKÖY",
"AKKUŞ",
"AKKUŞ",
"AKLAN",
"AKLEN",
"AKLI",
"AKLIK",
"AKLİ",
"AKMA",
"AKMAK",
"AKMAN",
"AKMAZ",
"AKNE",
"AKONT",
"AKOR",
"AKORT",
"AKPAK",
"AKPAS",
"AKRAN",
"AKREP",
"AKREP",
"AKS",
"AKSAK",
"AKSAM",
"AKSAN",
"AKSE",
"AKSİ",
"AKSON",
"AKSU",
"AKSU",
"AKŞAM",
"AKŞIN",
"AKTAR",
"AKTAŞ",
"AKTİF",
"AKTÖR",
"AKUR",
"AKUT",
"AKÜ",
"AKVA",
"AKVAM",
"AKYEL",
"AKYEM",
"AL",
"ALA",
"ALA",
"ALACA",
"ALACA",
"ALAKA",
"ALAN",
"ALARM",
"ALAY",
"ALAZ",
"ALAZA",
"ALBAY",
"ALBÜM",
"ALÇAK",
"ALÇI",
"ALEM",
"ALEM",
"ALENİ",
"ALET",
"ALEV",
"ALEVİ",
"ALEYH",
"ALFA",
"ALG",
"ALGI",
"ALGIN",
"ALICI",
"ALIÇ",
"ALIK",
"ALIM",
"ALIN",
"ALIŞ",
"ALİ",
"ALİ",
"ALİL",
"ALİM",
"ALİM",
"ALİZE",
"ALKAN",
"ALKIM",
"ALKIŞ",
"ALKİL",
"ALKOL",
"ALLAH",
"ALLAH",
"ALLEM",
"ALLI",
"ALLIK",
"ALMA",
"ALMAÇ",
"ALMAK",
"ALMAN",
"ALMAŞ",
"ALMUS",
"ALNAÇ",
"ALO",
"ALP",
"ALPU",
"ALT",
"ALTES",
"ALTI",
"ALTIK",
"ALTIN",
"ALTIZ",
"ALTLI",
"ALTO",
"ALYAN",
"ALYON",
"AMA",
"AMA",
"AMAÇ",
"AMADE",
"AMAL",
"AMAN",
"AMBAR",
"AMBER",
"AMCA",
"AMEL",
"AMELE",
"AMELİ",
"AMFİ",
"AMİGO",
"AMİL",
"AMİN",
"AMİN",
"AMİP",
"AMİR",
"AMİT",
"AMMA",
"AMME",
"AMOR",
"AMORF",
"AMPER",
"AMPİR",
"AMPUL",
"AMUDİ",
"AMUT",
"AN",
"ANA",
"ANACA",
"ANAÇ",
"ANAL",
"ANALI",
"ANANE",
"ANCA",
"ANCAK",
"ANDAÇ",
"ANDIÇ",
"ANDIK",
"ANDIZ",
"ANELE",
"ANEMİ",
"ANGIÇ",
"ANGIN",
"ANGUT",
"ANI",
"ANIK",
"ANIK",
"ANIT",
"ANIZ",
"ANİ",
"ANİDE",
"ANİF",
"ANİME",
"ANJİN",
"ANKA",
"ANKET",
"ANLAK",
"ANLAM",
"ANLIK",
"ANMA",
"ANMAK",
"ANNE",
"ANONS",
"ANOT",
"ANSIZ",
"ANT",
"ANTEN",
"ANTET",
"ANTİK",
"ANTLI",
"ANTRE",
"ANUT",
"ANÜRİ",
"ANÜS",
"ANYON",
"ANZAK",
"AORT",
"APACI",
"APAK",
"APAŞ",
"APAZ",
"APEL",
"APIŞ",
"APİKO",
"APLİK",
"APORT",
"APOŞİ",
"APOTR",
"APRE",
"APRİL",
"APSE",
"APSİS",
"APTAL",
"AR",
"ARA",
"ARABA",
"ARABİ",
"ARACI",
"ARAÇ",
"ARAÇ",
"ARAF",
"ARAK",
"ARAKA",
"ARALI",
"ARAMA",
"ARAMİ",
"ARAP",
"ARAP",
"ARAZ",
"ARAZ",
"ARAZİ",
"ARDA",
"ARDAK",
"ARDIÇ",
"ARDIL",
"ARENA",
"ARGAÇ",
"ARGIN",
"ARGIT",
"ARGO",
"ARGON",
"ARI",
"ARICI",
"ARIK",
"ARIK",
"ARIŞ",
"ARIZ",
"ARIZA",
"ARIZİ",
"ARİ",
"ARİ",
"ARİF",
"ARİFE",
"ARİYA",
"ARİZA",
"ARK",
"ARKA",
"ARKAÇ",
"ARKIT",
"ARKOZ",
"ARLI",
"ARMA",
"ARMUT",
"ARMUZ",
"AROMA",
"ARP",
"ARPA",
"ARPÇI",
"ARPEJ",
"ARSA",
"ARSIZ",
"ARSİN",
"ARŞ",
"ARŞE",
"ARŞIN",
"ARŞİV",
"ART",
"ARTÇI",
"ARTER",
"ARTI",
"ARTIK",
"ARTIM",
"ARTIN",
"ARTIŞ",
"ARTMA",
"ARUZ",
"ARYA",
"ARZ",
"ARZU",
"AS",
"ASA",
"ASABİ",
"ASAL",
"ASAP",
"ASAR",
"ASES",
"ASHAP",
"ASI",
"ASIK",
"ASIL",
"ASILI",
"ASIM",
"ASIR",
"ASİ",
"ASİDE",
"ASİL",
"ASİST",
"ASİT",
"ASK",
"ASKAT",
"ASKER",
"ASKI",
"ASKLI",
"ASLA",
"ASLAN",
"ASLAN",
"ASLEN",
"ASLIK",
"ASLİ",
"ASMA",
"ASMAK",
"ASPUR",
"ASRİ",
"AST",
"ASTAR",
"ASTAT",
"ASTIM",
"ASTİK",
"ASUDE",
"ASYA",
"ASYÖN",
"AŞ",
"AŞAĞI",
"AŞAMA",
"AŞAR",
"AŞARİ",
"AŞÇI",
"AŞEVİ",
"AŞI",
"AŞICI",
"AŞIK",
"AŞIK",
"AŞILI",
"AŞIM",
"AŞIRI",
"AŞIT",
"AŞİNA",
"AŞİR",
"AŞK",
"AŞKIN",
"AŞLIK",
"AŞMA",
"AŞMAK",
"AŞOZ",
"AŞURE",
"AT",
"ATA",
"ATAK",
"ATAK",
"ATAMA",
"ATARİ",
"ATAŞ",
"ATAŞE",
"ATÇI",
"ATE",
"ATEH",
"ATEL",
"ATEŞ",
"ATFEN",
"ATICI",
"ATIF",
"ATIK",
"ATIL",
"ATILI",
"ATIM",
"ATIŞ",
"ATİ",
"ATİK",
"ATKI",
"ATLAS",
"ATLET",
"ATLI",
"ATMA",
"ATMAK",
"ATMIK",
"ATOL",
"ATOM",
"AUT",
"AV",
"AVAL",
"AVAL",
"AVAM",
"AVANE",
"AVANS",
"AVAR",
"AVARA",
"AVARE",
"AVAZ",
"AVAZE",
"AVCI",
"AVCI",
"AVDET",
"AVİZE",
"AVLAK",
"AVLU",
"AVRAT",
"AVRET",
"AVRO",
"AVŞAR",
"AVUÇ",
"AVUNÇ",
"AVURT",
"AY",
"AYA",
"AYAK",
"AYAL",
"AYAN",
"AYAN",
"AYAR",
"AYAŞ",
"AYAZ",
"AYÇA",
"AYDIN",
"AYDIN",
"AYET",
"AYEVİ",
"AYGIR",
"AYGIT",
"AYI",
"AYICI",
"AYIK",
"AYIN",
"AYIP",
"AYIRT",
"AYIT",
"AYİN",
"AYLA",
"AYLAK",
"AYLI",
"AYLIK",
"AYMA",
"AYMAK",
"AYMAZ",
"AYN",
"AYNA",
"AYNAZ",
"AYNEN",
"AYNI",
"AYNİ",
"AYOL",
"AYRAÇ",
"AYRAN",
"AYRI",
"AYRIÇ",
"AYRIK",
"AYRIM",
"AYRIT",
"AYSAR",
"AYSIZ",
"AYVA",
"AYVAN",
"AYVAZ",
"AYYAR",
"AYYAŞ",
"AYYUK",
"AZ",
"AZA",
"AZADE",
"AZAMİ",
"AZAP",
"AZAR",
"AZAT",
"AZAZ",
"AZCA",
"AZÇOK",
"AZEL",
"AZERİ",
"AZGIN",
"AZI",
"AZIK",
"AZILI",
"AZIŞ",
"AZİL",
"AZİM",
"AZİT",
"AZİZ",
"AZİZE",
"AZLIK",
"AZMA",
"AZMAK",
"AZMAK",
"AZMAN",
"AZNİF",
"AZOİK",
"AZOL",
"AZOT",
"AZVAY",
"BABA",
"BABAÇ",
"BABAİ",
"BABİ",
"BACA",
"BACAK",
"BACI",
"BAÇ",
"BAÇÇI",
"BAD",
"BADAL",
"BADAS",
"BADAT",
"BADE",
"BADEM",
"BADIÇ",
"BADİ",
"BADİK",
"BADYA",
"BAFRA",
"BAGAJ",
"BAGET",
"BAĞ",
"BAĞA",
"BAĞAN",
"BAĞCI",
"BAĞDA",
"BAĞI",
"BAĞIL",
"BAĞIM",
"BAĞIN",
"BAĞIR",
"BAĞIŞ",
"BAĞIT",
"BAĞLI",
"BAHAİ",
"BAHAR",
"BAHÇE",
"BAHÇE",
"BAHİR",
"BAHİS",
"BAHRİ",
"BAHT",
"BAKAÇ",
"BAKAM",
"BAKAN",
"BAKI",
"BAKIM",
"BAKIR",
"BAKIŞ",
"BAKİ",
"BAKİR",
"BAKLA",
"BAKMA",
"BAKS",
"BAL",
"BALA",
"BALA",
"BALAR",
"BALAT",
"BALCI",
"BALDO",
"BALE",
"BALET",
"BALIK",
"BALIK",
"BALİĞ",
"BALKI",
"BALLI",
"BALO",
"BALON",
"BALOZ",
"BALTA",
"BALYA",
"BALYA",
"BAMBU",
"BAMYA",
"BAN",
"BANA",
"BANAK",
"BANAL",
"BANAZ",
"BANDO",
"BANİ",
"BANJO",
"BANK",
"BANKA",
"BANKO",
"BANMA",
"BANT",
"BANYO",
"BAP",
"BAR",
"BARA",
"BARAJ",
"BARAK",
"BARAN",
"BARBA",
"BARCI",
"BARÇA",
"BARDA",
"BARDO",
"BAREM",
"BARET",
"BARI",
"BARIŞ",
"BARİ",
"BARİT",
"BARİZ",
"BARKA",
"BARO",
"BAROK",
"BARON",
"BARUT",
"BAS",
"BASAK",
"BASAR",
"BASEN",
"BASI",
"BASIK",
"BASIM",
"BASIN",
"BASIŞ",
"BASİL",
"BASİT",
"BASKI",
"BASMA",
"BASSO",
"BASTI",
"BASUR",
"BASYA",
"BAŞ",
"BAŞAK",
"BAŞAK",
"BAŞAT",
"BAŞÇI",
"BAŞKA",
"BAŞLI",
"BAŞTA",
"BAT",
"BATAK",
"BATAR",
"BATI",
"BATI",
"BATIK",
"BATIL",
"BATIN",
"BATIN",
"BATIŞ",
"BATİ",
"BATİK",
"BATKI",
"BATMA",
"BATON",
"BATUR",
"BAV",
"BAVCI",
"BAVLI",
"BAVUL",
"BAY",
"BAYAN",
"BAYAT",
"BAYAT",
"BAYIR",
"BAYİ",
"BAYMA",
"BAYRI",
"BAZ",
"BAZA",
"BAZAL",
"BAZEN",
"BAZI",
"BAZİK",
"BAZİT",
"BE",
"BEBE",
"BEBEK",
"BECET",
"BED",
"BEDEL",
"BEDEN",
"BEDİİ",
"BEDİK",
"BEDİR",
"BEDÜK",
"BEGÜM",
"BEHER",
"BEHEY",
"BEHRE",
"BEİS",
"BEJ",
"BEK",
"BEKA",
"BEKAR",
"BEKAR",
"BEKAS",
"BEKÇİ",
"BEKRİ",
"BEL",
"BELA",
"BELCE",
"BELDE",
"BELEK",
"BELEN",
"BELEN",
"BELEŞ",
"BELGE",
"BELGİ",
"BELİ",
"BELİĞ",
"BELİK",
"BELİT",
"BELKİ",
"BELLİ",
"BEMOL",
"BEN",
"BENCE",
"BENCİ",
"BENDE",
"BENEK",
"BENGİ",
"BENİZ",
"BENLİ",
"BENT",
"BERAT",
"BERE",
"BERİ",
"BERİL",
"BERK",
"BERRİ",
"BESİ",
"BESİN",
"BESNİ",
"BESNİ",
"BESTE",
"BEŞ",
"BEŞER",
"BEŞİK",
"BEŞİZ",
"BEŞLİ",
"BEŞME",
"BEŞON",
"BEŞON",
"BEŞUŞ",
"BEŞÜÇ",
"BET",
"BETA",
"BETER",
"BETİ",
"BETİK",
"BETİM",
"BETON",
"BEY",
"BEYAN",
"BEYAZ",
"BEYİN",
"BEYİT",
"BEYZİ",
"BEZ",
"BEZCİ",
"BEZE",
"BEZEK",
"BEZEN",
"BEZGİ",
"BEZİK",
"BEZİR",
"BEZİŞ",
"BEZM",
"BEZME",
"BEZSİ",
"BICIL",
"BIÇAK",
"BIÇIK",
"BIÇKI",
"BIDIK",
"BIKIŞ",
"BIKMA",
"BIYIK",
"BIZIR",
"BİAT",
"BİBER",
"BİBİ",
"BİBLO",
"BİCİK",
"BİÇEM",
"BİÇİM",
"BİÇİŞ",
"BİÇKİ",
"BİÇME",
"BİDAR",
"BİDAT",
"BİDE",
"BİDON",
"BİGA",
"BİHUŞ",
"BİKES",
"BİKİR",
"BİLAR",
"BİLE",
"BİLEK",
"BİLET",
"BİLGE",
"BİLGİ",
"BİLİ",
"BİLİM",
"BİLİR",
"BİLİŞ",
"BİLME",
"BİLYE",
"BİN",
"BİNA",
"BİNDİ",
"BİNEK",
"BİNER",
"BİNGİ",
"BİNİ",
"BİNİŞ",
"BİNİT",
"BİNME",
"BİR",
"BİRA",
"BİRAN",
"BİRAZ",
"BİRCİ",
"BİRER",
"BİREY",
"BİRİ",
"BİRİM",
"BİRLİ",
"BİRUN",
"BİS",
"BİŞEK",
"BİŞİ",
"BİT",
"BİTAP",
"BİTEK",
"BİTEY",
"BİTİK",
"BİTİM",
"BİTİŞ",
"BİTKİ",
"BİTLİ",
"BİTME",
"BİTÜM",
"BİYE",
"BİYEL",
"BİZ",
"BİZAR",
"BİZCE",
"BİZON",
"BLOK",
"BLOKE",
"BLÖF",
"BLUM",
"BLUZ",
"BOA",
"BOBİN",
"BOCA",
"BOCCE",
"BOCİ",
"BOCUK",
"BODUÇ",
"BODUR",
"BOĞA",
"BOĞA",
"BOĞAK",
"BOĞAZ",
"BOĞMA",
"BOĞUK",
"BOĞUM",
"BOHÇA",
"BOHEM",
"BOK",
"BOKLU",
"BOKS",
"BOL",
"BOL",
"BOLCA",
"BOLU",
"BOM",
"BOMBA",
"BOMBE",
"BONE",
"BONO",
"BOP",
"BOR",
"BOR",
"BORA",
"BORAK",
"BORAN",
"BORAT",
"BORÇ",
"BORDA",
"BORDO",
"BORİK",
"BORSA",
"BORU",
"BORUK",
"BOŞ",
"BOT",
"BOY",
"BOYA",
"BOYAR",
"BOYCA",
"BOYLU",
"BOYNA",
"BOYOZ",
"BOYUN",
"BOYUT",
"BOZ",
"BOZA",
"BOZCA",
"BOZMA",
"BOZUK",
"BOZUM",
"BOZUŞ",
"BÖBÜR",
"BÖCE",
"BÖCEK",
"BÖCÜ",
"BÖĞ",
"BÖĞÜR",
"BÖKE",
"BÖLE",
"BÖLEN",
"BÖLGE",
"BÖLME",
"BÖLÜ",
"BÖLÜK",
"BÖLÜM",
"BÖLÜŞ",
"BÖLÜT",
"BÖN",
"BÖNCE",
"BÖREK",
"BÖRK",
"BÖYLE",
"BRANŞ",
"BRAVO",
"BRE",
"BREŞ",
"BRİÇ",
"BRİK",
"BRİT",
"BRİZ",
"BROM",
"BRONŞ",
"BRONZ",
"BROŞ",
"BRÖVE",
"BRÜT",
"BU",
"BUAT",
"BUCA",
"BUCAK",
"BUCAK",
"BUÇUK",
"BUDAK",
"BUDUN",
"BUGÜN",
"BUĞRA",
"BUĞU",
"BUĞUR",
"BUĞUZ",
"BUHAR",
"BUHUR",
"BUJİ",
"BUKE",
"BUKET",
"BUKLE",
"BUL",
"BULA",
"BULAK",
"BULGU",
"BULMA",
"BULUŞ",
"BULUT",
"BUN",
"BUNAK",
"BUNCA",
"BUNLU",
"BUNMA",
"BURA",
"BURAK",
"BURCU",
"BURÇ",
"BURGU",
"BURMA",
"BURS",
"BURSA",
"BURU",
"BURUK",
"BURUN",
"BUSE",
"BUŞON",
"BUT",
"BUTİK",
"BUTLU",
"BUTON",
"BUUT",
"BUYMA",
"BUYOT",
"BUZ",
"BUZCU",
"BUZLA",
"BUZLU",
"BUZUL",
"BÜCÜR",
"BÜFE",
"BÜĞET",
"BÜĞLÜ",
"BÜK",
"BÜKEN",
"BÜKME",
"BÜKÜÇ",
"BÜKÜK",
"BÜKÜM",
"BÜKÜN",
"BÜKÜŞ",
"BÜLUĞ",
"BÜNYE",
"BÜRGÜ",
"BÜRO",
"BÜRÜK",
"BÜRÜN",
"BÜST",
"BÜTAN",
"BÜTÇE",
"BÜTEN",
"BÜTEY",
"BÜTÜN",
"BÜVE",
"BÜVET",
"BÜYÜ",
"BÜYÜK",
"BÜZ",
"BÜZGÜ",
"BÜZME",
"BÜZÜK",
"CABA",
"CACIK",
"CADDE",
"CADI",
"CAĞ",
"CAHİL",
"CAİZ",
"CAİZE",
"CAKA",
"CALİP",
"CAM",
"CAMCI",
"CAMIZ",
"CAMİ",
"CAMİ",
"CAMİA",
"CAMİT",
"CAMLI",
"CAMSI",
"CAN",
"CANA",
"CANAN",
"CANİ",
"CANİK",
"CANİP",
"CANLI",
"CAR",
"CARİ",
"CARLI",
"CART",
"CARTA",
"CASUS",
"CAYIŞ",
"CAYMA",
"CAZ",
"CAZCI",
"CAZİP",
"CAZLI",
"CC",
"CE",
"CEBE",
"CEBEL",
"CEBİN",
"CEBİR",
"CEBRİ",
"CEDEL",
"CEDİ",
"CEDİT",
"CEDRE",
"CEFA",
"CEHİL",
"CEHRE",
"CEHRİ",
"CEHT",
"CEKET",
"CELAL",
"CELBE",
"CELEP",
"CELİ",
"CELİL",
"CELP",
"CELSE",
"CEM",
"CEMAL",
"CEMAN",
"CEMİ",
"CEMİL",
"CEMRE",
"CENAH",
"CENAP",
"CENİN",
"CENK",
"CENUP",
"CEP",
"CEPÇİ",
"CEPHE",
"CER",
"CEREN",
"CERH",
"CESET",
"CESİM",
"CESUR",
"CET",
"CEVAP",
"CEVAZ",
"CEVİR",
"CEVİZ",
"CEVVİ",
"CEVZA",
"CEZA",
"CEZAİ",
"CEZBE",
"CEZİR",
"CEZP",
"CEZRİ",
"CEZVE",
"CIBIL",
"CICIK",
"CIK",
"CILIZ",
"CILK",
"CIRT",
"CIS",
"CIVA",
"CIVIK",
"CIVMA",
"CIZ",
"CIZIK",
"CİBİN",
"CİBRE",
"CİCİ",
"CİCİK",
"CİCİM",
"CİCOZ",
"CİDAL",
"CİDAR",
"CİDDİ",
"CİDE",
"CİFE",
"CİĞER",
"CİHAN",
"CİHAR",
"CİHAT",
"CİHAZ",
"CİHET",
"CİLA",
"CİLT",
"CİLVE",
"CİM",
"CİMA",
"CİMRİ",
"CİN",
"CİNAİ",
"CİNAS",
"CİNCİ",
"CİNLİ",
"CİNS",
"CİNSİ",
"CİP",
"CİPS",
"CİRİM",
"CİRİT",
"CİRO",
"CİSİM",
"CİVAN",
"CİVAR",
"CİZRE",
"CİZYE",
"CONTA",
"COP",
"CORUM",
"COŞKU",
"COŞMA",
"COŞUM",
"COŞUŞ",
"CÖNK",
"CUDAM",
"CUK",
"CUKKA",
"CUMA",
"CUMBA",
"CUNDA",
"CUNTA",
"CUP",
"CURA",
"CUŞİŞ",
"CÜBBE",
"CÜCE",
"CÜCÜK",
"CÜDA",
"CÜLUS",
"CÜMLE",
"CÜNHA",
"CÜNUN",
"CÜNÜP",
"CÜRET",
"CÜRUF",
"CÜRÜM",
"CÜSSE",
"CÜZ",
"CÜZİ",
"ÇABA",
"ÇABUK",
"ÇAÇA",
"ÇADIR",
"ÇAĞ",
"ÇAĞA",
"ÇAĞLA",
"ÇAĞMA",
"ÇAĞRI",
"ÇAK",
"ÇAKAL",
"ÇAKAR",
"ÇAKER",
"ÇAKI",
"ÇAKIL",
"ÇAKIM",
"ÇAKIN",
"ÇAKIR",
"ÇAKIŞ",
"ÇAKMA",
"ÇAKRA",
"ÇAL",
"ÇAL",
"ÇALAK",
"ÇALAP",
"ÇALAR",
"ÇALGI",
"ÇALI",
"ÇALIK",
"ÇALIM",
"ÇALIŞ",
"ÇALKI",
"ÇALMA",
"ÇALTI",
"ÇAM",
"ÇAMAŞ",
"ÇAMAT",
"ÇAMÇA",
"ÇAMUR",
"ÇAN",
"ÇAN",
"ÇANAK",
"ÇANCI",
"ÇANDI",
"ÇANLI",
"ÇANTA",
"ÇAP",
"ÇAPA",
"ÇAPAK",
"ÇAPAR",
"ÇAPLA",
"ÇAPLI",
"ÇAPMA",
"ÇAPUL",
"ÇAPUT",
"ÇAR",
"ÇARE",
"ÇARIK",
"ÇARK",
"ÇARKA",
"ÇARPI",
"ÇARŞI",
"ÇASAR",
"ÇAŞIT",
"ÇAT",
"ÇAT",
"ÇATAK",
"ÇATAK",
"ÇATAL",
"ÇATI",
"ÇATIK",
"ÇATIŞ",
"ÇATKI",
"ÇATMA",
"ÇAV",
"ÇAVLI",
"ÇAVMA",
"ÇAVUN",
"ÇAVUŞ",
"ÇAY",
"ÇAY",
"ÇAYAN",
"ÇAYCI",
"ÇAYIR",
"ÇAYLI",
"ÇE",
"ÇEBİÇ",
"ÇECİK",
"ÇEÇ",
"ÇEÇE",
"ÇEÇEN",
"ÇEDİK",
"ÇEHRE",
"ÇEK",
"ÇEK",
"ÇEKÇE",
"ÇEKEK",
"ÇEKEL",
"ÇEKEM",
"ÇEKER",
"ÇEKİ",
"ÇEKİÇ",
"ÇEKİK",
"ÇEKİM",
"ÇEKİŞ",
"ÇEKME",
"ÇEKÜL",
"ÇELEK",
"ÇELEN",
"ÇELGİ",
"ÇELİK",
"ÇELİM",
"ÇELLO",
"ÇELME",
"ÇEMÇE",
"ÇEMEN",
"ÇEMİÇ",
"ÇEMİŞ",
"ÇENE",
"ÇENEK",
"ÇENET",
"ÇENGİ",
"ÇENK",
"ÇEPEL",
"ÇEPER",
"ÇEPEZ",
"ÇEPİN",
"ÇEPNİ",
"ÇERAĞ",
"ÇERÇİ",
"ÇEREZ",
"ÇERGE",
"ÇERİ",
"ÇEŞİT",
"ÇEŞME",
"ÇEŞME",
"ÇEŞNİ",
"ÇETE",
"ÇETİN",
"ÇEVİK",
"ÇEVRE",
"ÇEVRİ",
"ÇEYİZ",
"ÇIBAN",
"ÇIDAM",
"ÇIFIT",
"ÇIFIT",
"ÇIĞ",
"ÇIĞA",
"ÇIĞIR",
"ÇIKAK",
"ÇIKAN",
"ÇIKAR",
"ÇIKI",
"ÇIKIK",
"ÇIKIN",
"ÇIKIŞ",
"ÇIKIT",
"ÇIKMA",
"ÇIKRA",
"ÇIKTI",
"ÇIMA",
"ÇIN",
"ÇINAR",
"ÇINAR",
"ÇINGI",
"ÇIPA",
"ÇIPIR",
"ÇIRA",
"ÇIRAĞ",
"ÇIRAK",
"ÇIRPI",
"ÇIT",
"ÇITA",
"ÇITAK",
"ÇIVMA",
"ÇIYAN",
"ÇİÇEK",
"ÇİFT",
"ÇİFTE",
"ÇİGAN",
"ÇİĞ",
"ÇİĞDE",
"ÇİĞİL",
"ÇİĞİN",
"ÇİĞİT",
"ÇİĞLİ",
"ÇİL",
"ÇİLE",
"ÇİLEK",
"ÇİLLİ",
"ÇİM",
"ÇİMEK",
"ÇİMEN",
"ÇİMME",
"ÇİNCE",
"ÇİNE",
"ÇİNİ",
"ÇİNKO",
"ÇİNLİ",
"ÇİP",
"ÇİPİL",
"ÇİPO",
"ÇİR",
"ÇİRİŞ",
"ÇİROZ",
"ÇİS",
"ÇİSE",
"ÇİSEN",
"ÇİŞ",
"ÇİŞİK",
"ÇİT",
"ÇİTA",
"ÇİTEN",
"ÇİTİ",
"ÇİTME",
"ÇİVİ",
"ÇİVİT",
"ÇİY",
"ÇİZER",
"ÇİZGE",
"ÇİZGİ",
"ÇİZİ",
"ÇİZİK",
"ÇİZİM",
"ÇİZİŞ",
"ÇİZME",
"ÇOBAN",
"ÇOCUK",
"ÇOĞU",
"ÇOĞUL",
"ÇOĞUN",
"ÇOK",
"ÇOKAL",
"ÇOKÇA",
"ÇOKÇU",
"ÇOKLU",
"ÇOLAK",
"ÇOLPA",
"ÇOMAK",
"ÇOMAR",
"ÇOPRA",
"ÇOPUR",
"ÇOR",
"ÇORAK",
"ÇORAP",
"ÇORBA",
"ÇORLU",
"ÇORLU",
"ÇORUM",
"ÇOTRA",
"ÇOTUK",
"ÇÖĞME",
"ÇÖĞÜR",
"ÇÖKEK",
"ÇÖKEL",
"ÇÖKME",
"ÇÖKÜK",
"ÇÖKÜM",
"ÇÖKÜŞ",
"ÇÖL",
"ÇÖMÇE",
"ÇÖMEZ",
"ÇÖMME",
"ÇÖP",
"ÇÖPÇÜ",
"ÇÖPLÜ",
"ÇÖREK",
"ÇÖRKÜ",
"ÇÖRTÜ",
"ÇÖVEN",
"ÇÖZ",
"ÇÖZGÜ",
"ÇÖZME",
"ÇÖZÜK",
"ÇÖZÜM",
"ÇÖZÜŞ",
"ÇUBUK",
"ÇUBUK",
"ÇUCU",
"ÇUHA",
"ÇUKA",
"ÇUKUR",
"ÇUL",
"ÇULCU",
"ÇULHA",
"ÇUMRA",
"ÇUPRA",
"ÇUŞKA",
"ÇUVAL",
"ÇUVAŞ",
"ÇÜK",
"ÇÜKÜR",
"ÇÜNKÜ",
"ÇÜRÜK",
"ÇÜŞ",
"DADAŞ",
"DADAY",
"DADI",
"DAĞ",
"DAĞAR",
"DAĞCI",
"DAĞİ",
"DAĞLI",
"DAH",
"DAHA",
"DAHİ",
"DAHİ",
"DAHİL",
"DAHİL",
"DAİM",
"DAİMA",
"DAİMİ",
"DAİR",
"DAİRE",
"DAKİK",
"DAL",
"DALAK",
"DALAN",
"DALAŞ",
"DALGA",
"DALGI",
"DALIŞ",
"DALIZ",
"DALLI",
"DALMA",
"DALSI",
"DALYA",
"DAM",
"DAMA",
"DAMAK",
"DAMAL",
"DAMAR",
"DAMAT",
"DAMGA",
"DAMLA",
"DAMLI",
"DANA",
"DANCA",
"DANE",
"DANG",
"DANIŞ",
"DANK",
"DANS",
"DAR",
"DARA",
"DARAÇ",
"DARAŞ",
"DARBE",
"DARCA",
"DARI",
"DARP",
"DARU",
"DASİT",
"DATA",
"DATÇA",
"DATİF",
"DAV",
"DAVA",
"DAVAR",
"DAVET",
"DAVUL",
"DAVYA",
"DAYAK",
"DAYE",
"DAYI",
"DAZ",
"DE",
"DEBBE",
"DEBİ",
"DEBİL",
"DEDE",
"DEF",
"DEFA",
"DEFİ",
"DEFİN",
"DEFNE",
"DEFO",
"DEGAJ",
"DEĞEÇ",
"DEĞER",
"DEĞİL",
"DEĞİM",
"DEĞİN",
"DEĞİŞ",
"DEĞME",
"DEH",
"DEHA",
"DEİST",
"DEİZM",
"DEK",
"DEKAN",
"DEKAR",
"DEKOR",
"DELGİ",
"DELİ",
"DELİK",
"DELİL",
"DELK",
"DELME",
"DELTA",
"DEM",
"DEMCİ",
"DEME",
"DEMEÇ",
"DEMEK",
"DEMET",
"DEMİN",
"DEMİR",
"DEMLİ",
"DEMO",
"DEMRE",
"DENEK",
"DENET",
"DENEY",
"DENGE",
"DENİ",
"DENİM",
"DENİZ",
"DENK",
"DENLİ",
"DENME",
"DENYO",
"DEPAR",
"DEPO",
"DERBİ",
"DERÇ",
"DERE",
"DERGİ",
"DERİ",
"DERİK",
"DERİN",
"DERK",
"DERME",
"DERS",
"DERT",
"DERUN",
"DERYA",
"DERZ",
"DESEN",
"DESTE",
"DEŞİK",
"DEŞME",
"DETAY",
"DEV",
"DEVA",
"DEVAM",
"DEVCE",
"DEVE",
"DEVİM",
"DEVİR",
"DEVRE",
"DEVRİ",
"DEYİ",
"DEYİM",
"DEYİŞ",
"DIĞAN",
"DILAK",
"DIŞ",
"DIŞAĞ",
"DIŞIK",
"DIŞKI",
"DİBA",
"DİBEK",
"DİCLE",
"DİDAR",
"DİDE",
"DİDİM",
"DİDON",
"DİGOR",
"DİĞER",
"DİK",
"DİKÇE",
"DİKEÇ",
"DİKEL",
"DİKEN",
"DİKEY",
"DİKİM",
"DİKİŞ",
"DİKİT",
"DİKİZ",
"DİKME",
"DİKSE",
"DİKTA",
"DİKTE",
"DİL",
"DİLCE",
"DİLCİ",
"DİLEK",
"DİLİM",
"DİLİŞ",
"DİLLİ",
"DİLME",
"DİLSİ",
"DİMAĞ",
"DİMİ",
"DİN",
"DİNAR",
"DİNAR",
"DİNCE",
"DİNCİ",
"DİNÇ",
"DİNE",
"DİNEK",
"DİNEN",
"DİNGİ",
"DİNGO",
"DİNİ",
"DİNİŞ",
"DİNK",
"DİNLİ",
"DİNME",
"DİP",
"DİPLİ",
"DİREK",
"DİREN",
"DİREY",
"DİRİ",
"DİRİĞ",
"DİRİL",
"DİRİM",
"DİSK",
"DİSKO",
"DİŞ",
"DİŞÇİ",
"DİŞİ",
"DİŞİL",
"DİŞLİ",
"DİTME",
"DİVA",
"DİVAL",
"DİVAN",
"DİVİK",
"DİVİT",
"DİYA",
"DİYAR",
"DİYE",
"DİYET",
"DİYEZ",
"DİYOT",
"DİZ",
"DİZE",
"DİZEK",
"DİZEL",
"DİZEM",
"DİZEY",
"DİZGE",
"DİZGİ",
"DİZİ",
"DİZİM",
"DİZİN",
"DİZİŞ",
"DİZME",
"DO",
"DOBRA",
"DOGMA",
"DOĞA",
"DOĞAÇ",
"DOĞAL",
"DOĞAN",
"DOĞMA",
"DOĞRU",
"DOĞU",
"DOĞU",
"DOĞUM",
"DOĞUŞ",
"DOK",
"DOKU",
"DOKUZ",
"DOLAK",
"DOLAM",
"DOLAP",
"DOLAR",
"DOLAY",
"DOLGU",
"DOLMA",
"DOLU",
"DOLUM",
"DOLUŞ",
"DOMUR",
"DOMUZ",
"DON",
"DONAM",
"DONE",
"DONLU",
"DONMA",
"DONÖR",
"DONRA",
"DONUK",
"DORE",
"DORSE",
"DORU",
"DORUK",
"DORUM",
"DOST",
"DOSYA",
"DOYGU",
"DOYMA",
"DOYUM",
"DOYUŞ",
"DOZ",
"DOZAJ",
"DOZER",
"DÖGER",
"DÖKME",
"DÖKÜK",
"DÖKÜM",
"DÖKÜŞ",
"DÖL",
"DÖLEK",
"DÖLÜT",
"DÖNEÇ",
"DÖNEK",
"DÖNEL",
"DÖNEM",
"DÖNER",
"DÖNGÜ",
"DÖNME",
"DÖNÜK",
"DÖNÜM",
"DÖNÜŞ",
"DÖNÜT",
"DÖPER",
"DÖRT",
"DÖŞ",
"DÖŞEK",
"DÖŞEM",
"DÖŞLÜ",
"DÖVEÇ",
"DÖVİZ",
"DÖVME",
"DÖVÜŞ",
"DRAJE",
"DRAM",
"DRAMA",
"DREÇ",
"DREN",
"DROG",
"DUA",
"DUACI",
"DUALI",
"DUBA",
"DUBAR",
"DUBLE",
"DUDAK",
"DUDU",
"DUHUL",
"DUKA",
"DUL",
"DULDA",
"DULUK",
"DUMA",
"DUMAN",
"DUMUR",
"DUN",
"DURAÇ",
"DURAK",
"DURAL",
"DURGU",
"DURMA",
"DURU",
"DURUK",
"DURUM",
"DURUŞ",
"DUŞ",
"DUŞAK",
"DUŞLU",
"DUT",
"DUTÇU",
"DUVAK",
"DUVAR",
"DUY",
"DUYAR",
"DUYGU",
"DUYMA",
"DUYU",
"DUYUM",
"DUYUŞ",
"DÜ",
"DÜBEL",
"DÜBEŞ",
"DÜÇAR",
"DÜDEN",
"DÜDÜK",
"DÜET",
"DÜGAH",
"DÜĞME",
"DÜĞÜ",
"DÜĞÜM",
"DÜĞÜN",
"DÜK",
"DÜMEN",
"DÜN",
"DÜNİT",
"DÜNKÜ",
"DÜNÜR",
"DÜNYA",
"DÜNYA",
"DÜRME",
"DÜRTÜ",
"DÜRÜ",
"DÜRÜM",
"DÜRZİ",
"DÜRZÜ",
"DÜSE",
"DÜŞ",
"DÜŞÇÜ",
"DÜŞES",
"DÜŞEŞ",
"DÜŞEY",
"DÜŞKÜ",
"DÜŞME",
"DÜŞÜK",
"DÜŞÜN",
"DÜŞÜŞ",
"DÜŞÜT",
"DÜVE",
"DÜVEL",
"DÜVEN",
"DÜVER",
"DÜYEK",
"DÜYUN",
"DÜZ",
"DÜZCE",
"DÜZCE",
"DÜZE",
"DÜZEÇ",
"DÜZEM",
"DÜZEN",
"DÜZEY",
"DÜZGÜ",
"DÜZME",
"EBAT",
"EBCET",
"EBE",
"EBEDİ",
"EBELİ",
"EBET",
"EBLEH",
"EBRU",
"ECDAT",
"ECE",
"ECEL",
"ECİR",
"ECZA",
"EÇHEL",
"EDA",
"EDALI",
"EDAT",
"EDE",
"EDEBİ",
"EDEP",
"EDER",
"EDİ",
"EDİ",
"EDİK",
"EDİM",
"EDİNÇ",
"EDİP",
"EDNA",
"EDVAR",
"EFDAL",
"EFE",
"EFECE",
"EFEKT",
"EFKAR",
"EFLAK",
"EFOR",
"EFRAT",
"EFRİZ",
"EFSUN",
"EFSUS",
"EGALE",
"EGE",
"EGLOG",
"EGO",
"EGZOZ",
"EĞE",
"EĞER",
"EĞİÇ",
"EĞİK",
"EĞİL",
"EĞİM",
"EĞİN",
"EĞİR",
"EĞİŞ",
"EĞLEK",
"EĞME",
"EĞMEÇ",
"EĞMEK",
"EĞMÜR",
"EĞREZ",
"EĞRİ",
"EĞRİM",
"EĞSİ",
"EH",
"EHEM",
"EHİL",
"EHLİ",
"EHRAM",
"EHVEN",
"EJDER",
"EK",
"EKE",
"EKGÜN",
"EKİCİ",
"EKİLİ",
"EKİM",
"EKİN",
"EKİP",
"EKİT",
"EKKÖK",
"EKLEM",
"EKLER",
"EKLİ",
"EKME",
"EKMEK",
"EKMEK",
"EKO",
"EKOL",
"EKOSE",
"EKRAN",
"EKRU",
"EKSEN",
"EKSER",
"EKSİ",
"EKSİK",
"EKSİN",
"EKSİZ",
"EKŞİ",
"EKTİ",
"EKÜ",
"EKÜRİ",
"EL",
"ELA",
"ELAN",
"ELBET",
"ELCİ",
"ELCİK",
"ELÇEK",
"ELÇİ",
"ELÇİM",
"ELDE",
"ELDEN",
"ELEJİ",
"ELEK",
"ELELE",
"ELEM",
"ELEME",
"ELGİN",
"ELHAK",
"ELİF",
"ELİFİ",
"ELİK",
"ELİM",
"ELİPS",
"ELİŞİ",
"ELİT",
"ELİUZ",
"ELK",
"ELLİ",
"ELLİK",
"ELMA",
"ELMAS",
"ELMEK",
"ELTİ",
"ELVAN",
"ELYAF",
"ELZEM",
"EM",
"EMAN",
"EMARE",
"EMAY",
"EMAYE",
"EMCEK",
"EMCİK",
"EMEÇ",
"EMEK",
"EMEL",
"EMEN",
"EMET",
"EMİCİ",
"EMİK",
"EMİN",
"EMİR",
"EMİR",
"EMİŞ",
"EMLAK",
"EMLİK",
"EMME",
"EMMEÇ",
"EMMEK",
"EMMİ",
"EMRAZ",
"EMSAL",
"EMTİA",
"EMVAL",
"EMZİK",
"EN",
"ENAM",
"ENAYİ",
"ENCAM",
"ENDAM",
"ENDER",
"ENEK",
"ENEME",
"ENEZ",
"ENEZ",
"ENEZE",
"ENFES",
"ENGEL",
"ENGİN",
"ENİK",
"ENİR",
"ENKAZ",
"ENLEM",
"ENLİ",
"ENÖTE",
"ENSAR",
"ENSE",
"ENSİZ",
"ENTEL",
"ENVA",
"ENZİM",
"EOSEN",
"EPE",
"EPER",
"EPEY",
"EPEYİ",
"EPİK",
"EPOPE",
"EPSEM",
"ER",
"ERAT",
"ERBAA",
"ERBAP",
"ERBAŞ",
"ERBİN",
"ERCE",
"ERCİK",
"ERCİŞ",
"ERDEK",
"ERDEM",
"ERDEN",
"EREK",
"EREN",
"ERG",
"ERGEÇ",
"ERGEN",
"ERGİ",
"ERGİN",
"ERİK",
"ERİKA",
"ERİL",
"ERİM",
"ERİME",
"ERİN",
"ERİNÇ",
"ERİŞ",
"ERK",
"ERKAN",
"ERKE",
"ERKEÇ",
"ERKEK",
"ERKEN",
"ERKİN",
"ERKLİ",
"ERLİK",
"ERME",
"ERMEK",
"ERMİN",
"ERMİŞ",
"EROİN",
"EROS",
"EROS",
"ERSİZ",
"ERUH",
"ERVAH",
"ERZAK",
"ERZEL",
"ERZİN",
"ES",
"ESAME",
"ESANS",
"ESAS",
"ESASİ",
"ESBAK",
"ESBAP",
"ESEF",
"ESEME",
"ESEN",
"ESER",
"ESHAM",
"ESİK",
"ESİM",
"ESİN",
"ESİR",
"ESİRE",
"ESİŞ",
"ESKİ",
"ESKİL",
"ESKİZ",
"ESLAF",
"ESMA",
"ESME",
"ESMEK",
"ESMER",
"ESNA",
"ESNAF",
"ESNEK",
"ESPAS",
"ESPRİ",
"ESRAR",
"ESRE",
"ESRİK",
"ESSAH",
"ESTER",
"ESTET",
"ESVAP",
"EŞ",
"EŞARP",
"EŞEK",
"EŞEY",
"EŞHAS",
"EŞİK",
"EŞİT",
"EŞKAL",
"EŞKİN",
"EŞLEK",
"EŞLEM",
"EŞLİ",
"EŞLİK",
"EŞME",
"EŞME",
"EŞMEK",
"EŞRAF",
"EŞREF",
"EŞSİZ",
"EŞYA",
"ET",
"ETAP",
"ETÇİ",
"ETÇİK",
"ETÇİL",
"ETEK",
"ETEN",
"ETENE",
"ETER",
"ETFAL",
"ETİ",
"ETİK",
"ETİL",
"ETKEN",
"ETKİ",
"ETKİN",
"ETLİ",
"ETLİK",
"ETME",
"ETMEK",
"ETMEN",
"ETNİK",
"ETOL",
"ETRAF",
"ETSEL",
"ETSİZ",
"ETUNU",
"ETÜT",
"ETÜV",
"EV",
"EVAZE",
"EVCE",
"EVCEK",
"EVCİ",
"EVCİK",
"EVCİL",
"EVÇ",
"EVDEŞ",
"EVET",
"EVGİN",
"EVHAM",
"EVİÇ",
"EVİN",
"EVİŞİ",
"EVİYE",
"EVKAF",
"EVLA",
"EVLAT",
"EVLEK",
"EVLİ",
"EVLİK",
"EVRAK",
"EVRAT",
"EVRE",
"EVREN",
"EVREN",
"EVRİK",
"EVRİM",
"EVSAF",
"EVSİN",
"EVSİZ",
"EVVEL",
"EY",
"EYER",
"EYLEM",
"EYLÜL",
"EYTAM",
"EYÜP",
"EYVAH",
"EYVAN",
"EYYAM",
"EZA",
"EZAN",
"EZANİ",
"EZBER",
"EZEL",
"EZELİ",
"EZGİ",
"EZGİÇ",
"EZGİN",
"EZİCİ",
"EZİK",
"EZİNÇ",
"EZİNE",
"EZME",
"EZMEK",
"FA",
"FAAL",
"FABL",
"FACİA",
"FAÇA",
"FAGOT",
"FAHİŞ",
"FAHRİ",
"FAHTE",
"FAHUR",
"FAİK",
"FAİL",
"FAİZ",
"FAK",
"FAKAT",
"FAKİH",
"FAKİR",
"FAKR",
"FAKS",
"FAKÜL",
"FAL",
"FALAN",
"FALCI",
"FALEZ",
"FALSO",
"FALYA",
"FAN",
"FANİ",
"FANİ",
"FANTA",
"FANTİ",
"FANUS",
"FANYA",
"FAR",
"FARAD",
"FARAŞ",
"FARBA",
"FARE",
"FARİĞ",
"FARİL",
"FARK",
"FARS",
"FARS",
"FARZ",
"FASET",
"FASIK",
"FASIL",
"FASİH",
"FASİT",
"FASKA",
"FASLI",
"FASON",
"FAŞ",
"FATİH",
"FATİH",
"FATSA",
"FAUL",
"FAUNA",
"FAVA",
"FAY",
"FAYDA",
"FAZ",
"FAZIL",
"FAZLA",
"FE",
"FECİ",
"FECİR",
"FEDA",
"FEDAİ",
"FEHİM",
"FEHVA",
"FEK",
"FEKE",
"FEKÜL",
"FEL",
"FELAH",
"FELÇ",
"FELEK",
"FEN",
"FENA",
"FENCİ",
"FENER",
"FENİK",
"FENNİ",
"FENOL",
"FENT",
"FER",
"FERAĞ",
"FERAH",
"FERÇ",
"FERDA",
"FERDİ",
"FERİ",
"FERİH",
"FERİK",
"FERLİ",
"FERMA",
"FERT",
"FERZ",
"FES",
"FESAT",
"FESİH",
"FETHA",
"FETİH",
"FETİŞ",
"FETÜS",
"FETVA",
"FEVÇ",
"FEVK",
"FEVRİ",
"FEVT",
"FEYİZ",
"FEZA",
"FIÇI",
"FIKIH",
"FIKRA",
"FIR",
"FIRÇA",
"FIRIN",
"FIRKA",
"FIRT",
"FIŞKI",
"FITIK",
"FITRİ",
"FİBER",
"FİDAN",
"FİDE",
"FİDYE",
"FİFRE",
"FİGAN",
"FİGÜR",
"FİĞ",
"FİİL",
"FİİLİ",
"FİKİR",
"FİKRİ",
"FİL",
"FİLAN",
"FİLAR",
"FİLE",
"FİLET",
"FİLİZ",
"FİLM",
"FİLO",
"FİLOZ",
"FİLSİ",
"FİLUM",
"FİN",
"FİNAL",
"FİNCE",
"FİNİŞ",
"FİNK",
"FİNO",
"FİRAK",
"FİRAR",
"FİRE",
"FİREZ",
"FİRİK",
"FİRMA",
"FİSKE",
"FİSTO",
"FİŞ",
"FİŞE",
"FİŞEK",
"FİŞKA",
"FİŞLİ",
"FİT",
"FİTÇİ",
"FİTİL",
"FİTİN",
"FİTNE",
"FİTRE",
"FİYAT",
"FİZİK",
"FLAMA",
"FLAŞ",
"FLEOL",
"FLİT",
"FLOK",
"FLOR",
"FLORA",
"FLORİ",
"FLOŞ",
"FLÖRE",
"FLÖRT",
"FLU",
"FLÜT",
"FOBİ",
"FOÇA",
"FODLA",
"FODRA",
"FODUL",
"FOK",
"FOKUS",
"FOL",
"FOLK",
"FOLYO",
"FON",
"FONDA",
"FONDÜ",
"FONEM",
"FONT",
"FORA",
"FORM",
"FORMA",
"FOROZ",
"FORS",
"FORSA",
"FORTE",
"FORUM",
"FOS",
"FOSİL",
"FOŞ",
"FOŞA",
"FOTA",
"FOTO",
"FOYA",
"FÖN",
"FÖTR",
"FÖY",
"FRAK",
"FRANK",
"FRAPE",
"FREN",
"FRENK",
"FRER",
"FRESK",
"FREZE",
"FRİGO",
"FRİSA",
"FRİZ",
"FUAR",
"FUAYE",
"FUHUŞ",
"FUJER",
"FUL",
"FULAR",
"FULE",
"FULYA",
"FUNDA",
"FURYA",
"FUT",
"FUTA",
"FÜG",
"FÜLÜS",
"FÜME",
"FÜNYE",
"FÜRS",
"FÜRU",
"FÜSUN",
"FÜTUR",
"FÜZE",
"FÜZEN",
"GABİ",
"GABİN",
"GABRO",
"GABYA",
"GACI",
"GADİR",
"GAF",
"GAFİL",
"GAFUR",
"GAGA",
"GAH",
"GAHİ",
"GAİLE",
"GAİP",
"GAİTA",
"GAK",
"GALA",
"GALAT",
"GALE",
"GALİ",
"GALİP",
"GALİZ",
"GALON",
"GALOP",
"GALOŞ",
"GAM",
"GAMA",
"GAMBA",
"GAMET",
"GAMLI",
"GAMZE",
"GANG",
"GANİ",
"GAR",
"GARAJ",
"GARAZ",
"GARBİ",
"GARD",
"GARİP",
"GARK",
"GAROZ",
"GARP",
"GARP",
"GASİL",
"GASP",
"GAŞİY",
"GATO",
"GAUSS",
"GAVOT",
"GAVUR",
"GAYDA",
"GAYE",
"GAYET",
"GAYR",
"GAYRI",
"GAYRİ",
"GAYUR",
"GAYYA",
"GAYYA",
"GAYZ",
"GAZ",
"GAZA",
"GAZAL",
"GAZAP",
"GAZEL",
"GAZİ",
"GAZLI",
"GAZOZ",
"GAZVE",
"GE",
"GEBE",
"GEBEŞ",
"GEBRE",
"GEBZE",
"GECE",
"GEÇ",
"GEÇÇE",
"GEÇE",
"GEÇEK",
"GEÇEN",
"GEÇER",
"GEÇİM",
"GEÇİŞ",
"GEÇİT",
"GEÇME",
"GEDA",
"GEDİK",
"GEDİZ",
"GEDME",
"GELE",
"GELEN",
"GELİN",
"GELİR",
"GELİŞ",
"GELME",
"GEM",
"GEMİ",
"GEMRE",
"GEN",
"GENÇ",
"GENÇ",
"GENE",
"GENEL",
"GENİŞ",
"GENİZ",
"GENOM",
"GEOİT",
"GERÇİ",
"GEREÇ",
"GEREK",
"GEREN",
"GERGİ",
"GERİ",
"GERİM",
"GERİŞ",
"GERİZ",
"GERME",
"GERZE",
"GETR",
"GETTO",
"GEVAŞ",
"GEVEN",
"GEVİŞ",
"GEVME",
"GEY",
"GEYİK",
"GEYŞA",
"GEYVE",
"GEZ",
"GEZİ",
"GEZİŞ",
"GEZME",
"GICIK",
"GICIR",
"GIDA",
"GIDIK",
"GIDIM",
"GIGI",
"GIK",
"GINA",
"GIPTA",
"GIR",
"GIRLA",
"GIRT",
"GIYAP",
"GİBİ",
"GİDER",
"GİDİ",
"GİDİŞ",
"GİDON",
"GİNE",
"GİRAY",
"GİRDİ",
"GİREN",
"GİRİM",
"GİRİŞ",
"GİRME",
"GİŞE",
"GİTAR",
"GİTME",
"GİYİM",
"GİYİŞ",
"GİYİT",
"GİYME",
"GİYSİ",
"GİZ",
"GİZEM",
"GİZİL",
"GİZLİ",
"GLASE",
"GNAYS",
"GOCUK",
"GODOŞ",
"GOL",
"GOLCÜ",
"GOLF",
"GOLLÜ",
"GONCA",
"GONK",
"GORİL",
"GOTÇA",
"GOTİK",
"GÖBEK",
"GÖBEL",
"GÖBÜT",
"GÖCE",
"GÖCEN",
"GÖÇ",
"GÖÇER",
"GÖÇME",
"GÖÇÜ",
"GÖÇÜK",
"GÖÇÜM",
"GÖÇÜŞ",
"GÖDEN",
"GÖDEŞ",
"GÖĞEM",
"GÖĞÜS",
"GÖK",
"GÖKÇE",
"GÖL",
"GÖLE",
"GÖLEK",
"GÖLET",
"GÖLGE",
"GÖLÜK",
"GÖMEÇ",
"GÖMME",
"GÖMÜ",
"GÖMÜK",
"GÖMÜŞ",
"GÖMÜT",
"GÖN",
"GÖNCÜ",
"GÖNÇ",
"GÖNEN",
"GÖNEN",
"GÖNÜL",
"GÖNYE",
"GÖRE",
"GÖREV",
"GÖRGÜ",
"GÖRK",
"GÖRME",
"GÖRÜ",
"GÖRÜM",
"GÖRÜŞ",
"GÖT",
"GÖVDE",
"GÖVEK",
"GÖVEL",
"GÖVEM",
"GÖYME",
"GÖYÜK",
"GÖZ",
"GÖZCÜ",
"GÖZDE",
"GÖZE",
"GÖZER",
"GÖZGÜ",
"GÖZLÜ",
"GRADO",
"GRAM",
"GREK",
"GREN",
"GRENA",
"GRES",
"GREV",
"GRİ",
"GRİL",
"GRİP",
"GRİZU",
"GROGİ",
"GROSA",
"GRUP",
"GUAJ",
"GUANO",
"GUATR",
"GUDDE",
"GUGUK",
"GULAŞ",
"GULET",
"GURK",
"GURME",
"GURU",
"GURUP",
"GURUR",
"GUSTO",
"GUSÜL",
"GUŞA",
"GUT",
"GÜBRE",
"GÜBÜR",
"GÜCE",
"GÜCÜ",
"GÜCÜK",
"GÜCÜN",
"GÜÇ",
"GÜÇ",
"GÜÇLÜ",
"GÜDEK",
"GÜDÜ",
"GÜDÜK",
"GÜDÜL",
"GÜDÜM",
"GÜFTE",
"GÜĞÜM",
"GÜL",
"GÜLCÜ",
"GÜLEÇ",
"GÜLLE",
"GÜLLÜ",
"GÜLME",
"GÜLÜK",
"GÜLÜŞ",
"GÜLÜT",
"GÜM",
"GÜME",
"GÜMEÇ",
"GÜMÜL",
"GÜMÜŞ",
"GÜN",
"GÜNAH",
"GÜNCE",
"GÜNDE",
"GÜNEÇ",
"GÜNEŞ",
"GÜNEŞ",
"GÜNEY",
"GÜNEY",
"GÜNLÜ",
"GÜNÜ",
"GÜPÜR",
"GÜR",
"GÜRCÜ",
"GÜRE",
"GÜREŞ",
"GÜRSU",
"GÜRUH",
"GÜRÜN",
"GÜRZ",
"GÜTME",
"GÜVE",
"GÜVEÇ",
"GÜVEN",
"GÜVEZ",
"GÜYA",
"GÜZ",
"GÜZEL",
"GÜZEY",
"GÜZÜN",
"HA",
"HAB",
"HABBE",
"HABER",
"HABEŞ",
"HABEŞ",
"HABİP",
"HABİS",
"HAC",
"HACET",
"HACI",
"HACİM",
"HACİR",
"HACİZ",
"HAÇ",
"HAÇLI",
"HAD",
"HADDE",
"HADIM",
"HADİ",
"HADİM",
"HADİM",
"HADİS",
"HAF",
"HAFIZ",
"HAFİ",
"HAFİF",
"HAFİK",
"HAFİT",
"HAFTA",
"HAH",
"HAHAM",
"HAHHA",
"HAİL",
"HAİLE",
"HAİN",
"HAİZ",
"HAJE",
"HAK",
"HAK",
"HAK",
"HAK",
"HAKAN",
"HAKAS",
"HAKÇA",
"HAKEM",
"HAKİ",
"HAKİM",
"HAKİM",
"HAKİR",
"HAKLI",
"HAL",
"HAL",
"HAL",
"HAL",
"HALA",
"HALA",
"HALAÇ",
"HALAS",
"HALAT",
"HALAY",
"HALE",
"HALEF",
"HALEL",
"HALEN",
"HALET",
"HALFA",
"HALI",
"HALİ",
"HALİÇ",
"HALİK",
"HALİM",
"HALİS",
"HALK",
"HALKA",
"HALT",
"HALUK",
"HAM",
"HAMAK",
"HAMAL",
"HAMAM",
"HAMEL",
"HAMIZ",
"HAMİ",
"HAMİL",
"HAMİŞ",
"HAMLA",
"HAMLE",
"HAMSE",
"HAMSİ",
"HAMT",
"HAMUR",
"HAMUR",
"HAMUT",
"HAN",
"HAN",
"HANAK",
"HANAY",
"HANCI",
"HANDE",
"HANE",
"HANEK",
"HANGİ",
"HANIM",
"HANİ",
"HANİ",
"HANUT",
"HANYA",
"HAP",
"HAPAZ",
"HAPÇI",
"HAPİS",
"HAPŞU",
"HAR",
"HARA",
"HARAÇ",
"HARAM",
"HARAP",
"HARAR",
"HARBE",
"HARBİ",
"HARBİ",
"HARÇ",
"HARE",
"HAREM",
"HARF",
"HARIM",
"HARIN",
"HARİÇ",
"HARİM",
"HARİR",
"HARİS",
"HARLI",
"HARP",
"HARS",
"HART",
"HARTA",
"HAS",
"HASA",
"HASAR",
"HASAT",
"HASBİ",
"HASEP",
"HASET",
"HASIL",
"HASIM",
"HASIR",
"HASIR",
"HASİS",
"HASPA",
"HASSA",
"HASSA",
"HASSE",
"HASTA",
"HASUN",
"HASUT",
"HAŞA",
"HAŞAT",
"HAŞIL",
"HAŞİN",
"HAŞİR",
"HAŞİŞ",
"HAŞİV",
"HAT",
"HATA",
"HATAY",
"HATIL",
"HATIR",
"HATİF",
"HATİM",
"HATİP",
"HATMİ",
"HATTA",
"HATUN",
"HAV",
"HAVA",
"HAVAİ",
"HAVAN",
"HAVAS",
"HAVİ",
"HAVLI",
"HAVLU",
"HAVRA",
"HAVSA",
"HAVUÇ",
"HAVUT",
"HAVUZ",
"HAVVA",
"HAVYA",
"HAVZA",
"HAVZA",
"HAY",
"HAYA",
"HAYA",
"HAYAL",
"HAYAT",
"HAYBE",
"HAYCI",
"HAYDA",
"HAYDİ",
"HAYFA",
"HAYIF",
"HAYIR",
"HAYIR",
"HAYIT",
"HAYIZ",
"HAYLİ",
"HAYTA",
"HAYTA",
"HAZ",
"HAZA",
"HAZAN",
"HAZAR",
"HAZAR",
"HAZCI",
"HAZIK",
"HAZIM",
"HAZIR",
"HAZİN",
"HAZNE",
"HAZRO",
"HE",
"HEBA",
"HECE",
"HECİN",
"HEDEF",
"HEDER",
"HEDİK",
"HEKİM",
"HELA",
"HELAK",
"HELAL",
"HELE",
"HELEN",
"HELİK",
"HELİS",
"HELKE",
"HELME",
"HELVA",
"HEM",
"HEMEN",
"HEMPA",
"HEMZE",
"HENÜZ",
"HEP",
"HEPSİ",
"HER",
"HEREK",
"HERİF",
"HERİK",
"HERK",
"HERTZ",
"HERZE",
"HESAP",
"HEVES",
"HEY",
"HEYBE",
"HEYET",
"HEZEL",
"HEZEN",
"HIFIZ",
"HIK",
"HIMIŞ",
"HINÇ",
"HINIS",
"HINK",
"HIR",
"HIRBO",
"HIRKA",
"HIRLI",
"HIRS",
"HIRT",
"HISIM",
"HIŞIM",
"HIŞIR",
"HIYAR",
"HIZ",
"HIZAR",
"HIZIR",
"HIZLA",
"HIZLI",
"HIZMA",
"HİBE",
"HİCAP",
"HİCAZ",
"HİCİV",
"HİCRİ",
"HİÇ",
"HİÇÇİ",
"HİDİV",
"HİDRA",
"HİKEM",
"HİLAF",
"HİLAL",
"HİLAT",
"HİLE",
"HİLYE",
"HİMEN",
"HİN",
"HİNDİ",
"HİNDU",
"HİNT",
"HİPPİ",
"HİS",
"HİSAR",
"HİSLİ",
"HİSSE",
"HİSSİ",
"HİŞ",
"HİŞT",
"HİT",
"HİTAM",
"HİTAN",
"HİTAP",
"HİTİT",
"HİZA",
"HİZAN",
"HİZİP",
"HOBİ",
"HOCA",
"HODAN",
"HODRİ",
"HOKEY",
"HOKKA",
"HOL",
"HONA",
"HONAZ",
"HOP",
"HOPA",
"HOPPA",
"HOR",
"HORA",
"HORON",
"HOROZ",
"HORST",
"HOŞ",
"HOŞAF",
"HOŞÇA",
"HOŞT",
"HOŞUR",
"HOTOZ",
"HOZAN",
"HOZAT",
"HÖDÜK",
"HÖST",
"HÖT",
"HÖYÜK",
"HU",
"HUDUT",
"HUĞ",
"HUKUK",
"HULUL",
"HULUS",
"HUMAR",
"HUMMA",
"HUMOR",
"HUMUS",
"HUN",
"HUNİ",
"HURÇ",
"HURDA",
"HURİ",
"HURMA",
"HURRA",
"HURUÇ",
"HUSUF",
"HUSUL",
"HUSUS",
"HUSYE",
"HUŞ",
"HUŞU",
"HUTBE",
"HUTUT",
"HUY",
"HUYLU",
"HUZUR",
"HÜCRE",
"HÜCUM",
"HÜDA",
"HÜKMİ",
"HÜKÜM",
"HÜLLE",
"HÜLYA",
"HÜMA",
"HÜNER",
"HÜNSA",
"HÜR",
"HÜRLE",
"HÜRYA",
"HÜSÜN",
"HÜYÜK",
"HÜZME",
"HÜZÜN",
"IĞDIR",
"IĞIL",
"IĞRIP",
"IH",
"IHMA",
"IHMAK",
"ILGAR",
"ILGAZ",
"ILGIM",
"ILGIN",
"ILGIN",
"ILICA",
"ILICA",
"ILIK",
"ILIM",
"ILIMA",
"ILTAR",
"IRA",
"IRAK",
"IRAK",
"IRAMA",
"IRGAT",
"IRIP",
"IRK",
"IRKÇI",
"IRKİ",
"IRMAK",
"IRZ",
"ISI",
"ISIL",
"ISIN",
"ISKA",
"ISKAT",
"ISLAH",
"ISLAK",
"ISLIK",
"ISRAR",
"ISSIZ",
"ISTAR",
"IŞIK",
"IŞIL",
"IŞIMA",
"IŞIN",
"IŞKI",
"IŞKIN",
"IŞTIR",
"ITIR",
"ITLAK",
"ITRAH",
"ITRİ",
"IZGIN",
"IZRAR",
"İADE",
"İANE",
"İARE",
"İAŞE",
"İBARE",
"İBATE",
"İBDA",
"İBDAİ",
"İBİK",
"İBİS",
"İBİŞ",
"İBLAĞ",
"İBLİS",
"İBNE",
"İBRA",
"İBRAZ",
"İBRE",
"İBRET",
"İBRİK",
"İBZAL",
"İCAP",
"İCAR",
"İCAT",
"İCAZ",
"İCBAR",
"İCMA",
"İCMAL",
"İCRA",
"İÇ",
"İÇAĞ",
"İÇAĞA",
"İÇCEP",
"İÇEK",
"İÇERİ",
"İÇGÖÇ",
"İÇHAT",
"İÇISI",
"İÇİCİ",
"İÇİÇE",
"İÇİM",
"İÇİN",
"İÇİŞ",
"İÇİT",
"İÇKİ",
"İÇKİN",
"İÇLEM",
"İÇLİ",
"İÇLİK",
"İÇME",
"İÇMEK",
"İÇRE",
"İÇREK",
"İÇSEL",
"İÇSES",
"İÇSİZ",
"İÇSU",
"İÇTEN",
"İÇYÜZ",
"İÇYÜZ",
"İÇZAR",
"İDADİ",
"İDAM",
"İDAME",
"İDARE",
"İDARİ",
"İDDİA",
"İDE",
"İDEA",
"İDEAL",
"İDİL",
"İDİL",
"İDMAN",
"İDOL",
"İDRAK",
"İDRAR",
"İFA",
"İFADE",
"İFFET",
"İFHAM",
"İFLAH",
"İFLAS",
"İFNA",
"İFRAĞ",
"İFRAT",
"İFRAZ",
"İFRİT",
"İFSAT",
"İFŞA",
"İFTA",
"İFTAR",
"İGLU",
"İĞ",
"İĞCİ",
"İĞDE",
"İĞDİR",
"İĞDİŞ",
"İĞFAL",
"İĞLİK",
"İĞNE",
"İHALE",
"İHAM",
"İHATA",
"İHBAR",
"İHDAS",
"İHLAL",
"İHLAS",
"İHLAS",
"İHMAL",
"İHRAÇ",
"İHRAM",
"İHRAZ",
"İHSAN",
"İHSAS",
"İHTAR",
"İHVAN",
"İHYA",
"İHZAR",
"İKAME",
"İKAZ",
"İKBAL",
"İKDAM",
"İKİ",
"İKİCİ",
"İKİLİ",
"İKİZ",
"İKLİM",
"İKMAL",
"İKNA",
"İKON",
"İKONA",
"İKRAH",
"İKRAM",
"İKRAR",
"İKRAZ",
"İKSA",
"İKSİR",
"İKTA",
"İL",
"İLA",
"İLAÇ",
"İLAH",
"İLAH",
"İLAHE",
"İLAHİ",
"İLAHİ",
"İLAM",
"İLAN",
"İLAVE",
"İLBAY",
"İLCA",
"İLÇE",
"İLE",
"İLEK",
"İLENÇ",
"İLERİ",
"İLETİ",
"İLGA",
"İLGEÇ",
"İLGİ",
"İLHAK",
"İLHAM",
"İLHAN",
"İLİÇ",
"İLİK",
"İLİM",
"İLK",
"İLKAH",
"İLKE",
"İLKEL",
"İLKİN",
"İLLA",
"İLLE",
"İLLET",
"İLLİ",
"İLME",
"İLMEK",
"İLMEK",
"İLMİ",
"İLMİK",
"İLZAM",
"İM",
"İMA",
"İMAJ",
"İMAL",
"İMALE",
"İMALI",
"İMAM",
"İMAME",
"İMAN",
"İMAR",
"İMBAT",
"İMBİK",
"İMDAT",
"İMDİ",
"İMECE",
"İMGE",
"İMHA",
"İMKAN",
"İMLA",
"İMLEÇ",
"İMLEK",
"İMLİK",
"İMREN",
"İMROZ",
"İMSAK",
"İMZA",
"İN",
"İNAK",
"İNAL",
"İNAM",
"İNAN",
"İNANÇ",
"İNAT",
"İNCE",
"İNCİ",
"İNCİK",
"İNCİL",
"İNCİR",
"İNÇ",
"İNDİ",
"İNDİS",
"İNEÇ",
"İNEK",
"İNFAK",
"İNFAZ",
"İNGİN",
"İNHA",
"İNİ",
"İNİK",
"İNİŞ",
"İNKAR",
"İNME",
"İNMEK",
"İNÖNÜ",
"İNSAF",
"İNSAN",
"İNŞA",
"İNŞAT",
"İNTAÇ",
"İNTAK",
"İNTAN",
"İNTİ",
"İNZAL",
"İP",
"İPÇİ",
"İPÇİK",
"İPEK",
"İPEKA",
"İPHAM",
"İPKA",
"İPLİK",
"İPSİ",
"İPSİZ",
"İPTAL",
"İPUCU",
"İRADE",
"İRADİ",
"İRAP",
"İRAT",
"İRCA",
"İRFAN",
"İRİ",
"İRİCE",
"İRİN",
"İRİS",
"İRİTE",
"İRKME",
"İRMİK",
"İRONİ",
"İRS",
"İRSAL",
"İRSEN",
"İRSİ",
"İRŞAT",
"İS",
"İSA",
"İSAL",
"İSALE",
"İSEVİ",
"İSHAL",
"İSİM",
"İSKAN",
"İSKOÇ",
"İSLAM",
"İSLAV",
"İSLİ",
"İSLİM",
"İSMEN",
"İSMET",
"İSNAT",
"İSOT",
"İSPAT",
"İSPİR",
"İSPİR",
"İSPİT",
"İSRAF",
"İSTEK",
"İSTEM",
"İSTER",
"İSTİF",
"İSTİM",
"İSTOP",
"İSYAN",
"İŞ",
"İŞAR",
"İŞBA",
"İŞBU",
"İŞÇİ",
"İŞEME",
"İŞERİ",
"İŞGAL",
"İŞGÜÇ",
"İŞKAL",
"İŞKİL",
"İŞLEK",
"İŞLEM",
"İŞLEV",
"İŞLİ",
"İŞLİK",
"İŞMAR",
"İŞRET",
"İŞSİZ",
"İŞTAH",
"İŞTE",
"İŞTEŞ",
"İŞVE",
"İŞYAR",
"İT",
"İTA",
"İTAAT",
"İTAP",
"İTÇE",
"İTEĞİ",
"İTFA",
"İTHAF",
"İTHAL",
"İTHAM",
"İTİ",
"İTİCİ",
"İTİLA",
"İTİNA",
"İTİŞ",
"İTKİ",
"İTLAF",
"İTLİK",
"İTMAM",
"İTME",
"İTMEK",
"İVAZ",
"İVEDİ",
"İVESİ",
"İVGİ",
"İVME",
"İVMEK",
"İYE",
"İYİ",
"İYİCE",
"İYON",
"İYOT",
"İZ",
"İZABE",
"İZAÇ",
"İZAFE",
"İZAFİ",
"İZAH",
"İZALE",
"İZAM",
"İZAN",
"İZAZ",
"İZBE",
"İZCİ",
"İZGE",
"İZHAR",
"İZİN",
"İZLEK",
"İZLEM",
"İZLEV",
"İZMİR",
"İZMİT",
"İZNİK",
"İZOLE",
"İZZET",
"JAKAR",
"JALE",
"JANR",
"JANT",
"JAPON",
"JARSE",
"JE",
"JEL",
"JELOZ",
"JEST",
"JET",
"JETON",
"JİG",
"JİKLE",
"JİLE",
"JİLET",
"JİPS",
"JOKER",
"JOKEY",
"JÖLE",
"JÖN",
"JUDO",
"JUL",
"JURO",
"JÜPON",
"JÜRİ",
"JÜT",
"KABA",
"KABAK",
"KABAN",
"KABE",
"KABIZ",
"KABİL",
"KABİL",
"KABİN",
"KABİR",
"KABLO",
"KABUK",
"KABUL",
"KABUS",
"KABZA",
"KAÇ",
"KAÇA",
"KAÇAK",
"KAÇAR",
"KAÇI",
"KAÇIK",
"KAÇIŞ",
"KAÇLI",
"KAÇMA",
"KAÇTA",
"KADAR",
"KADEH",
"KADEM",
"KADER",
"KADI",
"KADIN",
"KADİM",
"KADİR",
"KADİR",
"KADİT",
"KADRO",
"KADÜK",
"KAFA",
"KAFE",
"KAFES",
"KAFİ",
"KAFİR",
"KAFUR",
"KAGİR",
"KAĞAN",
"KAĞIT",
"KAĞNI",
"KAH",
"KAHIR",
"KAHİL",
"KAHİN",
"KAHİR",
"KAHPE",
"KAHTA",
"KAHVE",
"KAHYA",
"KAİDE",
"KAİL",
"KAİM",
"KAİME",
"KAİN",
"KAK",
"KAKA",
"KAKAÇ",
"KAKAO",
"KAKIÇ",
"KAKIM",
"KAKIŞ",
"KAKMA",
"KAKÜL",
"KAL",
"KAL",
"KALA",
"KALAK",
"KALAN",
"KALAS",
"KALAY",
"KALBİ",
"KALCI",
"KALÇA",
"KALE",
"KALE",
"KALEM",
"KALFA",
"KALIÇ",
"KALIK",
"KALIM",
"KALIN",
"KALIP",
"KALIŞ",
"KALIT",
"KALMA",
"KALP",
"KALP",
"KALYA",
"KAM",
"KAM",
"KAMA",
"KAMAN",
"KAMÇI",
"KAME",
"KAMER",
"KAMET",
"KAMGA",
"KAMIŞ",
"KAMİL",
"KAMP",
"KAMU",
"KAMUS",
"KAN",
"KANA",
"KANAL",
"KANAT",
"KANCA",
"KANI",
"KANIK",
"KANIŞ",
"KANIT",
"KANİ",
"KANİŞ",
"KANKA",
"KANLI",
"KANMA",
"KANO",
"KANON",
"KANT",
"KANTO",
"KANUN",
"KANUN",
"KAOS",
"KAP",
"KAP",
"KAP",
"KAPAK",
"KAPAN",
"KAPI",
"KAPIŞ",
"KAPİK",
"KAPİŞ",
"KAPLI",
"KAPMA",
"KAPUT",
"KAPUZ",
"KAR",
"KAR",
"KARA",
"KARAR",
"KARAY",
"KARE",
"KARGA",
"KARGI",
"KARGI",
"KARGO",
"KARHA",
"KARI",
"KARIK",
"KARIN",
"KARIŞ",
"KARİ",
"KARLI",
"KARLI",
"KARMA",
"KARNE",
"KARNİ",
"KARO",
"KARS",
"KARST",
"KARŞI",
"KART",
"KARUN",
"KARUN",
"KARYE",
"KAS",
"KASA",
"KASAP",
"KASE",
"KASEM",
"KASET",
"KASIK",
"KASIM",
"KASIR",
"KASIT",
"KASİS",
"KASK",
"KASKO",
"KASLI",
"KASMA",
"KASNI",
"KAST",
"KASTİ",
"KAŞ",
"KAŞ",
"KAŞAN",
"KAŞAR",
"KAŞE",
"KAŞIK",
"KAŞİF",
"KAŞLI",
"KAT",
"KATAR",
"KATÇI",
"KATI",
"KATIK",
"KATIM",
"KATIR",
"KATİ",
"KATİL",
"KATİL",
"KATİP",
"KATKI",
"KATLI",
"KATMA",
"KATOT",
"KATRE",
"KAV",
"KAVAF",
"KAVAK",
"KAVAK",
"KAVAL",
"KAVAS",
"KAVAT",
"KAVGA",
"KAVİ",
"KAVİL",
"KAVİM",
"KAVİS",
"KAVKI",
"KAVMİ",
"KAVUK",
"KAVUN",
"KAVUT",
"KAVUZ",
"KAY",
"KAY",
"KAYA",
"KAYAÇ",
"KAYAK",
"KAYAN",
"KAYAR",
"KAYGI",
"KAYI",
"KAYIK",
"KAYIN",
"KAYIN",
"KAYIP",
"KAYIR",
"KAYIŞ",
"KAYIT",
"KAYIT",
"KAYMA",
"KAYME",
"KAYRA",
"KAYŞA",
"KAZ",
"KAZA",
"KAZAK",
"KAZAK",
"KAZAN",
"KAZAN",
"KAZAZ",
"KAZI",
"KAZIK",
"KAZIL",
"KAZIM",
"KAZMA",
"KE",
"KEBAN",
"KEBAP",
"KEBE",
"KEBİR",
"KEBZE",
"KEÇE",
"KEÇİ",
"KEDER",
"KEDİ",
"KEF",
"KEFAL",
"KEFE",
"KEFEN",
"KEFİL",
"KEFİR",
"KEFNE",
"KEHLE",
"KEK",
"KEKA",
"KEKE",
"KEKEÇ",
"KEKİK",
"KEKRE",
"KEL",
"KELAM",
"KELE",
"KELEK",
"KELEM",
"KELEP",
"KELER",
"KELES",
"KELEŞ",
"KELİK",
"KELLE",
"KELLİ",
"KEM",
"KEMAH",
"KEMAL",
"KEMAN",
"KEME",
"KEMER",
"KEMER",
"KEMHA",
"KEMİK",
"KEMRE",
"KENAR",
"KENDİ",
"KENE",
"KENEF",
"KENET",
"KENT",
"KEP",
"KEPÇE",
"KEPEK",
"KEPEZ",
"KEPEZ",
"KEPİR",
"KEPME",
"KER",
"KERDE",
"KERE",
"KEREM",
"KERES",
"KERH",
"KERİH",
"KERİM",
"KERİZ",
"KERKİ",
"KERTE",
"KERTİ",
"KES",
"KESAT",
"KESBİ",
"KESE",
"KESEK",
"KESEL",
"KESEN",
"KESER",
"KESİ",
"KESİF",
"KESİK",
"KESİM",
"KESİN",
"KESİR",
"KESİŞ",
"KESİT",
"KESKİ",
"KESME",
"KESP",
"KESRE",
"KEŞ",
"KEŞAN",
"KEŞAP",
"KEŞEN",
"KEŞİF",
"KEŞİK",
"KEŞİŞ",
"KEŞKE",
"KEŞKİ",
"KET",
"KETAL",
"KETE",
"KETEN",
"KETON",
"KETUM",
"KEVEL",
"KEVEN",
"KEYFİ",
"KEYİF",
"KEZ",
"KEZA",
"KIBLE",
"KIÇ",
"KIDEM",
"KIĞ",
"KIĞI",
"KIL",
"KILGI",
"KILIÇ",
"KILIF",
"KILIK",
"KILIR",
"KILIŞ",
"KILLI",
"KILMA",
"KIMIL",
"KIMIZ",
"KIN",
"KINA",
"KINIK",
"KINLI",
"KIPIK",
"KIPMA",
"KIPTİ",
"KIR",
"KIRAÇ",
"KIRAN",
"KIRAT",
"KIRAY",
"KIRBA",
"KIRCA",
"KIRCI",
"KIRÇ",
"KIRIK",
"KIRIM",
"KIRK",
"KIRKI",
"KIRMA",
"KISA",
"KISAS",
"KISIK",
"KISIM",
"KISIR",
"KISIŞ",
"KISIT",
"KISKA",
"KISKI",
"KISMA",
"KISMİ",
"KISSA",
"KIŞ",
"KIŞIN",
"KIŞIR",
"KIŞLA",
"KIT",
"KITA",
"KITAL",
"KITIK",
"KITIR",
"KIVAM",
"KIYA",
"KIYAK",
"KIYAM",
"KIYAS",
"KIYGI",
"KIYI",
"KIYIK",
"KIYIM",
"KIYIN",
"KIYIŞ",
"KIYMA",
"KIYYE",
"KIZ",
"KIZAK",
"KIZAN",
"KIZIK",
"KIZIL",
"KIZIŞ",
"KIZMA",
"Kİ",
"KİBAR",
"KİBİR",
"KİFAF",
"KİĞI",
"KİK",
"KİKLA",
"KİL",
"KİLE",
"KİLER",
"KİLİM",
"KİLİS",
"KİLİT",
"KİLİZ",
"KİLLİ",
"KİLO",
"KİLS",
"KİLSİ",
"KİLÜS",
"KİM",
"KİMİ",
"KİMSE",
"KİMÜS",
"KİMYA",
"KİN",
"KİNCİ",
"KİNİK",
"KİNİN",
"KİNİŞ",
"KİNLİ",
"KİP",
"KİPE",
"KİR",
"KİRA",
"KİRAZ",
"KİRAZ",
"KİRDE",
"KİREÇ",
"KİRİL",
"KİRİŞ",
"KİRLİ",
"KİRPİ",
"KİRVE",
"KİST",
"KİSVE",
"KİŞİ",
"KİT",
"KİTAP",
"KİTİN",
"KİTLE",
"KİTRE",
"KİVİ",
"KİZİR",
"KLAN",
"KLAPA",
"KLAS",
"KLİK",
"KLİMA",
"KLİP",
"KLİPS",
"KLİŞE",
"KLON",
"KLOR",
"KLOŞ",
"KLÜZ",
"KOALA",
"KOBAY",
"KOBRA",
"KOCA",
"KOÇ",
"KOÇ",
"KOÇAK",
"KOÇAN",
"KOÇMA",
"KOÇU",
"KOD",
"KODES",
"KOF",
"KOFA",
"KOFRA",
"KOFTİ",
"KOFUL",
"KOĞUŞ",
"KOK",
"KOKA",
"KOKAK",
"KOKET",
"KOKMA",
"KOKOŞ",
"KOKOT",
"KOKOZ",
"KOKU",
"KOKUŞ",
"KOL",
"KOLA",
"KOLAJ",
"KOLAN",
"KOLAY",
"KOLCU",
"KOLEJ",
"KOLİ",
"KOLİK",
"KOLİT",
"KOLLU",
"KOLON",
"KOLPO",
"KOLSU",
"KOLYE",
"KOLZA",
"KOM",
"KOMA",
"KOMAR",
"KOMBİ",
"KOMİ",
"KOMİK",
"KOMOT",
"KOMŞU",
"KOMUT",
"KOMÜN",
"KONAK",
"KONAK",
"KONÇ",
"KONDU",
"KONİ",
"KONİK",
"KONMA",
"KONSA",
"KONT",
"KONU",
"KONUK",
"KONUM",
"KONUR",
"KONUŞ",
"KONUT",
"KONYA",
"KOPAL",
"KOPÇA",
"KOPİL",
"KOPMA",
"KOPOY",
"KOPUK",
"KOPUŞ",
"KOPUZ",
"KOPYA",
"KOR",
"KORAL",
"KORE",
"KORKU",
"KORNA",
"KORNO",
"KORO",
"KORSE",
"KORT",
"KORTE",
"KORU",
"KORUK",
"KORUN",
"KORZA",
"KOSA",
"KOŞA",
"KOŞAÇ",
"KOŞAM",
"KOŞİN",
"KOŞMA",
"KOŞU",
"KOŞUK",
"KOŞUL",
"KOŞUM",
"KOŞUN",
"KOŞUT",
"KOT",
"KOTA",
"KOTAN",
"KOTON",
"KOTRA",
"KOV",
"KOVA",
"KOVA",
"KOVAN",
"KOVCU",
"KOVMA",
"KOVUK",
"KOVUŞ",
"KOY",
"KOYAK",
"KOYAR",
"KOYMA",
"KOYU",
"KOYUN",
"KOYUN",
"KOYUŞ",
"KOYUT",
"KOZ",
"KOZA",
"KOZAK",
"KOZAN",
"KÖÇEK",
"KÖFTE",
"KÖHNE",
"KÖK",
"KÖKÇÜ",
"KÖKEN",
"KÖKLÜ",
"KÖKSÜ",
"KÖLE",
"KÖLÜK",
"KÖMBE",
"KÖMEÇ",
"KÖMÜR",
"KÖMÜŞ",
"KÖPEK",
"KÖPRÜ",
"KÖPÜK",
"KÖR",
"KÖRE",
"KÖRPE",
"KÖRÜK",
"KÖS",
"KÖSÇÜ",
"KÖSE",
"KÖSE",
"KÖSEM",
"KÖSNÜ",
"KÖŞE",
"KÖŞEK",
"KÖŞK",
"KÖŞK",
"KÖTEK",
"KÖTÜ",
"KÖY",
"KÖYCÜ",
"KÖYLÜ",
"KÖZ",
"KRAÇA",
"KRAL",
"KRAMP",
"KRANK",
"KRAVL",
"KREDİ",
"KREM",
"KREMA",
"KREP",
"KREŞ",
"KRİKO",
"KRİZ",
"KROKİ",
"KROM",
"KROME",
"KRON",
"KROS",
"KROŞE",
"KUBAT",
"KUBBE",
"KUBUR",
"KUCAK",
"KUDAS",
"KUDUZ",
"KUDÜM",
"KUFİ",
"KUĞU",
"KUHİ",
"KUKA",
"KUKLA",
"KUL",
"KULA",
"KULA",
"KULAÇ",
"KULAK",
"KULE",
"KULİS",
"KULLE",
"KULP",
"KULP",
"KULU",
"KULUN",
"KULÜP",
"KUM",
"KUMA",
"KUMAN",
"KUMAR",
"KUMAŞ",
"KUMCU",
"KUMLA",
"KUMLU",
"KUMLU",
"KUMRU",
"KUMRU",
"KUMSU",
"KUMUÇ",
"KUMUK",
"KUMUL",
"KUNDA",
"KUNT",
"KUP",
"KUPA",
"KUPES",
"KUPLE",
"KUPON",
"KUPÜR",
"KUR",
"KURA",
"KURAK",
"KURAL",
"KURAM",
"KURCA",
"KURGU",
"KURMA",
"KURNA",
"KURON",
"KURS",
"KURT",
"KURT",
"KURU",
"KURUL",
"KURUM",
"KURUŞ",
"KURUT",
"KURYA",
"KURYE",
"KUSMA",
"KUSUR",
"KUŞ",
"KUŞAK",
"KUŞÇA",
"KUŞÇU",
"KUŞE",
"KUŞET",
"KUŞKU",
"KUT",
"KUTAN",
"KUTLU",
"KUTNU",
"KUTSİ",
"KUTU",
"KUTUP",
"KUTUR",
"KUUT",
"KUVER",
"KUVÖZ",
"KUVVE",
"KUYTU",
"KUYU",
"KUYUM",
"KUZ",
"KUZEN",
"KUZEY",
"KUZİN",
"KUZU",
"KÜBİK",
"KÜÇÜK",
"KÜF",
"KÜFE",
"KÜFLÜ",
"KÜFÜR",
"KÜKRE",
"KÜL",
"KÜL",
"KÜLAH",
"KÜLÇE",
"KÜLEK",
"KÜLLİ",
"KÜLLÜ",
"KÜLOT",
"KÜLT",
"KÜLTE",
"KÜME",
"KÜMES",
"KÜNCÜ",
"KÜNDE",
"KÜNH",
"KÜNK",
"KÜNYE",
"KÜP",
"KÜPE",
"KÜPLÜ",
"KÜR",
"KÜRAR",
"KÜRDİ",
"KÜRE",
"KÜRE",
"KÜREK",
"KÜRİT",
"KÜRK",
"KÜRSÜ",
"KÜRT",
"KÜS",
"KÜSKÜ",
"KÜSME",
"KÜSPE",
"KÜSUF",
"KÜSUR",
"KÜSÜ",
"KÜŞAT",
"KÜŞNE",
"KÜŞÜM",
"KÜT",
"KÜTİN",
"KÜTLE",
"KÜTLÜ",
"KÜTÖR",
"KÜTÜK",
"KÜVET",
"LA",
"LAÇIN",
"LAÇİN",
"LAÇKA",
"LADEN",
"LADES",
"LADİK",
"LADİN",
"LAF",
"LAFÇI",
"LAFIZ",
"LAFZİ",
"LAGAR",
"LAGOS",
"LAGÜN",
"LAĞIM",
"LAĞIV",
"LAHİT",
"LAHOS",
"LAHUT",
"LAHZA",
"LAİK",
"LAİN",
"LAK",
"LAKA",
"LAKAP",
"LAKÇI",
"LAKE",
"LAKİN",
"LAKOZ",
"LAL",
"LAL",
"LALA",
"LALE",
"LAM",
"LAM",
"LAMA",
"LAMBA",
"LAME",
"LAMEL",
"LAN",
"LANDO",
"LANET",
"LANSE",
"LAP",
"LAPA",
"LAPON",
"LARGO",
"LARP",
"LARVA",
"LASKİ",
"LASO",
"LASTA",
"LAŞE",
"LATA",
"LATİF",
"LATİN",
"LAV",
"LAVA",
"LAVAJ",
"LAVAŞ",
"LAVTA",
"LAVUK",
"LAYIK",
"LAZ",
"LAZA",
"LAZCA",
"LAZER",
"LAZIM",
"LAZUT",
"LE",
"LEB",
"LEÇE",
"LEÇEK",
"LEDÜN",
"LEF",
"LEGAL",
"LEĞEN",
"LEH",
"LEH",
"LEHÇE",
"LEHÇE",
"LEHİM",
"LEKE",
"LEMİS",
"LENF",
"LENFA",
"LENS",
"LENTO",
"LEP",
"LEPRA",
"LERZE",
"LEŞ",
"LETÇE",
"LEVA",
"LEVHA",
"LEVYE",
"LEY",
"LEYDİ",
"LEYLİ",
"LEZAR",
"LEZİZ",
"LIĞ",
"LIĞLI",
"LIKIR",
"LİBAS",
"LİBOŞ",
"LİBRE",
"LİCE",
"LİDER",
"LİET",
"LİF",
"LİFLİ",
"LİFT",
"LİG",
"LİGA",
"LİGER",
"LİKA",
"LİKEN",
"LİKİT",
"LİKÖR",
"LİM",
"LİMAN",
"LİMBO",
"LİME",
"LİMİT",
"LİMON",
"LİNÇ",
"LİNET",
"LİNİN",
"LİNK",
"LİPİT",
"LİPOM",
"LİR",
"LİRA",
"LİRET",
"LİRİK",
"LİSAN",
"LİSE",
"LİSTE",
"LİTRE",
"LİVA",
"LİVAR",
"LİYAN",
"LİZOL",
"LİZÖZ",
"LOBİ",
"LOBUT",
"LOCA",
"LOÇA",
"LODA",
"LODOS",
"LOGO",
"LOGOS",
"LOĞ",
"LOJİK",
"LOK",
"LOKAL",
"LOKMA",
"LOKUM",
"LOLO",
"LONCA",
"LONGA",
"LOP",
"LOP",
"LOPET",
"LOPUR",
"LOR",
"LORT",
"LORTA",
"LOŞ",
"LOŞÇA",
"LOT",
"LOTA",
"LOTUS",
"LÖK",
"LÖKOZ",
"LÖP",
"LÖPÜR",
"LÖS",
"LUP",
"LUTİ",
"LUTR",
"LÜFER",
"LÜGAT",
"LÜGOL",
"LÜK",
"LÜKS",
"LÜLE",
"LÜMEN",
"LÜNET",
"LÜP",
"LÜPÇÜ",
"LÜTUF",
"LÜZUM",
"MAADA",
"MAAŞ",
"MABAT",
"MABET",
"MABUT",
"MACAR",
"MACUN",
"MAÇ",
"MAÇA",
"MAÇKA",
"MAÇO",
"MADAM",
"MADDE",
"MADDİ",
"MADEM",
"MADEN",
"MADEN",
"MADER",
"MADİK",
"MADUN",
"MAFİŞ",
"MAFYA",
"MAGMA",
"MAGRİ",
"MAH",
"MAHAL",
"MAHFE",
"MAHFİ",
"MAHIV",
"MAHİR",
"MAHRA",
"MAHUR",
"MAHUT",
"MAHYA",
"MAİ",
"MAİL",
"MAİLE",
"MAİN",
"MAJÖR",
"MAKAK",
"MAKAM",
"MAKAS",
"MAKAT",
"MAKET",
"MAKİ",
"MAKRO",
"MAKSİ",
"MAKTA",
"MAKTU",
"MAKUL",
"MAKUS",
"MAL",
"MALA",
"MALAK",
"MALAZ",
"MALCA",
"MALCI",
"MALÇ",
"MALEN",
"MALİ",
"MALİK",
"MALT",
"MALUL",
"MALUM",
"MALYA",
"MAMA",
"MAMAK",
"MAMBO",
"MAMUL",
"MAMUR",
"MAMUT",
"MANA",
"MANAS",
"MANAT",
"MANAV",
"MANAV",
"MANCA",
"MANÇU",
"MANDA",
"MANEJ",
"MANEN",
"MANGA",
"MANGO",
"MANİ",
"MANİ",
"MANİA",
"MANO",
"MANTI",
"MANTİ",
"MANTO",
"MAOCU",
"MAPA",
"MAPUS",
"MARAL",
"MARAZ",
"MARDA",
"MARİZ",
"MARJ",
"MARK",
"MARKA",
"MARKE",
"MARKİ",
"MARN",
"MARON",
"MARS",
"MARS",
"MARŞ",
"MART",
"MARTI",
"MARUF",
"MARUL",
"MARUZ",
"MARYA",
"MAS",
"MASA",
"MASAJ",
"MASAL",
"MASAT",
"MASİF",
"MASK",
"MASKE",
"MASNU",
"MASON",
"MASÖR",
"MASÖZ",
"MASTI",
"MASUM",
"MASUN",
"MAŞ",
"MAŞA",
"MAŞER",
"MAŞUK",
"MAT",
"MATAH",
"MATBU",
"MATEM",
"MATİZ",
"MATLA",
"MATUF",
"MATUH",
"MAUN",
"MAVAL",
"MAVİ",
"MAVİŞ",
"MAVNA",
"MAVRA",
"MAYA",
"MAYIN",
"MAYIS",
"MAYİ",
"MAYNA",
"MAYO",
"MAZAK",
"MAZI",
"MAZİ",
"MAZOT",
"MAZUR",
"ME",
"MEAL",
"MEBDE",
"MEBİZ",
"MEBNİ",
"MEBUS",
"MECAL",
"MECAZ",
"MECMU",
"MECRA",
"MECUS",
"MEÇ",
"MEDAR",
"MEDET",
"MEDİH",
"MEDÜZ",
"MEDYA",
"MEFUL",
"MEĞER",
"MEH",
"MEHAZ",
"MEHDİ",
"MEHİL",
"MEHLE",
"MEKAN",
"MEKE",
"MEKİK",
"MELAL",
"MELAS",
"MELCE",
"MELEK",
"MELES",
"MELEŞ",
"MELEZ",
"MELİK",
"MELON",
"MELUL",
"MELUN",
"MEMAT",
"MEMBA",
"MEME",
"MEMNU",
"MEMUL",
"MEMUR",
"MEN",
"MENFA",
"MENFİ",
"MENİ",
"MENŞE",
"MENUS",
"MENÜ",
"MERA",
"MERAK",
"MERAM",
"MERAM",
"MERCİ",
"MEREK",
"MERES",
"MERET",
"MERİ",
"MERİÇ",
"MERİH",
"MERMİ",
"MERT",
"MESAİ",
"MESAJ",
"MESEL",
"MESEN",
"MESH",
"MESİH",
"MESMU",
"MEST",
"MESUL",
"MESUT",
"MEŞBU",
"MEŞE",
"MEŞİN",
"MEŞK",
"MEŞRU",
"MEŞUM",
"MET",
"MET",
"META",
"METAL",
"METAN",
"METBU",
"METİL",
"METİN",
"METİN",
"METİS",
"METOT",
"METRE",
"METRO",
"MEVDU",
"MEVKİ",
"MEVLA",
"MEVLA",
"MEVT",
"MEVTA",
"MEVUT",
"MEVZİ",
"MEVZU",
"MEY",
"MEYAN",
"MEYİL",
"MEYUS",
"MEYVE",
"MEZAR",
"MEZAT",
"MEZE",
"MEZON",
"MEZRA",
"MEZRU",
"MEZUN",
"MEZÜR",
"MEZZO",
"MICIR",
"MIGIR",
"MIGRİ",
"MIH",
"MIHLI",
"MIRRA",
"MISIR",
"MISIR",
"MISRA",
"Mİ",
"MİAT",
"MİÇEL",
"MİÇO",
"MİDE",
"MİDİ",
"MİDYE",
"MİHR",
"MİKA",
"MİKAP",
"MİKOZ",
"MİKRO",
"MİL",
"MİLAS",
"MİLAT",
"MİLEL",
"MİLİM",
"MİLİS",
"MİLLİ",
"MİLLİ",
"MİM",
"MİMAR",
"MİMİK",
"MİMLİ",
"MİNE",
"MİNİ",
"MİNİK",
"MİNK",
"MİNÖR",
"MİR",
"MİRA",
"MİRAÇ",
"MİRAS",
"MİRAT",
"MİRİ",
"MİRZA",
"MİS",
"MİSAK",
"MİSAL",
"MİSEL",
"MİSİL",
"MİSİS",
"MİSK",
"MİSO",
"MİT",
"MİTİL",
"MİTOS",
"MİTOZ",
"MİYAR",
"MİYAV",
"MİYAZ",
"MİYOM",
"MİYOP",
"MİZA",
"MİZAÇ",
"MİZAH",
"MİZAN",
"MOBİL",
"MODA",
"MODEL",
"MODEM",
"MODÜL",
"MOĞOL",
"MOHER",
"MOKA",
"MOLA",
"MOLAS",
"MOLLA",
"MOLOZ",
"MONAT",
"MONT",
"MONTE",
"MOPET",
"MOR",
"MORAL",
"MOREN",
"MORG",
"MORS",
"MORS",
"MORTO",
"MORUK",
"MOTEL",
"MOTİF",
"MOTOR",
"MOTTO",
"MOZAK",
"MÖBLE",
"MÖSYÖ",
"MUAF",
"MUARE",
"MUCİP",
"MUCİR",
"MUCİT",
"MUCUK",
"MUCUR",
"MUCUR",
"MUÇO",
"MUDİ",
"MUF",
"MUFLA",
"MUĞLA",
"MUHAL",
"MUHAT",
"MUHİK",
"MUHİL",
"MUHİP",
"MUHİT",
"MUİN",
"MUİT",
"MUJİK",
"MUKİM",
"MUKNİ",
"MUKUS",
"MULAJ",
"MUM",
"MUMCU",
"MUMLU",
"MUMYA",
"MUNİS",
"MURAT",
"MURÇ",
"MURİS",
"MURT",
"MUSAP",
"MUSIR",
"MUSKA",
"MUSON",
"MUŞ",
"MUŞ",
"MUŞLU",
"MUŞTA",
"MUŞTU",
"MUT",
"MUT",
"MUT",
"MUTA",
"MUTAF",
"MUTAT",
"MUTÇU",
"MUTİ",
"MUTKİ",
"MUTLU",
"MUYLU",
"MUZ",
"MUZIR",
"MUZİP",
"MUZSU",
"MÜBAH",
"MÜDÜR",
"MÜFİT",
"MÜFT",
"MÜFTÜ",
"MÜGE",
"MÜHİM",
"MÜHRE",
"MÜHÜR",
"MÜJDE",
"MÜL",
"MÜLGA",
"MÜLK",
"MÜLKİ",
"MÜMAS",
"MÜMİN",
"MÜNŞİ",
"MÜRAİ",
"MÜRİT",
"MÜRT",
"MÜRUR",
"MÜŞİR",
"MÜZE",
"MÜZİÇ",
"MÜZİK",
"NAAŞ",
"NAAT",
"NABIZ",
"NACAK",
"NAÇAR",
"NAÇİZ",
"NADAN",
"NADAS",
"NADİM",
"NADİR",
"NAFİ",
"NAFİA",
"NAFİZ",
"NAFTA",
"NAĞME",
"NAH",
"NAHAK",
"NAHIR",
"NAHİF",
"NAHİV",
"NAHOŞ",
"NAİF",
"NAİL",
"NAİP",
"NAKDİ",
"NAKIS",
"NAKIŞ",
"NAKIZ",
"NAKİL",
"NAKİL",
"NAKİP",
"NAKİT",
"NAKLİ",
"NAKŞİ",
"NAL",
"NALAN",
"NALÇA",
"NALE",
"NALIN",
"NAM",
"NAMAZ",
"NAME",
"NAMLI",
"NAMLU",
"NAMUS",
"NAN",
"NANAY",
"NANE",
"NANİK",
"NAR",
"NAR",
"NARA",
"NARH",
"NARİN",
"NAS",
"NASIL",
"NASIP",
"NASIR",
"NASİP",
"NASİR",
"NAŞİ",
"NAŞİR",
"NATIR",
"NATO",
"NATUK",
"NATÜR",
"NAZ",
"NAZAL",
"NAZAR",
"NAZIM",
"NAZIM",
"NAZIR",
"NAZİ",
"NAZİK",
"NAZİL",
"NAZİR",
"NAZLI",
"NE",
"NE",
"NEALA",
"NEBAT",
"NEBİ",
"NECAT",
"NECE",
"NECİ",
"NECİP",
"NEDBE",
"NEDEN",
"NEDİM",
"NEFER",
"NEFES",
"NEFHA",
"NEFİR",
"NEFİS",
"NEFİY",
"NEFT",
"NEFTİ",
"NEHİR",
"NEHİY",
"NEKES",
"NEKRE",
"NEM",
"NEMA",
"NEMÇE",
"NEMF",
"NEMLİ",
"NEMSE",
"NENE",
"NEON",
"NERE",
"NESEP",
"NESİ",
"NESİÇ",
"NESİH",
"NESİL",
"NESİM",
"NESİR",
"NESNE",
"NEŞE",
"NEŞET",
"NEŞİR",
"NET",
"NEV",
"NEVA",
"NEVİR",
"NEY",
"NEYÇE",
"NEYSE",
"NEZİF",
"NEZİH",
"NEZİR",
"NEZLE",
"NISIF",
"NİCE",
"NİCEL",
"NİÇİN",
"NİDA",
"NİFAK",
"NİĞDE",
"NİHAİ",
"NİHAN",
"NİKAH",
"NİKAP",
"NİKEL",
"NİM",
"NİMET",
"NİNE",
"NİNNİ",
"NİPEL",
"NİSA",
"NİSAİ",
"NİSAN",
"NİSAP",
"NİSPİ",
"NİŞ",
"NİŞAN",
"NİTE",
"NİTEL",
"NİYAZ",
"NİYE",
"NİYET",
"NİZA",
"NİZAM",
"NİZİP",
"NODUL",
"NODÜL",
"NOEL",
"NOGAY",
"NOHUT",
"NOKRA",
"NOKTA",
"NOM",
"NONOŞ",
"NORM",
"NOT",
"NOTA",
"NOTAM",
"NOTER",
"NOVA",
"NÖBET",
"NÖRON",
"NÖTR",
"NUH",
"NUKUT",
"NUMEN",
"NUR",
"NURLU",
"NUTUK",
"NÜ",
"NÜANS",
"NÜFUS",
"NÜFUZ",
"NÜKS",
"NÜKTE",
"NÜKUL",
"NÜSHA",
"NÜVE",
"NÜZUL",
"OARA",
"OBA",
"OBERJ",
"OBEZ",
"OBJE",
"OBRUK",
"OBU",
"OBUA",
"OBUR",
"OBÜS",
"OCAK",
"OCUMA",
"OD",
"ODA",
"ODACI",
"ODAK",
"ODALI",
"ODEON",
"ODSUZ",
"ODUN",
"OF",
"OFANS",
"OFİS",
"OFLAZ",
"OFRİS",
"OFSET",
"OĞAN",
"OĞLAK",
"OĞLAK",
"OĞLAN",
"OĞUL",
"OĞUZ",
"OĞUZ",
"OH",
"OHA",
"OJE",
"OJELİ",
"OJİT",
"OK",
"OKAPİ",
"OKAR",
"OKÇU",
"OKEY",
"OKKA",
"OKLUK",
"OKSİT",
"OKTAN",
"OKTAV",
"OKUL",
"OKUMA",
"OKUME",
"OKUR",
"OL",
"OLASI",
"OLAY",
"OLÇUM",
"OLDU",
"OLE",
"OLEİK",
"OLEİN",
"OLEY",
"OLGU",
"OLGUN",
"OLMA",
"OLMAK",
"OLMAZ",
"OLMUŞ",
"OLTA",
"OLTU",
"OLUK",
"OLUR",
"OLUR",
"OLURU",
"OLUŞ",
"OM",
"OMA",
"OMBRA",
"OMCA",
"OMLET",
"OMUR",
"OMUZ",
"ON",
"ONA",
"ONAMA",
"ONAR",
"ONAT",
"ONAY",
"ONCA",
"ONGEN",
"ONGUN",
"ONİKS",
"ONLAR",
"ONLU",
"ONLUK",
"ONMA",
"ONMAK",
"ONS",
"ONSUZ",
"ONUM",
"ONUR",
"OOSİT",
"OPAK",
"OPAL",
"OPERA",
"OPTİK",
"OPUS",
"ORA",
"ORADA",
"ORAK",
"ORAL",
"ORALI",
"ORAN",
"ORASI",
"ORAYA",
"ORCİK",
"ORDU",
"ORDU",
"ORFOZ",
"ORG",
"ORGAN",
"ORGCU",
"ORİON",
"ORKİT",
"ORLON",
"ORMAN",
"ORSA",
"ORTA",
"ORTA",
"ORTAÇ",
"ORTAK",
"ORTAM",
"ORTAY",
"ORTEZ",
"ORTOZ",
"ORUÇ",
"ORUN",
"ORYA",
"OSAAT",
"OT",
"OTACI",
"OTAĞ",
"OTAMA",
"OTÇU",
"OTÇUL",
"OTEL",
"OTİST",
"OTİZM",
"OTLAK",
"OTLU",
"OTLUK",
"OTSU",
"OTSUL",
"OTSUZ",
"OTUZ",
"OVA",
"OVAL",
"OVALI",
"OVMA",
"OVMAÇ",
"OVMAK",
"OY",
"OYA",
"OYACI",
"OYALI",
"OYDAŞ",
"OYLUM",
"OYMA",
"OYMAK",
"OYMAK",
"OYNAK",
"OYNAŞ",
"OYSA",
"OYUK",
"OYUM",
"OYUN",
"OYUŞ",
"OZAN",
"OZMOZ",
"OZON",
"OZUGA",
"ÖBEK",
"ÖBÜR",
"ÖBÜRÜ",
"ÖCÜ",
"ÖÇ",
"ÖÇLÜ",
"ÖD",
"ÖDEM",
"ÖDEME",
"ÖDEV",
"ÖDLEK",
"ÖDÜL",
"ÖDÜN",
"ÖDÜNÇ",
"ÖF",
"ÖFKE",
"ÖGE",
"ÖĞLE",
"ÖĞLEN",
"ÖĞREK",
"ÖĞÜN",
"ÖĞÜR",
"ÖĞÜT",
"ÖHÖ",
"ÖKÇE",
"ÖKE",
"ÖKSE",
"ÖKSÜZ",
"ÖKÜZ",
"ÖLÇEK",
"ÖLÇER",
"ÖLÇME",
"ÖLÇÜ",
"ÖLÇÜM",
"ÖLÇÜN",
"ÖLÇÜŞ",
"ÖLÇÜT",
"ÖLET",
"ÖLGÜN",
"ÖLME",
"ÖLMEK",
"ÖLMEZ",
"ÖLMÜŞ",
"ÖLÜ",
"ÖLÜK",
"ÖLÜM",
"ÖLÜŞ",
"ÖMÜR",
"ÖN",
"ÖNAD",
"ÖNCE",
"ÖNCEL",
"ÖNCÜ",
"ÖNCÜL",
"ÖNDER",
"ÖNEK",
"ÖNEL",
"ÖNEM",
"ÖNERİ",
"ÖNEZE",
"ÖNGÜN",
"ÖNKOL",
"ÖNLEM",
"ÖNLÜK",
"ÖNODA",
"ÖNSEL",
"ÖNSES",
"ÖNSÖZ",
"ÖPME",
"ÖPMEK",
"ÖPÜŞ",
"ÖRCİN",
"ÖRDEK",
"ÖREK",
"ÖREKE",
"ÖREN",
"ÖRF",
"ÖRFİ",
"ÖRGE",
"ÖRGEN",
"ÖRGÜ",
"ÖRGÜN",
"ÖRGÜT",
"ÖRK",
"ÖRME",
"ÖRMEK",
"ÖRNEK",
"ÖRS",
"ÖRTME",
"ÖRTÜ",
"ÖRTÜK",
"ÖRTÜŞ",
"ÖRÜ",
"ÖRÜCÜ",
"ÖRÜK",
"ÖRÜLÜ",
"ÖRÜM",
"ÖRÜŞ",
"ÖŞÜR",
"ÖTE",
"ÖTEKİ",
"ÖTME",
"ÖTMEK",
"ÖTRE",
"ÖTÜCÜ",
"ÖTÜRÜ",
"ÖTÜŞ",
"ÖVEÇ",
"ÖVGÜ",
"ÖVME",
"ÖVMEK",
"ÖVÜCÜ",
"ÖVÜNÇ",
"ÖVÜŞ",
"ÖYKÜ",
"ÖYLE",
"ÖZ",
"ÖZALP",
"ÖZBEK",
"ÖZDEK",
"ÖZDEN",
"ÖZDEŞ",
"ÖZE",
"ÖZEK",
"ÖZEL",
"ÖZEME",
"ÖZEN",
"ÖZENÇ",
"ÖZENİ",
"ÖZERK",
"ÖZET",
"ÖZGE",
"ÖZGÜ",
"ÖZGÜL",
"ÖZGÜN",
"ÖZGÜR",
"ÖZISI",
"ÖZLEM",
"ÖZLÜ",
"ÖZLÜK",
"ÖZNE",
"ÖZNEL",
"ÖZSEL",
"ÖZSU",
"ÖZÜR",
"ÖZÜT",
"PABUÇ",
"PAÇA",
"PAÇAL",
"PAÇOZ",
"PADOK",
"PAFTA",
"PAGAN",
"PAH",
"PAHA",
"PAHAL",
"PAK",
"PAKET",
"PAKT",
"PAL",
"PALA",
"PALAN",
"PALAS",
"PALAZ",
"PALET",
"PALTO",
"PALU",
"PAMPA",
"PAMUK",
"PANDA",
"PANEL",
"PANİK",
"PANO",
"PAPA",
"PAPAK",
"PAPAZ",
"PAPEL",
"PARA",
"PARAF",
"PARÇA",
"PARE",
"PARK",
"PARKA",
"PARKE",
"PARPA",
"PARS",
"PARSA",
"PARTİ",
"PARYA",
"PAS",
"PASAJ",
"PASAK",
"PASÇI",
"PASİF",
"PASLI",
"PASO",
"PASÖR",
"PASTA",
"PAŞA",
"PAT",
"PATA",
"PATAK",
"PATAL",
"PATEN",
"PATİ",
"PATİK",
"PATOZ",
"PAUNT",
"PAY",
"PAYAN",
"PAYDA",
"PAYE",
"PAYEN",
"PAYET",
"PAYLI",
"PAZAR",
"PAZAR",
"PAZEN",
"PAZI",
"PE",
"PEÇ",
"PEÇE",
"PEÇİÇ",
"PEDAL",
"PEDER",
"PEK",
"PEKÇE",
"PEKİ",
"PELİN",
"PELİT",
"PELTE",
"PELÜR",
"PELÜŞ",
"PEMBE",
"PENA",
"PENÇE",
"PENES",
"PENGÖ",
"PENİ",
"PENİS",
"PENS",
"PENSE",
"PENYE",
"PEPE",
"PER",
"PERDE",
"PEREN",
"PERİ",
"PERKİ",
"PERMA",
"PERMİ",
"PERON",
"PERT",
"PERUK",
"PERVA",
"PES",
"PESEK",
"PESO",
"PEST",
"PESÜS",
"PEŞ",
"PEŞİN",
"PEŞLİ",
"PEŞTU",
"PETEK",
"PEY",
"PEYDA",
"PEYK",
"PEYKE",
"PIHTI",
"PINAR",
"PIR",
"PIRPI",
"PIRTI",
"PISMA",
"PIŞT",
"PIT",
"PİÇ",
"PİDE",
"PİGME",
"PİK",
"PİKAJ",
"PİKAP",
"PİKE",
"PİKET",
"PİKO",
"PİL",
"PİLAV",
"PİLİ",
"PİLİÇ",
"PİLLİ",
"PİLOT",
"PİM",
"PİNEL",
"PİNES",
"PİNTİ",
"PİPET",
"PİPİ",
"PİPO",
"PİR",
"PİRE",
"PİRİT",
"PİS",
"PİSİ",
"PİSİK",
"PİSİN",
"PİSSU",
"PİST",
"PİŞEK",
"PİŞİ",
"PİŞİK",
"PİŞİM",
"PİŞME",
"PİŞTİ",
"PİTON",
"PİYAN",
"PİYAZ",
"PİYES",
"PİYON",
"PİZZA",
"PLAJ",
"PLAK",
"PLAKA",
"PLAN",
"PLASE",
"PLATİ",
"PLATO",
"PLAZA",
"POF",
"POG",
"POKER",
"POLAR",
"POLAT",
"POLEN",
"POLİM",
"POLİP",
"POLİS",
"POLKA",
"POLO",
"POLÜM",
"POMAK",
"POMAT",
"POMPA",
"PONJE",
"PONZA",
"POP",
"POPÇU",
"POPO",
"PORNO",
"PORTE",
"PORTO",
"POS",
"POSA",
"POSOF",
"POST",
"POSTA",
"POŞET",
"POŞU",
"POT",
"POTA",
"POTAS",
"POTİN",
"POTUK",
"POTUR",
"POY",
"POYRA",
"POZ",
"PÖÇ",
"PÖÇÜK",
"PÖF",
"PRAFA",
"PRENS",
"PRES",
"PRESE",
"PRİM",
"PRİZ",
"PROJE",
"PROVA",
"PRUVA",
"PUAN",
"PUDRA",
"PUF",
"PUFLA",
"PUHU",
"PUL",
"PULCU",
"PULLU",
"PULSU",
"PULUÇ",
"PUMA",
"PUMBA",
"PUNÇ",
"PUNT",
"PUNTO",
"PUPA",
"PURO",
"PUS",
"PUSAT",
"PUSET",
"PUSLU",
"PUSMA",
"PUSU",
"PUŞT",
"PUT",
"PUVAR",
"PÜF",
"PÜNEZ",
"PÜR",
"PÜRE",
"PÜREN",
"PÜRÜZ",
"PÜSÜR",
"PÜTÜR",
"RAB",
"RABIT",
"RACA",
"RACİ",
"RACON",
"RADAR",
"RADDE",
"RADON",
"RADYO",
"RAF",
"RAFİT",
"RAFLI",
"RAFYA",
"RAGBİ",
"RAHAT",
"RAHİM",
"RAHİM",
"RAHİP",
"RAHLE",
"RAHNE",
"RAHT",
"RAKAM",
"RAKET",
"RAKI",
"RAKIM",
"RAKİK",
"RAKİP",
"RAKİT",
"RAKOR",
"RAKS",
"RAKUN",
"RALLİ",
"RAM",
"RAMAK",
"RAMİ",
"RAMP",
"RAMPA",
"RANDA",
"RANT",
"RANZA",
"RAP",
"RAPOR",
"RASAT",
"RASIT",
"RASPA",
"RAST",
"RASYO",
"RAŞE",
"RAŞİ",
"RATE",
"RATIP",
"RAUNT",
"RAY",
"RAYBA",
"RAYİÇ",
"RAZI",
"RE",
"REAYA",
"REBAP",
"RECEP",
"RECİM",
"REÇEL",
"REDİF",
"REEL",
"REFAH",
"REFİK",
"REFÜJ",
"REHA",
"REHİN",
"REİS",
"REJİ",
"REJİM",
"REKAT",
"REKİZ",
"REKOR",
"REMEL",
"REMİ",
"REMİL",
"REMİZ",
"RENDE",
"RENK",
"REPO",
"RESEN",
"RESİF",
"RESİM",
"RESMİ",
"REST",
"RESUL",
"REŞİT",
"REŞME",
"RET",
"REVA",
"REVAÇ",
"REVAK",
"REVAN",
"REVİR",
"REVİŞ",
"REVÜ",
"REY",
"REYE",
"REYON",
"REZE",
"REZİL",
"RIH",
"RIZA",
"RIZIK",
"RİCA",
"RİCAL",
"RİCAT",
"RİJİT",
"RİKA",
"RİMEL",
"RİNA",
"RİNG",
"RİNGA",
"RİNT",
"RİSK",
"RİTİM",
"RİYA",
"RİYAL",
"RİZE",
"ROBA",
"ROBOT",
"RODA",
"RODAJ",
"RODEO",
"ROKA",
"ROKET",
"ROL",
"ROLCÜ",
"ROM",
"ROMAN",
"ROMAN",
"ROMEN",
"ROP",
"ROSTO",
"ROT",
"ROTA",
"ROTİL",
"ROTOR",
"ROZA",
"ROZE",
"ROZET",
"RÖFLE",
"RÖGAR",
"RÖLE",
"RÖTAR",
"RÖTUŞ",
"RUA",
"RUAM",
"RUBA",
"RUBAİ",
"RUBLE",
"RUF",
"RUFAİ",
"RUGAN",
"RUH",
"RUHÇA",
"RUHEN",
"RUHİ",
"RUHLU",
"RUJ",
"RUJLU",
"RULET",
"RULO",
"RUM",
"RUMBA",
"RUMCA",
"RUMİ",
"RUMİ",
"RUMUZ",
"RUN",
"RUNİK",
"RUPİ",
"RUS",
"RUSÇA",
"RUTİN",
"RUZ",
"RÜCU",
"RÜESA",
"RÜKU",
"RÜKÜN",
"RÜKÜŞ",
"RÜSUM",
"RÜSUP",
"RÜSVA",
"RÜŞT",
"RÜTBE",
"RÜYA",
"RÜYET",
"SAAT",
"SABA",
"SABAH",
"SABAN",
"SABIK",
"SABIR",
"SABİ",
"SABİT",
"SABO",
"SABUH",
"SABUN",
"SABUR",
"SAC",
"SAÇ",
"SAÇAK",
"SAÇI",
"SAÇIK",
"SAÇIŞ",
"SAÇLI",
"SAÇMA",
"SADAK",
"SADE",
"SADET",
"SADIK",
"SADIR",
"SADIR",
"SADİK",
"SADME",
"SAF",
"SAFÇA",
"SAFER",
"SAFHA",
"SAFİ",
"SAFİR",
"SAFRA",
"SAFSU",
"SAGU",
"SAĞ",
"SAĞCI",
"SAĞI",
"SAĞIM",
"SAĞIN",
"SAĞIR",
"SAĞİÇ",
"SAĞMA",
"SAĞRI",
"SAH",
"SAHA",
"SAHAF",
"SAHAN",
"SAHİ",
"SAHİH",
"SAHİL",
"SAHİP",
"SAHN",
"SAHNE",
"SAHRA",
"SAHRE",
"SAHTE",
"SAHUR",
"SAİK",
"SAİKA",
"SAİR",
"SAK",
"SAKA",
"SAKAF",
"SAKAK",
"SAKAL",
"SAKAR",
"SAKAT",
"SAKE",
"SAKIN",
"SAKIT",
"SAKIZ",
"SAKİ",
"SAKİL",
"SAKİM",
"SAKİN",
"SAKİT",
"SAKLI",
"SAKO",
"SAKSI",
"SAL",
"SALAH",
"SALAK",
"SALAM",
"SALAŞ",
"SALAT",
"SALCI",
"SALÇA",
"SALEP",
"SALGI",
"SALI",
"SALIK",
"SALİK",
"SALİM",
"SALİP",
"SALLI",
"SALMA",
"SALON",
"SALOZ",
"SALPA",
"SALSA",
"SALT",
"SALTA",
"SALTO",
"SALUR",
"SALVO",
"SALYA",
"SAM",
"SAMAN",
"SAMBA",
"SAMİ",
"SAMSA",
"SAMUR",
"SAMUT",
"SAN",
"SANA",
"SANAL",
"SANAT",
"SANCI",
"SANEM",
"SANGI",
"SANI",
"SANIK",
"SANIŞ",
"SANKİ",
"SANLI",
"SANMA",
"SANRI",
"SAP",
"SAPA",
"SAPAK",
"SAPAN",
"SAPIK",
"SAPIŞ",
"SAPKI",
"SAPLI",
"SAPMA",
"SARA",
"SARAÇ",
"SARAK",
"SARAT",
"SARAY",
"SARAY",
"SARF",
"SARGI",
"SARI",
"SARIK",
"SARIM",
"SARIŞ",
"SARIZ",
"SARİ",
"SARİ",
"SARİG",
"SARİH",
"SARMA",
"SARP",
"SARPA",
"SASI",
"SASON",
"SATEN",
"SATHİ",
"SATI",
"SATIH",
"SATIM",
"SATIR",
"SATIŞ",
"SATİR",
"SATMA",
"SAUNA",
"SAV",
"SAVA",
"SAVAK",
"SAVAN",
"SAVAŞ",
"SAVAT",
"SAVCA",
"SAVCI",
"SAVLA",
"SAVLO",
"SAVMA",
"SAVUR",
"SAY",
"SAY",
"SAYA",
"SAYAÇ",
"SAYE",
"SAYFA",
"SAYGI",
"SAYHA",
"SAYI",
"SAYIM",
"SAYIN",
"SAYIŞ",
"SAYMA",
"SAYRI",
"SAZ",
"SAZAK",
"SAZAN",
"SAZCI",
"SAZLI",
"SE",
"SEANS",
"SEBAT",
"SEBEN",
"SEBEP",
"SEBİL",
"SEBZE",
"SECDE",
"SECİ",
"SEÇAL",
"SEÇİ",
"SEÇİM",
"SEÇİŞ",
"SEÇKİ",
"SEÇME",
"SEDA",
"SEDEF",
"SEDİR",
"SEDYE",
"SEFA",
"SEFER",
"SEFİH",
"SEFİL",
"SEFİR",
"SEGAH",
"SEHER",
"SEHİM",
"SEHİV",
"SEHPA",
"SEK",
"SEKEL",
"SEKİ",
"SEKİL",
"SEKİŞ",
"SEKİZ",
"SEKME",
"SEKS",
"SEKSİ",
"SEKTE",
"SEL",
"SELA",
"SELAM",
"SELE",
"SELEF",
"SELEK",
"SELEN",
"SELİM",
"SELİM",
"SELİS",
"SELP",
"SELVA",
"SELVİ",
"SEM",
"SEM",
"SEMA",
"SEMAH",
"SEMAİ",
"SEMAN",
"SEME",
"SEMEN",
"SEMER",
"SEMİH",
"SEMİZ",
"SEMT",
"SEN",
"SENA",
"SENCE",
"SENE",
"SENEK",
"SENET",
"SENİR",
"SENİT",
"SENT",
"SEPEK",
"SEPET",
"SEPİ",
"SEPYA",
"SER",
"SERA",
"SERAK",
"SERAP",
"SERÇE",
"SERE",
"SEREN",
"SERF",
"SERGİ",
"SERİ",
"SERİK",
"SERİM",
"SERİN",
"SERİŞ",
"SERME",
"SERT",
"SERUM",
"SERVİ",
"SES",
"SESÇİ",
"SESLİ",
"SET",
"SETER",
"SETİK",
"SETİR",
"SETRE",
"SEVAP",
"SEVAP",
"SEVDA",
"SEVER",
"SEVGİ",
"SEVİ",
"SEVİM",
"SEVİR",
"SEVİŞ",
"SEVK",
"SEVME",
"SEYEK",
"SEYİR",
"SEYİS",
"SEYİT",
"SEZA",
"SEZGİ",
"SEZİ",
"SEZİŞ",
"SEZME",
"SEZON",
"SEZÜ",
"SICAK",
"SIÇAN",
"SIÇMA",
"SIDK",
"SIFAT",
"SIFIR",
"SIĞ",
"SIĞA",
"SIĞIN",
"SIĞIR",
"SIĞLA",
"SIĞMA",
"SIHHİ",
"SIHRİ",
"SIK",
"SIKÇA",
"SIKI",
"SIKIM",
"SIKIT",
"SIKMA",
"SILA",
"SIMA",
"SIMAK",
"SINAİ",
"SINAV",
"SINDI",
"SINIF",
"SINIK",
"SINIR",
"SINMA",
"SIPA",
"SIR",
"SIR",
"SIRA",
"SIRAT",
"SIRCI",
"SIRÇA",
"SIRF",
"SIRIK",
"SIRIM",
"SIRLI",
"SIRMA",
"SIRP",
"SIRT",
"SISKA",
"SITMA",
"SIVA",
"SIVI",
"SIVIK",
"SIYGA",
"SIYGI",
"SIZAK",
"SIZI",
"SIZIŞ",
"SIZMA",
"Sİ",
"SİBOP",
"SİCİL",
"SİCİM",
"SİDİK",
"SİF",
"SİFİN",
"SİFON",
"SİGAR",
"SİĞİL",
"SİH",
"SİHİR",
"SİİRT",
"SİK",
"SİKKE",
"SİKME",
"SİLAH",
"SİLAJ",
"SİLGİ",
"SİLİ",
"SİLİK",
"SİLİS",
"SİLİŞ",
"SİLKİ",
"SİLLE",
"SİLME",
"SİLO",
"SİM",
"SİMA",
"SİMAV",
"SİMGE",
"SİMİT",
"SİMYA",
"SİN",
"SİN",
"SİNE",
"SİNEK",
"SİNİ",
"SİNİK",
"SİNİR",
"SİNİŞ",
"SİNLE",
"SİNME",
"SİNOP",
"SİNSİ",
"SİNÜS",
"SİPER",
"SİPSİ",
"SİREN",
"SİRK",
"SİRKE",
"SİRMO",
"SİROZ",
"SİRTO",
"SİS",
"SİSLİ",
"SİT",
"SİTE",
"SİTEM",
"SİTİL",
"SİVAS",
"SİVİL",
"SİVRİ",
"SİYA",
"SİYAH",
"SİYAK",
"SİYEK",
"SİYER",
"SİYME",
"SİZ",
"SİZCE",
"SKALA",
"SKEÇ",
"SKİ",
"SKİF",
"SKİNK",
"SKOR",
"SLAP",
"SLAV",
"SLAYT",
"SLİP",
"SMAÇ",
"SNOP",
"SOBA",
"SOBE",
"SODA",
"SOF",
"SOFA",
"SOFRA",
"SOFTA",
"SOFU",
"SOĞAN",
"SOĞUK",
"SOKAK",
"SOKET",
"SOKMA",
"SOKRA",
"SOKU",
"SOKUM",
"SOKUR",
"SOKUŞ",
"SOL",
"SOL",
"SOLAK",
"SOLCU",
"SOLİÇ",
"SOLMA",
"SOLO",
"SOLUK",
"SOLUK",
"SOLUŞ",
"SOM",
"SOMA",
"SOMA",
"SOMAK",
"SOMON",
"SOMUN",
"SOMUT",
"SOMYA",
"SON",
"SONAR",
"SONAT",
"SONDA",
"SONE",
"SONEK",
"SONLU",
"SONRA",
"SONUÇ",
"SOPA",
"SORGU",
"SORİT",
"SORMA",
"SORTİ",
"SORU",
"SORUM",
"SORUN",
"SORUŞ",
"SOS",
"SOSİS",
"SOTA",
"SOTE",
"SOY",
"SOYA",
"SOYKA",
"SOYLU",
"SOYMA",
"SOYUM",
"SOYUŞ",
"SOYUT",
"SÖBE",
"SÖĞÜŞ",
"SÖĞÜT",
"SÖĞÜT",
"SÖKE",
"SÖKEL",
"SÖKME",
"SÖKÜK",
"SÖKÜM",
"SÖKÜN",
"SÖKÜŞ",
"SÖLOM",
"SÖNME",
"SÖNÜK",
"SÖNÜM",
"SÖNÜŞ",
"SÖR",
"SÖRF",
"SÖVE",
"SÖVEN",
"SÖVGÜ",
"SÖVME",
"SÖVÜŞ",
"SÖZ",
"SÖZCE",
"SÖZCÜ",
"SÖZDE",
"SÖZEL",
"SÖZLÜ",
"SPAZM",
"SPERM",
"SPOR",
"SPOT",
"SPREY",
"STAJ",
"STANT",
"STAR",
"STAT",
"STATÜ",
"STEN",
"STENO",
"STENT",
"STEP",
"STEP",
"STER",
"STİL",
"STİLO",
"STOK",
"STOR",
"STREÇ",
"STRES",
"SU",
"SUAL",
"SUARE",
"SUBAY",
"SUBRA",
"SUBYE",
"SUCU",
"SUCUK",
"SUCUL",
"SUÇ",
"SUÇLU",
"SUDAK",
"SUDAN",
"SUFİ",
"SUFLE",
"SUKUT",
"SULAK",
"SULH",
"SULP",
"SULTA",
"SULU",
"SULUK",
"SUMA",
"SUMAK",
"SUNA",
"SUNAK",
"SUNGU",
"SUNİ",
"SUNMA",
"SUNTA",
"SUNU",
"SUNUM",
"SUNUŞ",
"SUOKU",
"SUP",
"SUPAP",
"SUR",
"SUR",
"SURA",
"SURAT",
"SURE",
"SURET",
"SURUÇ",
"SUSAK",
"SUSAM",
"SUSKU",
"SUSMA",
"SUSTA",
"SUSUŞ",
"SUSUZ",
"SUSUZ",
"SUŞİ",
"SUT",
"SUTAŞ",
"SUTLU",
"SUVAT",
"SUYUK",
"SÜBEK",
"SÜBUT",
"SÜBYE",
"SÜCUT",
"SÜET",
"SÜFLİ",
"SÜĞME",
"SÜİT",
"SÜJE",
"SÜKSE",
"SÜKUN",
"SÜKUT",
"SÜLF",
"SÜLUK",
"SÜLÜK",
"SÜLÜN",
"SÜLÜS",
"SÜMEK",
"SÜMEN",
"SÜMER",
"SÜMÜK",
"SÜNE",
"SÜNGÜ",
"SÜNME",
"SÜNNİ",
"SÜPER",
"SÜRAT",
"SÜRE",
"SÜREÇ",
"SÜREK",
"SÜRFE",
"SÜRGÜ",
"SÜRME",
"SÜRRE",
"SÜRUR",
"SÜRÜ",
"SÜRÜM",
"SÜRÜŞ",
"SÜS",
"SÜSEN",
"SÜSLÜ",
"SÜSME",
"SÜT",
"SÜTÇÜ",
"SÜTLÜ",
"SÜTRE",
"SÜTSÜ",
"SÜTUN",
"SÜVEN",
"SÜVME",
"SÜYEK",
"SÜYÜM",
"SÜZEK",
"SÜZGÜ",
"SÜZME",
"SÜZÜK",
"SÜZÜŞ",
"ŞABAN",
"ŞAD",
"ŞAFAK",
"ŞAFİİ",
"ŞAFT",
"ŞAFUL",
"ŞAH",
"ŞAHAP",
"ŞAHIS",
"ŞAHİN",
"ŞAHİT",
"ŞAHNE",
"ŞAHSİ",
"ŞAİBE",
"ŞAİR",
"ŞAİRE",
"ŞAK",
"ŞAK",
"ŞAKA",
"ŞAKAK",
"ŞAKİ",
"ŞAKUL",
"ŞAL",
"ŞALAK",
"ŞALE",
"ŞALİ",
"ŞALT",
"ŞAMA",
"ŞAMAN",
"ŞAMAN",
"ŞAMAR",
"ŞAMİL",
"ŞAN",
"ŞANLI",
"ŞANO",
"ŞANS",
"ŞAP",
"ŞAPÇI",
"ŞAPEL",
"ŞAPKA",
"ŞAPLI",
"ŞARAP",
"ŞARJ",
"ŞARK",
"ŞARK",
"ŞARKI",
"ŞARKİ",
"ŞARPİ",
"ŞART",
"ŞARYO",
"ŞASE",
"ŞASİ",
"ŞAŞAA",
"ŞAŞI",
"ŞAŞMA",
"ŞAT",
"ŞATIR",
"ŞATO",
"ŞAVK",
"ŞAVUL",
"ŞAYAK",
"ŞAYAN",
"ŞAYET",
"ŞAYİ",
"ŞAYİA",
"ŞAYKA",
"ŞAZ",
"ŞE",
"ŞEB",
"ŞEBEK",
"ŞECİ",
"ŞEDDE",
"ŞEDİT",
"ŞEF",
"ŞEFİK",
"ŞEHİR",
"ŞEHİT",
"ŞEHLA",
"ŞEK",
"ŞEKEL",
"ŞEKER",
"ŞEKİL",
"ŞEKLİ",
"ŞEKVA",
"ŞELEK",
"ŞELF",
"ŞEM",
"ŞEMA",
"ŞEMS",
"ŞEMSE",
"ŞEMSİ",
"ŞEN",
"ŞENİ",
"ŞEPİT",
"ŞER",
"ŞERAN",
"ŞERÇ",
"ŞEREF",
"ŞERH",
"ŞERHA",
"ŞERİ",
"ŞERİF",
"ŞERİK",
"ŞERİR",
"ŞERİT",
"ŞERPA",
"ŞEŞ",
"ŞET",
"ŞETİM",
"ŞEV",
"ŞEVK",
"ŞEY",
"ŞEYH",
"ŞIK",
"ŞIK",
"ŞINAV",
"ŞIP",
"ŞIPKA",
"ŞIRA",
"ŞIRAK",
"ŞİA",
"ŞİAR",
"ŞİF",
"ŞİFA",
"ŞİFON",
"ŞİFRE",
"Şİİ",
"ŞİİR",
"ŞİKAR",
"ŞİKE",
"ŞİLE",
"ŞİLE",
"ŞİLEM",
"ŞİLEP",
"ŞİLİN",
"ŞİLT",
"ŞİLTE",
"ŞİMAL",
"ŞİMDİ",
"ŞİNİK",
"ŞİNTO",
"ŞİP",
"ŞİRAN",
"ŞİRİN",
"ŞİRK",
"ŞİST",
"ŞİŞ",
"ŞİŞE",
"ŞİŞEK",
"ŞİŞİK",
"ŞİŞKO",
"ŞİŞLİ",
"ŞİŞME",
"ŞİTA",
"ŞİVE",
"ŞOFÖR",
"ŞOK",
"ŞOKE",
"ŞOM",
"ŞOPAR",
"ŞOR",
"ŞORCA",
"ŞORT",
"ŞOSE",
"ŞOSET",
"ŞOSON",
"ŞOV",
"ŞOVEN",
"ŞÖLEN",
"ŞÖMİZ",
"ŞÖYLE",
"ŞU",
"ŞUA",
"ŞUARA",
"ŞUBAT",
"ŞUBE",
"ŞUBU",
"ŞUFA",
"ŞUH",
"ŞUHUT",
"ŞULE",
"ŞUNCA",
"ŞURA",
"ŞURA",
"ŞURUP",
"ŞUT",
"ŞUUR",
"ŞÜKÜR",
"ŞÜMUL",
"ŞÜPHE",
"ŞÜYU",
"TA",
"TAAM",
"TAAT",
"TAB",
"TABA",
"TABAK",
"TABAN",
"TABİ",
"TABİİ",
"TABİP",
"TABİR",
"TABL",
"TABLA",
"TABLO",
"TABU",
"TABUR",
"TABUT",
"TABYA",
"TACİK",
"TACİL",
"TACİR",
"TACİZ",
"TAÇ",
"TAÇLI",
"TADAT",
"TADIM",
"TADİL",
"TAFRA",
"TAFTA",
"TAHIL",
"TAHİN",
"TAHRA",
"TAHT",
"TAHTA",
"TAK",
"TAKA",
"TAKAS",
"TAKAT",
"TAKI",
"TAKIK",
"TAKIM",
"TAKİM",
"TAKİP",
"TAKKE",
"TAKLA",
"TAKMA",
"TAKOZ",
"TAKSA",
"TAKSİ",
"TAKT",
"TAKTİ",
"TAKVA",
"TAL",
"TALAK",
"TALAN",
"TALAS",
"TALAŞ",
"TALAZ",
"TALEP",
"TALİ",
"TALİH",
"TALİK",
"TALİL",
"TALİM",
"TALİP",
"TALK",
"TAM",
"TAMAH",
"TAMAM",
"TAMER",
"TAMİK",
"TAMİM",
"TAMİR",
"TAMU",
"TAN",
"TANE",
"TANEN",
"TANGO",
"TANI",
"TANIK",
"TANIM",
"TANIŞ",
"TANIT",
"TANİN",
"TANK",
"TANRI",
"TANRI",
"TAOCU",
"TAPA",
"TAPAN",
"TAPI",
"TAPIŞ",
"TAPİ",
"TAPİR",
"TAPMA",
"TAPON",
"TAPU",
"TAR",
"TARAF",
"TARAK",
"TARAŞ",
"TARAZ",
"TARET",
"TARH",
"TARIM",
"TARİF",
"TARİH",
"TARİK",
"TARİZ",
"TARLA",
"TART",
"TART",
"TARTI",
"TARZ",
"TAS",
"TASA",
"TASAR",
"TASDİ",
"TASIM",
"TASMA",
"TASNİ",
"TAŞ",
"TAŞAK",
"TAŞÇI",
"TAŞIL",
"TAŞIM",
"TAŞIT",
"TAŞLI",
"TAŞMA",
"TAŞRA",
"TAŞSI",
"TAT",
"TAT",
"TAT",
"TATAR",
"TATAR",
"TATİL",
"TATLI",
"TATMA",
"TAUN",
"TAV",
"TAVA",
"TAVAF",
"TAVAN",
"TAVAS",
"TAVCI",
"TAVIR",
"TAVİK",
"TAVİZ",
"TAVLA",
"TAVLI",
"TAVUK",
"TAVUS",
"TAY",
"TAYA",
"TAYF",
"TAYFA",
"TAYGA",
"TAYIN",
"TAYİN",
"TAYİP",
"TAYT",
"TAZE",
"TAZI",
"TAZİM",
"TAZİP",
"TAZİZ",
"TE",
"TEALİ",
"TEATİ",
"TEBAA",
"TEBER",
"TECİL",
"TECİM",
"TEDAİ",
"TEDİP",
"TEF",
"TEFE",
"TEĞET",
"TEHİ",
"TEHİR",
"TEİN",
"TEİST",
"TEİZM",
"TEK",
"TEKÇİ",
"TEKE",
"TEKEL",
"TEKER",
"TEKİL",
"TEKİN",
"TEKİR",
"TEKİT",
"TEKKE",
"TEKLİ",
"TEKME",
"TEKNE",
"TEKST",
"TEL",
"TELA",
"TELAŞ",
"TELEF",
"TELEK",
"TELEM",
"TELES",
"TELİF",
"TELİN",
"TELİS",
"TELLİ",
"TELSİ",
"TELVE",
"TEM",
"TEMA",
"TEMAS",
"TEMEK",
"TEMEL",
"TEMİN",
"TEMİZ",
"TEMPO",
"TEN",
"TENGE",
"TENHA",
"TENİS",
"TENOR",
"TENTE",
"TENYA",
"TEORİ",
"TEPE",
"TEPİ",
"TEPİK",
"TEPİR",
"TEPİŞ",
"TEPKE",
"TEPKİ",
"TEPME",
"TEPSİ",
"TER",
"TERAS",
"TERBİ",
"TERE",
"TEREK",
"TERES",
"TERFİ",
"TERİM",
"TERK",
"TERKİ",
"TERLİ",
"TERME",
"TERME",
"TERÖR",
"TERS",
"TERZİ",
"TESİR",
"TESİS",
"TESİT",
"TESRİ",
"TEST",
"TESTİ",
"TEŞCİ",
"TEŞNE",
"TEŞRİ",
"TEŞT",
"TEŞYİ",
"TETİK",
"TETİK",
"TETİR",
"TEVDİ",
"TEVEK",
"TEVİL",
"TEVKİ",
"TEVSİ",
"TEVZİ",
"TEYEL",
"TEYİT",
"TEYP",
"TEYZE",
"TEZ",
"TEZAT",
"TEZCE",
"TEZEK",
"TEZLİ",
"TIBBİ",
"TIFIL",
"TIĞ",
"TIK",
"TIKAÇ",
"TIKIM",
"TIKIR",
"TIKIZ",
"TIKMA",
"TIMAR",
"TIN",
"TINAZ",
"TINI",
"TINMA",
"TIP",
"TIPA",
"TIPKI",
"TIR",
"TIRAK",
"TIRAŞ",
"TIRIK",
"TIRIL",
"TIRIS",
"TIRT",
"TIS",
"Tİ",
"TİFO",
"TİFÜS",
"TİK",
"TİKE",
"TİKEL",
"TİLKİ",
"TİM",
"TİMÜS",
"TİN",
"TİNER",
"TİP",
"TİPİ",
"TİPİK",
"TİPO",
"TİRAJ",
"TİRAN",
"TİRAT",
"TİRE",
"TİRE",
"TİRİT",
"TİRİZ",
"TİRLE",
"TİRSİ",
"TİRŞE",
"TİTAN",
"TİTİZ",
"TİTR",
"TİZ",
"TOFU",
"TOHT",
"TOHUM",
"TOK",
"TOKA",
"TOKAÇ",
"TOKAT",
"TOKAT",
"TOKLU",
"TOKUZ",
"TOKYO",
"TOL",
"TOLGA",
"TOMAK",
"TOMAR",
"TON",
"TONAJ",
"TONER",
"TONGA",
"TONİK",
"TONLA",
"TONLU",
"TONOZ",
"TONYA",
"TOP",
"TOPAÇ",
"TOPAK",
"TOPAL",
"TOPAZ",
"TOPÇU",
"TOPİK",
"TOPLA",
"TOPLU",
"TOPU",
"TOPUK",
"TOPUR",
"TOPUT",
"TOPUZ",
"TOR",
"TORAK",
"TORBA",
"TORİK",
"TORK",
"TORNA",
"TORTU",
"TORUL",
"TORUM",
"TORUN",
"TOS",
"TOST",
"TOSUN",
"TOSYA",
"TOTAL",
"TOTEM",
"TOTO",
"TOY",
"TOYCA",
"TOYCU",
"TOYGA",
"TOZ",
"TOZAN",
"TOZLU",
"TOZMA",
"TÖR",
"TÖRE",
"TÖREL",
"TÖREN",
"TÖRPÜ",
"TÖS",
"TÖVBE",
"TÖZ",
"TÖZEL",
"TRAFO",
"TRAKE",
"TRANS",
"TRANŞ",
"TRAP",
"TRAS",
"TRATA",
"TREN",
"TRİKO",
"TRİO",
"TROK",
"TROL",
"TROMP",
"TRÖST",
"TRUP",
"TRÜK",
"TU",
"TUBA",
"TUBA",
"TUFAN",
"TUGAY",
"TUĞ",
"TUĞCU",
"TUĞLA",
"TUĞLU",
"TUĞRA",
"TUH",
"TUHAF",
"TUL",
"TULU",
"TULUK",
"TULUM",
"TULUP",
"TUMAN",
"TUMBA",
"TUMP",
"TUN",
"TUNÇ",
"TUNİK",
"TUR",
"TURA",
"TURAÇ",
"TURAN",
"TURBA",
"TURBO",
"TURFA",
"TURNA",
"TURNE",
"TURNO",
"TURP",
"TURŞU",
"TURTA",
"TUŞ",
"TUT",
"TUTAÇ",
"TUTAK",
"TUTAK",
"TUTAM",
"TUTAR",
"TUTKU",
"TUTMA",
"TUTU",
"TUTUK",
"TUTUM",
"TUTUŞ",
"TUTYA",
"TUVAL",
"TUYUĞ",
"TUZ",
"TUZAK",
"TUZCU",
"TUZLA",
"TUZLA",
"TUZLU",
"TUZSU",
"TÜF",
"TÜFEK",
"TÜH",
"TÜL",
"TÜLÜ",
"TÜM",
"TÜMCE",
"TÜMEL",
"TÜMEN",
"TÜMEY",
"TÜMÖR",
"TÜMÜR",
"TÜN",
"TÜNEK",
"TÜNEL",
"TÜNME",
"TÜP",
"TÜPÇÜ",
"TÜPLÜ",
"TÜR",
"TÜRAP",
"TÜRBE",
"TÜRE",
"TÜREL",
"TÜREV",
"TÜRK",
"TÜRKİ",
"TÜRKÜ",
"TÜRLÜ",
"TÜRÜM",
"TÜTME",
"TÜTSÜ",
"TÜTÜN",
"TÜVİT",
"TÜY",
"TÜYLÜ",
"TÜYME",
"TÜYO",
"TÜZE",
"TÜZEL",
"TÜZÜK",
"TVİST",
"UCA",
"UCUBE",
"UCUZ",
"UÇ",
"UÇAK",
"UÇAR",
"UÇARI",
"UÇKUN",
"UÇKUR",
"UÇLU",
"UÇMA",
"UÇMAK",
"UÇMAK",
"UÇMAN",
"UÇSUZ",
"UÇUCA",
"UÇUCU",
"UÇUK",
"UÇUK",
"UÇUN",
"UÇUŞ",
"UDİ",
"UF",
"UFAK",
"UFKİ",
"UFUK",
"UFUL",
"UĞRA",
"UĞRAK",
"UĞRAŞ",
"UĞRU",
"UĞRUN",
"UĞUR",
"UĞUR",
"UĞUT",
"UHDE",
"UKALA",
"UKDE",
"UKNUM",
"ULA",
"ULAÇ",
"ULAH",
"ULAK",
"ULAM",
"ULAMA",
"ULAN",
"ULAŞ",
"ULEMA",
"ULU",
"ULUFE",
"ULUM",
"ULUMA",
"ULUS",
"ULUS",
"ULVİ",
"UMACI",
"UMAR",
"UMDE",
"UMMA",
"UMMAK",
"UMMAN",
"UMRE",
"UMU",
"UMUM",
"UMUMİ",
"UMUR",
"UMUŞ",
"UMUT",
"UN",
"UNCU",
"UNLUK",
"UNSU",
"UNSUR",
"UNVAN",
"UR",
"URAN",
"URAY",
"URBA",
"URBAN",
"URGAN",
"URLA",
"URUK",
"URUP",
"US",
"USANÇ",
"USARE",
"USÇU",
"USKUR",
"USLU",
"USSAL",
"USTA",
"USUL",
"USUL",
"UŞAK",
"UŞAK",
"UŞKUN",
"UŞŞAK",
"UT",
"UT",
"UTANÇ",
"UTÇU",
"UTKU",
"UTLU",
"UTMA",
"UTMAK",
"UYAK",
"UYARI",
"UYDU",
"UYGAR",
"UYGUN",
"UYGUR",
"UYKU",
"UYLUK",
"UYMA",
"UYMAK",
"UYMAZ",
"UYRUK",
"UYSAL",
"UYUM",
"UYUMA",
"UYUR",
"UYUZ",
"UZ",
"UZAK",
"UZAM",
"UZAMA",
"UZAY",
"UZİ",
"UZLET",
"UZLUK",
"UZMAN",
"UZO",
"UZUN",
"UZUV",
"UZVİ",
"ÜCRA",
"ÜCRET",
"ÜÇ",
"ÜÇBEŞ",
"ÜÇBİR",
"ÜÇER",
"ÜÇGEN",
"ÜÇGÜL",
"ÜÇİKİ",
"ÜÇKAT",
"ÜÇLER",
"ÜÇLÜ",
"ÜÇLÜK",
"ÜÇTAŞ",
"ÜÇÜZ",
"ÜDEBA",
"ÜFLEÇ",
"ÜĞRÜM",
"ÜLEN",
"ÜLEŞ",
"ÜLFET",
"ÜLGER",
"ÜLKE",
"ÜLKER",
"ÜLKÜ",
"ÜLSER",
"ÜLÜŞ",
"ÜMERA",
"ÜMİT",
"ÜMMET",
"ÜMMİ",
"ÜMRAN",
"ÜMÜK",
"ÜN",
"ÜNDEŞ",
"ÜNİK",
"ÜNİTE",
"ÜNLEM",
"ÜNLÜ",
"ÜNSÜZ",
"ÜNYE",
"ÜRAT",
"ÜRE",
"ÜREM",
"ÜREME",
"ÜREMİ",
"ÜRGÜP",
"ÜRKEK",
"ÜRKME",
"ÜRKÜ",
"ÜRKÜŞ",
"ÜRÜME",
"ÜRÜN",
"ÜRYAN",
"ÜS",
"ÜSERA",
"ÜSKÜF",
"ÜSLUP",
"ÜST",
"ÜSTAT",
"ÜSTE",
"ÜSTEL",
"ÜSTLÜ",
"ÜSTÜN",
"ÜŞENÇ",
"ÜŞME",
"ÜŞMEK",
"ÜŞÜME",
"ÜTME",
"ÜTMEK",
"ÜTÜ",
"ÜTÜCÜ",
"ÜTÜLÜ",
"ÜVEY",
"ÜVEZ",
"ÜYE",
"ÜZERE",
"ÜZERİ",
"ÜZGÜ",
"ÜZGÜN",
"ÜZLÜK",
"ÜZME",
"ÜZMEK",
"ÜZÜCÜ",
"ÜZÜM",
"ÜZÜNÇ",
"VAAT",
"VAAZ",
"VACİP",
"VADE",
"VADİ",
"VAGON",
"VAH",
"VAHA",
"VAHİ",
"VAHİM",
"VAHİT",
"VAHİY",
"VAHŞİ",
"VAİZ",
"VAKA",
"VAKAR",
"VAKFE",
"VAKIA",
"VAKIA",
"VAKIF",
"VAKIF",
"VAKİ",
"VAKİT",
"VAKS",
"VAKUM",
"VAKUR",
"VALE",
"VALF",
"VALİ",
"VALİZ",
"VALÖR",
"VALS",
"VAMP",
"VAN",
"VANA",
"VANLI",
"VAPUR",
"VAR",
"VARAK",
"VARAN",
"VARDA",
"VARGI",
"VARIŞ",
"VARİL",
"VARİS",
"VARİS",
"VARİT",
"VARMA",
"VAROŞ",
"VARTA",
"VARTO",
"VASAT",
"VASIF",
"VASIL",
"VASİ",
"VASİ",
"VASL",
"VAŞAK",
"VAT",
"VATAN",
"VATKA",
"VATOZ",
"VAY",
"VAZ",
"VAZIH",
"VAZO",
"VE",
"VEBA",
"VEBAL",
"VECA",
"VECİH",
"VECİZ",
"VECT",
"VEÇHE",
"VEDA",
"VEDİA",
"VEFA",
"VEFAT",
"VEHİM",
"VEKİL",
"VELET",
"VELİ",
"VELUR",
"VELUT",
"VENA",
"VENÜS",
"VERDİ",
"VERE",
"VEREM",
"VEREV",
"VERGİ",
"VERİ",
"VERİM",
"VERİŞ",
"VERİT",
"VERME",
"VETO",
"VEYA",
"VEZİN",
"VEZİR",
"VEZNE",
"VICIK",
"VIN",
"VIZ",
"VİDA",
"VİDEO",
"VİDO",
"VİGLA",
"VİLLA",
"VİNÇ",
"VİRA",
"VİRAJ",
"VİRAL",
"VİRAN",
"VİRT",
"VİRÜS",
"VİSAL",
"VİSKİ",
"VİŞNE",
"VİTES",
"VİTİR",
"VİYA",
"VİYAK",
"VİYOL",
"VİZE",
"VİZE",
"VİZON",
"VİZÖR",
"VOKAL",
"VOLAN",
"VOLE",
"VOLİ",
"VOLT",
"VOLTA",
"VONOZ",
"VOTKA",
"VOYVO",
"VUKU",
"VUKUF",
"VULVA",
"VURAÇ",
"VURGU",
"VURMA",
"VURU",
"VURUK",
"VURUŞ",
"VUSUL",
"VUZUH",
"VÜCUT",
"VÜRUT",
"VÜSAT",
"VYAKA",
"YA",
"YABA",
"YABAN",
"YABGU",
"YAD",
"YAD",
"YADA",
"YADEL",
"YAFTA",
"YAĞ",
"YAĞAR",
"YAĞCI",
"YAĞI",
"YAĞIR",
"YAĞIŞ",
"YAĞIZ",
"YAĞLI",
"YAĞMA",
"YAĞSI",
"YAHEY",
"YAHNİ",
"YAHŞİ",
"YAHU",
"YAHUT",
"YAK",
"YAKA",
"YAKI",
"YAKIM",
"YAKIN",
"YAKIŞ",
"YAKIT",
"YAKİN",
"YAKMA",
"YAKUT",
"YAKUT",
"YAL",
"YALAK",
"YALAN",
"YALAZ",
"YALI",
"YALIM",
"YALIN",
"YALIZ",
"YALPA",
"YALPI",
"YAMA",
"YAMAÇ",
"YAMAK",
"YAMAN",
"YAMÇI",
"YAMUK",
"YAN",
"YANAK",
"YANAL",
"YANAY",
"YANAZ",
"YANCI",
"YANGI",
"YANIK",
"YANIŞ",
"YANIT",
"YANİ",
"YANKI",
"YANLI",
"YANMA",
"YANSI",
"YAPAK",
"YAPAY",
"YAPI",
"YAPIK",
"YAPIM",
"YAPIŞ",
"YAPIT",
"YAPMA",
"YAR",
"YAR",
"YARA",
"YARAK",
"YARAN",
"YARAR",
"YARAŞ",
"YARDA",
"YAREN",
"YARGI",
"YARI",
"YARIK",
"YARIM",
"YARIN",
"YARIŞ",
"YARKA",
"YARMA",
"YAS",
"YASA",
"YASAK",
"YASAL",
"YASİN",
"YASLI",
"YASMA",
"YASSI",
"YAŞ",
"YAŞA",
"YAŞAM",
"YAŞÇA",
"YAŞIT",
"YAŞLI",
"YAT",
"YATAK",
"YATAY",
"YATÇI",
"YATI",
"YATIK",
"YATIM",
"YATIR",
"YATIŞ",
"YATMA",
"YATSI",
"YAVAN",
"YAVAŞ",
"YAVE",
"YAVER",
"YAVRU",
"YAVSI",
"YAVUZ",
"YAY",
"YAY",
"YAYA",
"YAYAN",
"YAYGI",
"YAYIK",
"YAYIM",
"YAYIN",
"YAYIŞ",
"YAYLA",
"YAYLI",
"YAYMA",
"YAZ",
"YAZAR",
"YAZGI",
"YAZI",
"YAZIK",
"YAZIM",
"YAZIN",
"YAZIR",
"YAZIŞ",
"YAZIT",
"YAZMA",
"YE",
"YEDEK",
"YEDİ",
"YEDİZ",
"YEDME",
"YEGAH",
"YEGAN",
"YEĞ",
"YEĞEN",
"YEĞİN",
"YEĞNİ",
"YEİS",
"YEK",
"YEKE",
"YEKTA",
"YEKUN",
"YEL",
"YELE",
"YELEÇ",
"YELEK",
"YELİN",
"YELLİ",
"YELME",
"YELVE",
"YEM",
"YEMCİ",
"YEME",
"YEMEK",
"YEMEK",
"YEMİN",
"YEMİŞ",
"YEN",
"YENGE",
"YENGİ",
"YENİ",
"YENİK",
"YENİK",
"YENLİ",
"YENME",
"YER",
"YEREL",
"YEREY",
"YERGİ",
"YERLİ",
"YERME",
"YEŞİL",
"YEŞİM",
"YETER",
"YETİ",
"YETİK",
"YETİM",
"YETKE",
"YETKİ",
"YETME",
"YEVMİ",
"YEYGİ",
"YEZİT",
"YIĞIN",
"YIĞIŞ",
"YIĞMA",
"YIKI",
"YIKIK",
"YIKIM",
"YIKIŞ",
"YIKMA",
"YIL",
"YILAN",
"YILGI",
"YILIK",
"YILKI",
"YILMA",
"YIR",
"YIRIK",
"YISA",
"YIVA",
"YİĞİT",
"YİNE",
"YİRİK",
"YİRMİ",
"YİTİK",
"YİTİM",
"YİTİŞ",
"YİTME",
"YİV",
"YİVLİ",
"YİYİM",
"YİYİŞ",
"YO",
"YOBAZ",
"YOGA",
"YOGİ",
"YOĞ",
"YOĞUN",
"YOK",
"YOKÇU",
"YOKSA",
"YOKUŞ",
"YOL",
"YOLAK",
"YOLCU",
"YOLLU",
"YOLMA",
"YOLUK",
"YOM",
"YOMA",
"YOMRA",
"YONCA",
"YONGA",
"YONMA",
"YONT",
"YONTU",
"YORGA",
"YORMA",
"YORTU",
"YORUM",
"YOSMA",
"YOSUN",
"YOZ",
"YÖN",
"YÖNLÜ",
"YÖRE",
"YÖRÜK",
"YUDUM",
"YUF",
"YUFKA",
"YUH",
"YUHA",
"YUKA",
"YUKAÇ",
"YULAF",
"YULAR",
"YUMA",
"YUMAK",
"YUMAK",
"YUMMA",
"YUMRU",
"YUMUK",
"YUNA",
"YUNAK",
"YUNAK",
"YUNAN",
"YUNMA",
"YUNUS",
"YURDU",
"YURT",
"YUTAK",
"YUTMA",
"YUTUM",
"YUTUŞ",
"YUVA",
"YUVAK",
"YUVAR",
"YUVGU",
"YÜCE",
"YÜK",
"YÜKÇÜ",
"YÜKLÜ",
"YÜKÜM",
"YÜKÜN",
"YÜLGÜ",
"YÜLÜK",
"YÜN",
"YÜNLÜ",
"YÜREK",
"YÜRÜK",
"YÜSRÜ",
"YÜZ",
"YÜZDE",
"YÜZER",
"YÜZEY",
"YÜZLÜ",
"YÜZME",
"YÜZÜK",
"YÜZÜŞ",
"ZAAF",
"ZABIT",
"ZABİT",
"ZAÇ",
"ZADE",
"ZAFER",
"ZAĞ",
"ZAĞAR",
"ZAĞCI",
"ZAĞLI",
"ZAHİR",
"ZAHİR",
"ZAHİT",
"ZAİL",
"ZAİT",
"ZALİM",
"ZAM",
"ZAMAN",
"ZAMİR",
"ZAMK",
"ZAMLI",
"ZAMME",
"ZAN",
"ZANKA",
"ZANLI",
"ZAPT",
"ZAR",
"ZARA",
"ZARAR",
"ZARCI",
"ZARF",
"ZARİF",
"ZARSI",
"ZARTA",
"ZAT",
"ZATEN",
"ZATİ",
"ZATİ",
"ZAYIF",
"ZAYİ",
"ZAZA",
"ZE",
"ZEBAN",
"ZEBRA",
"ZEBUN",
"ZEBUR",
"ZECİR",
"ZECRİ",
"ZEFİR",
"ZEHAP",
"ZEHİR",
"ZEKA",
"ZEKAT",
"ZEKER",
"ZEKİ",
"ZELİL",
"ZELVE",
"ZEM",
"ZEMİN",
"ZEN",
"ZENCİ",
"ZENNE",
"ZER",
"ZERDE",
"ZERK",
"ZERRE",
"ZEVAL",
"ZEVAT",
"ZEVCE",
"ZEVÇ",
"ZEVK",
"ZEYİL",
"ZIBIN",
"ZIH",
"ZIHLI",
"ZIMBA",
"ZIMNİ",
"ZINK",
"ZIP",
"ZIPIR",
"ZIPKA",
"ZIRH",
"ZIRVA",
"ZIT",
"ZİFAF",
"ZİFİN",
"ZİFİR",
"ZİFOS",
"ZİFT",
"ZİGON",
"ZİGOT",
"ZİHAF",
"ZİHİN",
"ZİHNİ",
"ZİKİR",
"ZİL",
"ZİLE",
"ZİLİ",
"ZİLLİ",
"ZİMMİ",
"ZİNA",
"ZİNDE",
"ZİR",
"ZİRA",
"ZİRAİ",
"ZİRVE",
"ZİYA",
"ZİYAN",
"ZLOTİ",
"ZOKA",
"ZOM",
"ZOMBİ",
"ZONA",
"ZONK",
"ZOR",
"ZORBA",
"ZORCA",
"ZORGU",
"ZORLA",
"ZORLU",
"ZORUN",
"ZUHUR",
"ZULA",
"ZULÜM",
"ZUM",
"ZURNA",
"ZÜBDE",
"ZÜHAL",
"ZÜHRE",
"ZÜHT",
"ZÜHUL",
"ZÜKAM",
"ZÜL",
"ZÜLAL",
"ZÜLÜF",
"ZÜMRE",
"ZÜPPE",
"ZÜRRA",
"ZÜYUF",
];
const DICTIONARY = new Set(WORD_ARRAY.map(word => 
    word.toLocaleUpperCase('tr-TR')
));

function isValidWord(word) {
    if (word.length < 2 || word.length > 5) {
        return false;
    }
    
    // Sözlükteki kelimeler büyük harf olduğu için, kontrol edilen kelimeyi de
    // Türkçe karakterlere duyarlı şekilde büyük harfe dönüştür.
    const upperCaseWord = word.toLocaleUpperCase('tr-TR');
    
    // DICTIONARY global Set'i içinde kelimeyi kontrol et.
    return DICTIONARY.has(upperCaseWord);
}

// Türkçedeki Sesli ve Sessiz Harfler
const VOWELS = ['A', 'E', 'I', 'İ', 'O', 'Ö', 'U', 'Ü'];

// TÜRKÇE HARF HAVUZU (Rastgele Mod İçin)
const LETTER_POOL_CONFIG = {
    'A': 4, 'E': 4, 'İ': 4, 'K': 6, 'L': 6, 'R': 6, 'N': 5, 'T': 5,
    'I': 3, 'M': 4, 'U': 3, 'Y': 3, 'S': 3, 'D': 3, 'O': 3, 'B': 2, 
    'Ü': 2, 'Ş': 2, 'Z': 2, 'G': 1, 'H': 2, 'Ç': 2, 'P': 2, 'C': 1, 
    'V': 2, 'Ö': 2, 'F': 2, 'J': 1, 'Ğ': 1 
};

// =========================================================
// YARDIMCI FONKSİYON: DİZİ KARIŞTIRMA (SHUFFLE)
// =========================================================
function shuffleArray(array) {
    for (let i = array.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [array[i], array[j]] = [array[j], array[i]];
    }
}

// =========================================================
// OYUN SIRASINI ÜRETME FONKSİYONU (12 SESLİ + 12 SESSİZ = 24 HARF)
// =========================================================

function generateGameSequence() {
    let vowelPool = [];
    let consonantPool = [];

    // 1. Ağırlığa göre Harf Havuzlarını Sesli ve Sessiz olarak ayır
    for (let [letter, count] of Object.entries(LETTER_POOL_CONFIG)) {
        for (let i = 0; i < count; i++) {
            if (VOWELS.includes(letter)) {
                vowelPool.push(letter);
            } else {
                consonantPool.push(letter);
            }
        }
    }

    // 2. Havuzları Karıştır (Rastgele çekim sırasını garantiler)
    shuffleArray(vowelPool);
    shuffleArray(consonantPool);

    // 3. Hedef Sayıları Belirle (25. hamle manuel olduğu için toplam 24 harf)
    const VOWEL_TARGET = 12;
    const CONSONANT_TARGET = 12; 

    let finalSequence = [];
    
    // 4. Hedef Sayıda Harf Çekimi
    
    // Sesli Harfleri Çek (12 tane)
    const requiredVowels = Math.min(VOWEL_TARGET, vowelPool.length);
    finalSequence.push(...vowelPool.splice(0, requiredVowels));

    // Sessiz Harfleri Çek (12 tane)
    const requiredConsonants = Math.min(CONSONANT_TARGET, consonantPool.length);
    finalSequence.push(...consonantPool.splice(0, requiredConsonants));

    // 5. Son 24 Harflik Diziyi Karıştır
    shuffleArray(finalSequence); 

    // Kontrol: Eğer toplam 24 harf üretemediysek (ki bu LETTER_POOL'un yetersizliği demektir), uyarı ver.
    if (finalSequence.length !== (VOWEL_TARGET + CONSONANT_TARGET)) {
        console.warn(`UYARI: Harf havuzu yetersiz. Yalnızca ${finalSequence.length} harf üretildi.`);
    }

    return finalSequence;
}

// ==========================================
// 2. OYUN DEĞİŞKENLERİ
// ==========================================
let currentGameId = null;     // Mevcut oyunun kodu (Örn: "AB12")
let myPlayerId = null;        // Oyuncunun rolü ('PlayerA' veya 'PlayerB')
let placementMode = false;    // Hücrelere tıklamaya izin var mı? (Boolean)
let myFinalLetter = null;     // 25. hamle için yerel olarak seçilen harf (Sadece 25. hamlede kullanılır)
let unsubscribe = null;       // Firebase anlık dinleyicisini kapatmak için kullanılır.
let selectedDraftIndex = -1; // Seçili hücrenin indeksini tutar (Onay mekanizması için)
let myGridData = [];
let selectedClassicLetter = null; // Klasik modda o an seçilen (ama henüz gönderilmeyen) harf

// ==========================================
// 3. DOM ELEMENTLERİ
// ==========================================

const lobbyPanel = document.getElementById('lobbyPanel');
const gamePanel = document.getElementById('gamePanel');
const gameOverPanel = document.getElementById('gameOverPanel'); 
const myGridEl = document.getElementById('myGrid');
const oppGridEl = document.getElementById('opponentGrid');
const statusMsg = document.getElementById('gameStatusMsg');
const ALL_VOWELS = "AEIİOÖUÜ";

// HTML'deki ID'lerle eşleştirilen kritik elementler
const gameCodeDisplay = document.getElementById('gameCodeDisplay'); 
const myPlayerRoleEl = document.getElementById('myPlayerRole'); 
const moveNumberDisplayEl = document.getElementById('moveNumberDisplay');
const randomLetterDisplay = document.getElementById('randomLetterDisplay');

// ==========================================
// YENİ OYUN KURMA (GÜNCELLENMİŞ VE PARAMETRELİ)
// ==========================================

async function createNewGame(mode) { // 'mode' parametresini dışarıdan (butondan) alıyoruz
    // 1. Oyun Kodunu Üret ve Oyuncu Rolünü Belirle
    const code = Math.random().toString(36).substring(2, 6).toUpperCase();
    myPlayerId = 'PlayerA';
    currentGameId = code;
    
    // 2. Mod Belirleme (Butondan gelen değeri kullan, yoksa varsayılan CLASSIC)
    const selectedMode = mode ? mode.toUpperCase() : 'CLASSIC'; 
    
    let sequence = null;
    let initialLetter = null;

    document.getElementById('lobbyStatus').textContent = `${selectedMode} oyun kuruluyor...`;

    // 3. Rastgele Mod (veya Puzzle) İçin Harf Dizisini Oluştur
    // Buraya 'RANDOM' yanına 'PUZZLE' da eklenebilir
    if (selectedMode === 'RANDOM' || selectedMode === 'PUZZLE') {
        try {
            sequence = generateGameSequence(); 
            
            if (!sequence || sequence.length < 24) {
                 throw new Error("Harf dizisi üretilemedi.");
            }
            
            // Random modda ilk harf hemen belirlenir
            initialLetter = sequence[0]; 
            
        } catch (e) {
            document.getElementById('lobbyStatus').textContent = `HATA: ${e.message}`;
            currentGameId = null; 
            return; 
        }
    }
    
    // 4. Firestore'a Veri Yazma
    try {
        await db.collection('games').doc(code).set({
            status: 'waiting',      
            turnOwner: 'PlayerA',   
            moveNumber: 1,  
            isSinglePlayer: false, 
            
            gameMode: selectedMode, // Artık butondan gelen gerçek mod yazılacak
            letterSequence: sequence,      
            currentLetter: initialLetter, 
            
            gridA: Array(25).fill(''),
            gridB: Array(25).fill(''),
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });

        // 5. Arayüzü Güncelle
        setupGameUI(code);

        // UI Görünürlük Ayarları
        const oppSection = document.getElementById('opponentSection');
        if(oppSection) oppSection.style.display = 'block';

        const turnIndicator = document.getElementById('turnIndicator');
        if(turnIndicator) turnIndicator.style.display = 'block';

        const codeDisplay = document.getElementById('gameCodeDisplay');
        if (codeDisplay && codeDisplay.parentElement) codeDisplay.parentElement.style.display = 'block';
        
        const roleDisplay = document.getElementById('myPlayerRole');
        if (roleDisplay && roleDisplay.parentElement) roleDisplay.parentElement.style.display = 'block';

        // Dinlemeyi Başlat
        listenToGame();

    } catch (error) {
        document.getElementById('lobbyStatus').textContent = "Oyun kurulamadı!";
        console.error("Firebase Yazma Hatası:", error);
        currentGameId = null; 
    }
}
// ==========================================
// OYUNA KATILMA FONKSİYONU (DÜZELTİLMİŞ)
// ==========================================

async function joinGame() {
    // 1. Oda Kodunu Al ve Kontrol Et
    const gameCodeInput = document.getElementById('gameCodeInput');
    const code = gameCodeInput ? gameCodeInput.value.trim().toUpperCase() : '';
    
    if (!code || code.length !== 4) {
        document.getElementById('lobbyStatus').textContent = "HATA: Lütfen 4 haneli geçerli bir oda kodu girin.";
        return;
    }

    const gameRef = db.collection('games').doc(code);
    document.getElementById('lobbyStatus').textContent = "Oyuna bağlanılıyor...";

    try {
        const doc = await gameRef.get();

        if (!doc.exists) {
            document.getElementById('lobbyStatus').textContent = "HATA: Bu kodda aktif/bekleyen bir oyun bulunamadı.";
            return;
        }

        const data = doc.data();

        // Oyun zaten başladıysa veya bittiyse (Veya Tek kişilik oyunsa - çünkü o da 'active' başlar)
        if (data.status === 'active' || data.status === 'finished') {
            document.getElementById('lobbyStatus').textContent = "HATA: Oyun zaten başladı veya doldu.";
            return;
        }
        
        // 2. Oyuncu B olarak global değişkenleri ayarla
        myPlayerId = 'PlayerB';
        currentGameId = code;

        // 3. Oyun durumunu "waiting" -> "active" olarak güncelle
        await gameRef.update({
            status: 'active'
        });

        // 4. Arayüzü oyun paneline geçir
        setupGameUI(code);

        // --- KRİTİK DÜZELTME: GİZLENEN ALANLARI GERİ AÇ ---
        // Tek kişilik oyundan gelindiyse bu alanlar 'display: none' kalmış olabilir.
        const oppSection = document.getElementById('opponentSection');
        if(oppSection) oppSection.style.display = 'block'; 

        const turnIndicator = document.getElementById('turnIndicator');
        if(turnIndicator) turnIndicator.style.display = 'block'; 

        const codeDisplay = document.getElementById('gameCodeDisplay');
        if (codeDisplay && codeDisplay.parentElement) codeDisplay.parentElement.style.display = 'block';
        
        const roleDisplay = document.getElementById('myPlayerRole');
        if (roleDisplay && roleDisplay.parentElement) roleDisplay.parentElement.style.display = 'block';

        // 5. Dinlemeyi Başlat
        listenToGame();

    } catch (error) {
        document.getElementById('lobbyStatus').textContent = "Oyuna katılırken bir hata oluştu.";
        console.error("Oyuna Katılma Hatası:", error);
        currentGameId = null; 
    }
}

// ==========================================
// TEK KİŞİLİK OYUN BAŞLATMA (GÜNCELLENMİŞ)
// ==========================================

async function startSinglePlayerGame() {
    // 1. Rastgele Kod Üret (Yine de bir ID lazım)
    const code = Math.random().toString(36).substring(2, 6).toUpperCase();
    myPlayerId = 'PlayerA';
    currentGameId = code;
    
    document.getElementById('lobbyStatus').textContent = "Tek kişilik oyun hazırlanıyor...";

    // 2. Harf Dizisini Oluştur
    let sequence = null;
    let initialLetter = null;
    
    try {
        sequence = generateGameSequence(); // 24 harflik dizi
        initialLetter = sequence[0];
    } catch (e) {
        console.error("Harf dizisi hatası:", e);
        return;
    }

    // 3. Firestore'a Yaz (Rakip beklemeden direkt ACTIVE)
    try {
        await db.collection('games').doc(code).set({
            status: 'active',       // Bekleme yok, direkt başla
            isSinglePlayer: true,   // TEK KİŞİLİK OLDUĞUNU BELİRTİYORUZ
            turnOwner: 'PlayerA',
            moveNumber: 1,
            
            gameMode: 'RANDOM',     // Tek kişilik mod her zaman Random'dır
            letterSequence: sequence,
            currentLetter: initialLetter,
            
            gridA: Array(25).fill(''),
            gridB: Array(25).fill(''), // Boş kalacak ama hata vermemesi için dursun
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });

        // Arayüzü Kur
        setupGameUI(code);
        
        // --- UI TEMİZLİĞİ (TEK KİŞİLİK MODA ÖZEL GİZLEMELER) ---
        
        // 1. Rakip Alanını (Sağ Taraf) Gizle
        const opponentSection = document.getElementById('opponentSection');
        if (opponentSection) opponentSection.style.display = 'none';

        // 2. "Sıra Bekleniyor" Yazısını Gizle
        const turnIndicator = document.getElementById('turnIndicator');
        if (turnIndicator) turnIndicator.style.display = 'none';

        // 3. "Oda Kodu" bilgisini gizle (Tek kişilikte gereksiz)
        const codeDisplay = document.getElementById('gameCodeDisplay');
        if (codeDisplay && codeDisplay.parentElement) {
            codeDisplay.parentElement.style.display = 'none';
        }

        // 4. "Rolün: Kurucu" bilgisini gizle
        const roleDisplay = document.getElementById('myPlayerRole');
        if (roleDisplay && roleDisplay.parentElement) {
            roleDisplay.parentElement.style.display = 'none';
        }

        // Dinlemeyi Başlat
        listenToGame();

    } catch (error) {
        console.error("Tek kişilik oyun hatası:", error);
        document.getElementById('lobbyStatus').textContent = "Oyun başlatılamadı.";
    }
}

// ==========================================
// ARAYÜZ HAZIRLAMA (DÜZELTİLMİŞ)
// ==========================================

function setupGameUI(gameId) {
    // 1. Panelleri Tanımla
    const lobbyPanel = document.getElementById('lobbyPanel');
    const gamePanel = document.getElementById('gamePanel');
    const gameOverPanel = document.getElementById('gameOverPanel');
    
    // DÜZELTME 1: HTML'deki ID 'gameCodeDisplay' olduğu için bunu kullanmalıyız
    const displayCode = document.getElementById('gameCodeDisplay');
    
    // 2. Panelleri Değiştir
    if (lobbyPanel) lobbyPanel.classList.add('hidden');
    if (gameOverPanel) gameOverPanel.classList.add('hidden');
    
    if (gamePanel) {
        gamePanel.classList.remove('hidden');
    } else {
        console.error("HATA: gamePanel HTML'de bulunamadı!");
        return; 
    }

    // 3. Oyun Kodunu Ekrana Yaz
    if (displayCode) {
        displayCode.textContent = gameId;
    }

    // 4. Durum Mesajını Sıfırla
    // DÜZELTME 2: HTML'deki ID 'gameStatusMsg'
    const statusMsg = document.getElementById('gameStatusMsg');
    if (statusMsg) {
        statusMsg.textContent = "Oyun Yükleniyor...";
        statusMsg.className = "status-msg"; 
    }

    // 5. Action Area ve Random Harf Ekranını Sıfırla
    const actionArea = document.getElementById('actionArea');
    const randomLetterDisplay = document.getElementById('randomLetterDisplay');
    
    if (actionArea) actionArea.classList.add('hidden');
    if (randomLetterDisplay) randomLetterDisplay.classList.add('hidden');
}

// ==========================================
// OYUNU DİNLEME (TABELA VE GRID SORUNSUZ VERSİYON)
// ==========================================

function listenToGame() {
    if (unsubscribe) unsubscribe();

    unsubscribe = db.collection('games').doc(currentGameId)
        .onSnapshot((doc) => {
            if (!doc.exists) {
                alert("Oyun sonlandırıldı veya bulunamadı.");
                window.location.reload();
                return;
            }

            const data = doc.data();

          // --- İSTATİSTİK GÜNCELLEME BURAYA ---
if (data.gameMode === 'RANDOM' || data.gameMode === 'PUZZLE') {
    // Random info alanını göster
    const infoArea = document.getElementById('randomGameInfoArea');
    if (infoArea) infoArea.classList.remove('hidden');
    
    // İstatistikleri hesapla ve yaz
    updateLetterStats(data.letterSequence, data.moveNumber);
} else {
    // Klasik modda bu paneli gizleyebiliriz
    const infoArea = document.getElementById('randomGameInfoArea');
    if (infoArea) infoArea.classList.add('hidden');
}
// ------------------------------------

            // 1. Grid Verilerini Al ve Çiz
            myGridData = (myPlayerId === 'PlayerA') ? data.gridA : data.gridB;
            const oppGridData = (myPlayerId === 'PlayerA') ? data.gridB : data.gridA;

            renderGrid(myGridData, 'myGrid');
            renderGrid(oppGridData, 'opponentGrid');

            // 2. Elementleri Hazırla
            const classicArea = document.getElementById('classicLetterSelectionArea');
            const randomDisplay = document.getElementById('randomLetterDisplay');
            const myGridEl = document.getElementById('myGrid');
            
            // --- TABELAYI BUL ---
            const turnBadge = document.getElementById('turnStatusBadge');

            // --- UI TEMİZLİĞİ ---
            if (classicArea) classicArea.classList.add('hidden');
            if (randomDisplay) randomDisplay.classList.add('hidden');
            
            // ==========================================================
            // YARDIMCI: UI DURUM GÜNCELLEYİCİ (Sorunu Çözen Kısım)
            // ==========================================================
            const updateUIState = (text, badgeColor, isInteractive) => {
                // 1. Tabela Yazısını Güncelle (ZORLAYICI YÖNTEM)
                if (turnBadge) {
                    turnBadge.textContent = text;
                    // Tüm olası renk sınıflarını temizle, yenisini ekle
                    turnBadge.className = `status-badge ${badgeColor}`; 
                    turnBadge.classList.remove('hidden'); 
                    turnBadge.style.display = 'block'; // Görünürlüğü garanti et
                }

                // 2. Grid Görselini ve Tıklamayı Yönet
                placementMode = isInteractive;
                
                if (myGridEl) {
                    if (isInteractive) {
                        myGridEl.classList.remove('waiting-turn'); 
                        myGridEl.classList.add('active-turn');     
                        myGridEl.style.opacity = "1";              
                        myGridEl.style.pointerEvents = "auto";     
                    } else {
                        myGridEl.classList.add('waiting-turn');    
                        myGridEl.classList.remove('active-turn');
                        myGridEl.style.opacity = "0.6";            
                        myGridEl.style.pointerEvents = "none";     
                    }
                }
                // Gridi yeniden çiz (Clickable sınıfları için)
                renderGrid(myGridData, 'myGrid');
            };

            // -------------------------------------------------
            // DURUM: OYUN AKTİF (ACTIVE)
            // -------------------------------------------------
            if (data.status === 'active') {
                document.getElementById('lobbyPanel').classList.add('hidden');
                document.getElementById('gamePanel').classList.remove('hidden');
                document.getElementById('gameOverPanel').classList.add('hidden');

                const isMyTurn = (data.turnOwner === myPlayerId);
                const myFilledCount = myGridData.filter(c => c !== '').length;
                const currentMove = data.moveNumber || 1; // moveNumber yoksa 1 kabul et
                
                // Bu turdaki hamlem yapıldı mı?
                const myMoveDone = (myFilledCount >= currentMove);

                // ====================================================
                // A. 25. TUR: JOKER HAMLESİ
                // ====================================================
                if (currentMove === 25) {
    // 1. Sayaç kutusunu bul (ID'nin doğruluğundan emin ol)
    const statsBox = document.getElementById('remainingStatsBox');

    if (randomDisplay) randomDisplay.classList.remove('hidden');
    
    // 2. 25. turda sayacı gizle
    if (statsBox) statsBox.classList.add('hidden');

    if (myFilledCount >= 25) {
        updateUIState("OYUN BİTİYOR... RAKİP BEKLENİYOR", "badge-neutral", false);
        // Biterken harf kutusunu da gizle
        if (randomDisplay) randomDisplay.classList.add('hidden');
    } else {
        renderAlphabetSelector(); 
        if (!myFinalLetter) {
            updateUIState("SON HARF: JOKER SEÇ", "badge-info", false);
        } else {
            updateUIState(`SEÇİLEN: ${myFinalLetter} - YERLEŞTİR`, "badge-success", true);
        }
    }
    return;
}

                // ====================================================
                // B. KLASİK MOD (CLASSIC)
                // ====================================================
                if (data.gameMode === 'CLASSIC') {
                    const harfSecildiMi = (data.currentLetter !== null && data.currentLetter !== "");

                    if (!harfSecildiMi) {
                        // HARF SEÇME AŞAMASI
                        if (isMyTurn) {
                            if (classicArea) {
                                classicArea.classList.remove('hidden');
                                if(classicArea.querySelector('#classicAlphabetContainer').children.length === 0) {
                                     renderClassicAlphabet(); 
                                }
                                const confirmBtn = document.getElementById('confirmLetterBtn');
                                if (confirmBtn && !selectedClassicLetter) {
                                    confirmBtn.disabled = true;
                                    confirmBtn.textContent = "BİR HARF SEÇİNİZ";
                                }
                            }
                            updateUIState("Sıra Sizde: Harf Seç", "your-turn", false);
                        } else {
                            updateUIState("Rakip Harf Seçiyor", "opponent-turn", false);
                        }
                    } else {
                        // YERLEŞTİRME AŞAMASI
                        if (randomDisplay) {
                            randomDisplay.textContent = data.currentLetter;
                            randomDisplay.classList.remove('hidden');
                            // Eğer içinde eski alfabe kaldıysa temizle
                            if (randomDisplay.querySelector('.alphabet-wrapper')) {
                                randomDisplay.textContent = data.currentLetter; 
                            }
                        }

                        if (!myMoveDone) {
                            updateUIState(`"${data.currentLetter}" Harfini Yerleştir`, "your-turn", true);
                        } else {
                            updateUIState("Rakip Yerleştiriyor", "opponent-turn", false);
                        }
                    }
                }

                // ====================================================
                // C. RANDOM MODLAR
                // ====================================================
                else {
                    if (randomDisplay) {
                        randomDisplay.textContent = data.currentLetter;
                        randomDisplay.classList.remove('hidden');
                        if (randomDisplay.querySelector('.alphabet-wrapper')) {
                             randomDisplay.textContent = data.currentLetter;
                        }
                    }

                    if (!myMoveDone) {
                        updateUIState("Harfi Yerleştirin", "your-turn", true);
                    } else {
                         if(data.isSinglePlayer) {
                             updateUIState("Kaydediliyor...", "badge-neutral", false);
                         } else {
                             updateUIState("Rakip Bekleniyor...", "opponent-turn", false);
                         }
                    }
                }
            } 
            
            // -------------------------------------------------
            // DURUM: OYUN BİTTİ (FINISHED)
            // -------------------------------------------------
            else if (data.status === 'finished') {
                  if (data.isDailyChallenge) {
                  showDailyResults(data);
                  } else {
                showResults(data);
    }
}
        });
}

// --- JOKER SEÇİMİ İÇİN YENİ FONKSİYONLAR (DÜZELTİLMİŞ) ---

// 1. Alfabeyi Ekrana Çizen Fonksiyon
function renderAlphabetSelector() {
    const display = document.getElementById('randomLetterDisplay');
    if (!display) return;

    // Zaten çiziliyse tekrar çizme
    if (display.querySelector('.alphabet-wrapper')) return;

    display.innerHTML = ''; 
    display.classList.remove('hidden');
    
    // Alfabe Wrapper Stili
    const wrapper = document.createElement('div');
    wrapper.className = 'alphabet-wrapper';
    
    // Harfler
    const alphabet = "ABCÇDEFGĞHIİJKLMNOÖPRSŞTUÜVYZ".split('');
    alphabet.forEach(letter => {
        const btn = document.createElement('div');
        btn.textContent = letter;
        btn.className = 'alpha-btn';
        btn.id = `btn-joker-${letter}`;
        
        btn.onclick = (e) => {
            e.stopPropagation(); // Event bubble'ı engelle
            selectJokerLetter(letter);
        };
        wrapper.appendChild(btn);
    });

    display.appendChild(wrapper);
}

// 2. Harf Seçildiğinde Çalışan Fonksiyon (Seçimi İşaretler)
function selectJokerLetter(letter) {
    // 1. Global değişkeni güncelle
    myFinalLetter = letter; 

    // 2. Görsel Olarak Harf Seçimini Göster
    const allBtns = document.querySelectorAll('.alpha-btn');
    allBtns.forEach(btn => btn.classList.remove('selected'));

    const selectedBtn = document.getElementById(`btn-joker-${letter}`);
    if (selectedBtn) {
        selectedBtn.classList.add('selected');
    }

    // 3. Oyun Durumunu Güncelle (Tabela)
    const turnBadge = document.getElementById('turnStatusBadge');
    if(turnBadge) {
        turnBadge.textContent = `SEÇİLEN: ${letter} - YERLEŞTİRİN`;
        turnBadge.className = "status-badge badge-success";
    }

    // 4. Grid'i Aktif Et (Tıklamayı Aç)
    const myGridEl = document.getElementById('myGrid');
    placementMode = true; 
    
    if (myGridEl) {
        myGridEl.classList.remove('waiting-turn');
        myGridEl.classList.add('active-turn');
        myGridEl.style.opacity = "1";
        myGridEl.style.pointerEvents = "auto";
    }

    // Gridi yeniden çiz (Değişikliklerin yansıması için)
    renderGrid(myGridData, 'myGrid');
} // <-- FONKSİYON BURADA BİTMELİ

// ==========================================
// HAMLE VE SIRA MANTIĞI (GÖRSEL EFEKTLİ)
// ==========================================

function handleTurnLogic(data, myGridData) {
    const actionArea = document.getElementById('actionArea');
    const randomLetterDisplay = document.getElementById('randomLetterDisplay');
    const turnBadge = document.getElementById('turnStatusBadge'); // Yeni Tabela
    const myGridElement = document.getElementById('myGrid'); // CSS sınıfı için
    
    // Yardımcı: Tabelayı Güncelleme Fonksiyonu
    const updateBadge = (text, type, isGridActive) => {
        if (!turnBadge) return;
        turnBadge.classList.remove('hidden', 'badge-success', 'badge-warning', 'badge-info', 'badge-neutral');
        turnBadge.classList.add(type);
        turnBadge.textContent = text;

        // Grid Efektini Yönet
        if (myGridElement) {
            myGridElement.classList.remove('active-turn', 'waiting-turn');
            if (isGridActive) {
                myGridElement.classList.add('active-turn');
            } else {
                myGridElement.classList.add('waiting-turn');
            }
        }
    };

    const moveNumber = data.moveNumber || 1;
    const currentLetter = data.currentLetter || "";
    const myFilledCount = myGridData.filter(cell => cell !== '' && cell !== null).length;

    // Varsayılan UI
    if (actionArea) actionArea.classList.add('hidden');
    if (randomLetterDisplay) randomLetterDisplay.classList.remove('hidden'); 
    if (randomLetterDisplay) randomLetterDisplay.textContent = currentLetter || "?";
    
    disableControls();
    placementMode = false;

 // --- 1. JOKER HAMLESİ (25. Hamle) ---
    if (moveNumber === 25) {
        if (actionArea) actionArea.classList.add('hidden');
        if (randomLetterDisplay) randomLetterDisplay.classList.remove('hidden');

        if (myFilledCount >= 25) {
            updateBadge("OYUN BİTİYOR...", "badge-neutral", false);
        } else {
            // HER DURUMDA ALFABEYİ GÖSTER
            // Kullanıcı seçmiş olsa bile değiştirebilmesi için alfabe ekranda kalmalı
            renderAlphabetSelector();

            if (!myFinalLetter) {
                // Henüz seçim yapılmadı
                updateBadge("JOKER HARFİ SEÇ", "badge-info", false); // Grid Pasif
            } else {
                // Seçim yapılmış (geri dönüldüyse görseli güncellemek gerekebilir)
                const selectedBtn = document.getElementById(`btn-joker-${myFinalLetter}`);
                if (selectedBtn) selectedBtn.classList.add('selected');

                updateBadge(`SEÇİLEN: ${myFinalLetter} - YERLEŞTİR`, "badge-success", true); // Grid Aktif
                placementMode = true;
            }
        }
        return; 
    }

    // --- 2. TEK KİŞİLİK MOD ---
    if (data.isSinglePlayer) {
        if (myFilledCount < moveNumber) {
            updateBadge("HARFİ YERLEŞTİR", "badge-success", true);
            placementMode = true;
            renderGrid(myGridData, 'myGrid');
        } else {
            updateBadge("KAYDEDİLİYOR...", "badge-neutral", false);
        }
        return;
    }

    // --- 3. ÇOK OYUNCULU MOD (KLASİK & RANDOM) ---
    const isMyTurn = (data.turnOwner === myPlayerId);
    const currentLetterIsAvailable = currentLetter !== null && currentLetter !== "";
    const hasNotPlacedInThisTurn = myFilledCount < moveNumber;

    // A. HARF SEÇİM AŞAMASI (Sadece Klasik Mod ve Harf Yoksa)
    if (data.gameMode === 'CLASSIC' && !currentLetterIsAvailable) {
        if (isMyTurn) { 
            // Sıra Bende: Harf Seçmeliyim
            updateBadge("HARF SEÇ", "badge-info", false); // Grid Pasif
            enableControls(true); 
            if(actionArea) actionArea.classList.remove('hidden');
        } else {
            // Sıra Rakipte: Harf Seçiyor
            updateBadge("RAKİP HARF SEÇİYOR", "badge-warning", false); // Grid Pasif
        }
        return;
    }

    // B. YERLEŞTİRME AŞAMASI
    if (currentLetterIsAvailable) {
        if(actionArea) actionArea.classList.add('hidden'); // Inputu gizle
        
        if (hasNotPlacedInThisTurn) {
            // Harf var, henüz koymadım -> YERLEŞTİR
            updateBadge("HARFİ YERLEŞTİR", "badge-success", true); // Grid AKTİF
            
            placementMode = true; 
            renderGrid(myGridData, 'myGrid'); 

        } else {
            // Ben koydum, rakibi bekliyorum
            updateBadge("RAKİP BEKLENİYOR", "badge-warning", false); // Grid Pasif
        }
        
    } else {
        // Beklenmedik durum
        updateBadge("BEKLENİYOR...", "badge-neutral", false);
    }
}

// ==========================================
// HARF SEÇİMİ VE GÖNDERİMİ (DÜZELTİLMİŞ)
// ==========================================

async function submitLetter(letterParam = null) {
    // 1. Harfi Belirle (Parametreden mi Inputtan mı?)
    let letter = letterParam;

    // Eğer parametre yoksa inputtan al (Eski input sistemi için güvenlik)
    if (!letter) {
        const letterInput = document.getElementById('letterInput');
        if (letterInput) letter = letterInput.value.trim().toUpperCase();
    }
    
    // Harf yoksa dur
    if (!letter) return;
    
    // Geçerlilik Kontrolü
    const validLetters = "ABCÇDEFGĞHIİJKLMNOÖPRSŞTUÜVYZ";
    if (!validLetters.includes(letter)) {
        alert("Geçersiz harf.");
        return;
    }

    const gameRef = db.collection('games').doc(currentGameId);
    
    try {
        await db.runTransaction(async (transaction) => {
            const doc = await transaction.get(gameRef);
            if (!doc.exists) throw new Error("Oyun bulunamadı.");
            
            const data = doc.data();

            // Klasik Mod Kontrolleri
            if (data.currentLetter) {
                 // Zaten harf seçilmiş, işlem yapma
                 return; 
            }
            if (data.turnOwner !== myPlayerId) {
                throw new Error("Sıra sizde değil.");
            }
            
            // Harfi Yaz
            transaction.update(gameRef, {
                currentLetter: letter
            });
        });
        
        // UI Temizliği
        const classicArea = document.getElementById('classicLetterSelectionArea');
        if (classicArea) classicArea.classList.add('hidden');
        selectedClassicLetter = null;

    } catch (error) {
        console.error("Harf gönderme hatası:", error);
    }
}

// ==========================================
// HÜCRE TIKLAMA VE HAMLE YAPMA (GÜNCELLENMİŞ)
// ==========================================
async function handleCellClick(index) {
    // 1. Kontrol: Yerleştirme modunda mıyız?
    if (!placementMode) {
        console.log("Yerleştirme modu kapalı.");
        return;
    }

    // --- ONAY (DRAFT) MEKANİZMASI ---
    if (selectedDraftIndex !== index) {
        selectedDraftIndex = index;
        renderGrid(myGridData, 'myGrid'); 
        return;
    }
    
    selectedDraftIndex = null; // Onaylandı, seçimi kaldır

    // Hücre dolu mu?
    if (myGridData[index] !== '') {
        alert("Bu hücre zaten dolu.");
        renderGrid(myGridData, 'myGrid'); 
        return;
    }
    
    const gameRef = db.collection('games').doc(currentGameId);

    try {
        await db.runTransaction(async (transaction) => {
            const doc = await transaction.get(gameRef);
            if (!doc.exists) throw new Error("Oyun bulunamadı.");
            
            const data = doc.data();
            const currentMoveNumber = data.moveNumber;
            const isSinglePlayer = data.isSinglePlayer || false;
            
            // --- HARF BELİRLEME ---
            let letterToPlace;
            const isFinalMove = (currentMoveNumber === 25);

            if (isFinalMove) {
                // 25. TUR: Yerel değişkenden al (myFinalLetter)
                if (!myFinalLetter) throw new Error("Joker harf seçilmedi!");
                letterToPlace = myFinalLetter;
            } else {
                // NORMAL TUR: Veritabanından al
                if (!data.currentLetter) throw new Error("Sunucudan harf gelmedi.");
                letterToPlace = data.currentLetter;
            }
            
            // --- GRID GÜNCELLEME ---
            let myCurrentGrid = (myPlayerId === 'PlayerA') ? [...data.gridA] : [...data.gridB];
            
            if (myCurrentGrid[index] !== '') throw new Error("Hücre dolu (Sunucu kontrolü).");
            myCurrentGrid[index] = letterToPlace;
            
            let updatePayload = {};
            if (myPlayerId === 'PlayerA') updatePayload.gridA = myCurrentGrid;
            else updatePayload.gridB = myCurrentGrid;
            
            // --- TUR ATLAMA VE BİTİRME MANTIĞI ---
            
            // SENARYO 1: TEK KİŞİLİK OYUN
            if (isSinglePlayer) {
                if (isFinalMove) {
                    updatePayload.status = 'finished'; // OYUN BİTTİ
                    updatePayload.currentLetter = null;
                } else {
                    const nextMove = currentMoveNumber + 1;
                    updatePayload.moveNumber = nextMove;
                    if (nextMove === 25) updatePayload.currentLetter = null;
                    else updatePayload.currentLetter = data.letterSequence[currentMoveNumber];
                }
            }
            
            // SENARYO 2: MULTIPLAYER
            else {
                let oppCurrentGrid = (myPlayerId === 'PlayerA') ? data.gridB : data.gridA;
                const oppFilledCount = oppCurrentGrid.filter(c => c !== '' && c !== null).length;
                const myNewFilledCount = myCurrentGrid.filter(c => c !== '' && c !== null).length;

                // A. 25. TUR (JOKER) BİTİŞ KONTROLÜ
                if (isFinalMove) {
                    if (myNewFilledCount === 25) {
                        // Ben bitirdim. Rakip de bitirdi mi?
                        if (oppFilledCount === 25) { 
                            updatePayload.status = 'finished'; // HERKES BİTİRDİ -> OYUN SONU
                            updatePayload.currentLetter = null;
                        } 
                        // Rakip bitirmediyse sadece benim gridim kaydolur, oyun 'active' kalır.
                    }
                } 
                
                // B. NORMAL TUR (1-24)
                else { 
                    // İkimiz de koyduysak tur atla
                    if (myNewFilledCount === currentMoveNumber && oppFilledCount === currentMoveNumber) {
                        const nextMove = currentMoveNumber + 1;
                        updatePayload.moveNumber = nextMove;
                        
                        // Modlara göre harf belirle
                        if (data.gameMode === 'CLASSIC') {
                            updatePayload.turnOwner = (nextMove % 2 !== 0) ? 'PlayerA' : 'PlayerB';
                            updatePayload.currentLetter = null; 
                            
                        } else if (data.gameMode === 'RANDOM') {
                            updatePayload.turnOwner = (data.turnOwner === 'PlayerA') ? 'PlayerB' : 'PlayerA'; 
                            if (nextMove <= 24) { 
                               updatePayload.currentLetter = data.letterSequence[nextMove - 1]; 
                            } else {
                               updatePayload.currentLetter = null; // 25. Tur için null yap
                            }
                        }
                    }
                }
            }
            
            transaction.update(gameRef, updatePayload);
        });
        
        // Başarılı işlem sonrası temizlik
        if (placementMode && myFinalLetter) {
             // Joker kullanıldıysa temizle ama hemen null yapma, renderGrid kullansın
             // myFinalLetter = null; // (Bunu kapattım, oyun bitiş ekranına geçerken sorun olmasın)
        }

    } catch (e) {
        console.error("Hücre hatası:", e);
        alert(e.message); // Kullanıcı hatayı görsün
        selectedDraftIndex = null;
        renderGrid(myGridData, 'myGrid');
    }
}

// ==========================================
// PUAN HESAPLAMA (GÜNCELLENMİŞ - Satır/Sütun Puanlarını Döndürür)
// ==========================================

function calculateScore(gridData) {
    const GRID_SIZE = 5;
    let totalScore = 0;
    let foundWords = new Set();
    
    // YENİ EKLENEN: Satır ve Sütun bazlı puanları tutar
    const rowScores = Array(5).fill(0);
    const colScores = Array(5).fill(0);
    
    const SCORE_RULES = { 2: 2, 3: 5, 4: 9, 5: 15 };

    const getSegmentMaxScore = (text) => {
        // --- ADIM 1: BASKIN KELİME KONTROLÜ (4 ve 5 Harfliler) ---
        if (text.length === 5 && isValidWord(text)) {
            foundWords.add(text);
            return SCORE_RULES[5];
        }

        for (let i = 0; i <= text.length - 4; i++) {
            const sub = text.substring(i, i + 4);
            if (isValidWord(sub)) {
                foundWords.add(sub);
                return SCORE_RULES[4];
            }
        }

        // --- ADIM 2: KOMBİNASYON KONTROLÜ (3 ve 2 Harfliler) ---
        let maxComboScore = 0;
        let bestWords = []; 
        let validSubWords = [];
        
        for (let len = 3; len >= 2; len--) {
            for (let i = 0; i <= text.length - len; i++) {
                const sub = text.substring(i, i + len);
                if (isValidWord(sub)) {
                    validSubWords.push({
                        word: sub,
                        start: i,
                        end: i + len,
                        score: SCORE_RULES[len]
                    });
                }
            }
        }

        validSubWords.forEach(item => {
            if (item.score > maxComboScore) {
                maxComboScore = item.score;
                bestWords = [item.word];
            }
        });

        for (let i = 0; i < validSubWords.length; i++) {
            for (let j = i + 1; j < validSubWords.length; j++) {
                const w1 = validSubWords[i];
                const w2 = validSubWords[j];
                const isOverlapping = (w1.start < w2.end && w2.start < w1.end);

                if (!isOverlapping) {
                    const currentTotal = w1.score + w2.score;
                    if (currentTotal > maxComboScore) {
                        maxComboScore = currentTotal;
                        bestWords = [w1.word, w2.word];
                    }
                }
            }
        }
        
        bestWords.forEach(w => foundWords.add(w));
        return maxComboScore;
    };

    const getLineString = (indices) => {
        return indices.map(index => gridData[index] || ' ').join('');
    };

    // --- Satır Tarama ---
    for (let row = 0; row < GRID_SIZE; row++) {
        const indices = Array.from({ length: GRID_SIZE }, (_, i) => row * GRID_SIZE + i);
        const lineStr = getLineString(indices);
        
        const segments = lineStr.replace(/\s+/g, ' ').split(' ');
        
        segments.forEach(segment => {
            if (segment.length >= 2) { 
                const segmentScore = getSegmentMaxScore(segment);
                totalScore += segmentScore;
                rowScores[row] += segmentScore; // <--- PUANI KAYDET
            }
        });
    }

    // --- Sütun Tarama ---
    for (let col = 0; col < GRID_SIZE; col++) {
        const indices = Array.from({ length: GRID_SIZE }, (_, i) => i * GRID_SIZE + col);
        const lineStr = getLineString(indices);
        
        const segments = lineStr.replace(/\s+/g, ' ').split(' ');
        
        segments.forEach(segment => {
            if (segment.length >= 2) {
                const segmentScore = getSegmentMaxScore(segment);
                totalScore += segmentScore;
                colScores[col] += segmentScore; // <--- PUANI KAYDET
            }
        });
    }

    // SONUÇLARI YENİ YAPIDA DÖNDÜR
    return {
        score: totalScore,
        words: Array.from(foundWords).sort((a, b) => {
            if (b.length !== a.length) return b.length - a.length;
            return a.localeCompare(b, 'tr');
        }),
        rowScores: rowScores,
        colScores: colScores
    };
}

// ==========================================
// OYUN SONUÇLARINI GÖSTERME (TEMİZLENMİŞ VERSİYON)
// ==========================================

function showResults(data) {
    // 1. Puanları Hesapla
    const resultA = calculateScore(data.gridA);
    const resultB = calculateScore(data.gridB);

    // 2. DOM Elementlerini Seç
    const scoreAEl = document.getElementById('scoreA');
    const scoreBEl = document.getElementById('scoreB');
    const wordsListAEl = document.getElementById('wordsListA');
    const wordsListBEl = document.getElementById('wordsListB');
    const finalResultMsgEl = document.getElementById('finalResultMsg');
    
    // Skor Kartlarını (Kutularını) Seç
    const resultCards = document.querySelectorAll('.result-card'); 

    // 3. Panelleri Geçiş
    document.getElementById('lobbyPanel').classList.add('hidden');
    document.getElementById('gamePanel').classList.add('hidden');
    document.getElementById('gameOverPanel').classList.remove('hidden');

    // 4. İstenmeyen Yazıları Gizle (Kazandın/Kaybettin)
    if (finalResultMsgEl) {
        finalResultMsgEl.style.display = 'none'; // Bu yazıyı tamamen kaldırıyoruz
        finalResultMsgEl.textContent = '';
    }

    // 5. Gridleri ve Skorları Doldur
    // A Oyuncusu (Her zaman var)
    scoreAEl.textContent = resultA.score;
    renderFinalScoreGrid(data.gridA, 'finalGridA', resultA.rowScores, resultA.colScores);
    
    // Kelime listesi A
    wordsListAEl.innerHTML = resultA.words.length > 0 
    ? resultA.words.map(w => `<li onclick="fetchDefinition('${w}')">${w}</li>`).join('') 
    : '<li>No words found</li>';

    // 6. TEK KİŞİLİK / ÇOK KİŞİLİK GÖRÜNÜM AYARI
    if (data.isSinglePlayer) {
        // --- TEK KİŞİLİK MOD ---
        
        // İkinci kartı (Rakip/Boş olanı) tamamen GİZLE
        if (resultCards.length > 1) {
            resultCards[1].style.display = 'none';
        }
        
        // Başlığı düzenle ("Kurucu A" yerine "SKOR TABLONUZ" gibi)
        const titleA = document.getElementById('resultTitleA');
        if (titleA) {
            titleA.innerHTML = 'OYUN SONUCUNUZ';
            titleA.style.color = '#2c3e50';
        }

    } else {
        // --- MULTIPLAYER MOD (KLASİK & RANDOM) ---

        // İkinci kartı GÖSTER (Eğer gizlendiyse geri aç)
        if (resultCards.length > 1) {
            resultCards[1].style.display = 'flex'; // Veya 'block', CSS yapınıza göre
        }

        // B Oyuncusunun verilerini doldur
        scoreBEl.textContent = resultB.score;
        renderFinalScoreGrid(data.gridB, 'finalGridB', resultB.rowScores, resultB.colScores);
        
        // Kelime listesi B
        wordsListBEl.innerHTML = resultB.words.length > 0 
    ? resultB.words.map(w => `<li onclick="fetchDefinition('${w}')">${w}</li>`).join('') 
    : '<li>No words found</li>';

        // Başlıkları "Sen" ve "Rakip" olarak ayarla
        const titleA = document.getElementById('resultTitleA');
        const titleB = document.getElementById('resultTitleB');

        if (titleA && titleB) {
            if (myPlayerId === 'PlayerA') {
                titleA.innerHTML = 'SİZİN ALANINIZ <span style="color:#2ecc71">(SEN)</span>';
                titleB.innerHTML = 'RAKİP ALANI';
                titleA.style.color = '#2c3e50'; 
                titleB.style.color = '#95a5a6';
            } else {
                titleA.innerHTML = 'RAKİP ALANI';
                titleB.innerHTML = 'SİZİN ALANINIZ <span style="color:#2ecc71">(SEN)</span>';
                titleB.style.color = '#2c3e50';
                titleA.style.color = '#95a5a6'; 
            }
        }
    }

    // Dinleyiciyi kapat
    if (unsubscribe) {
        unsubscribe();
        unsubscribe = null;
      }

      // ... (showResults fonksiyonunun en sonu) ...

    // --- YENİ İSTATİSTİK GÜNCELLEME (SADECE RANDOM MOD) ---
    const lastProcessedGame = localStorage.getItem('last_processed_game_id');
    
    if (lastProcessedGame !== currentGameId) {
        localStorage.setItem('last_processed_game_id', currentGameId);
        
        // Sadece RANDOM mod ise istatistiği işle
        // Not: Tek kişilik oyun her zaman Random'dır.
        if (data.gameMode === 'RANDOM' || data.isSinglePlayer) {
            
            let myScore = 0;
            if (myPlayerId === 'PlayerA') myScore = resultA.score;
            else myScore = resultB.score;

            // Yeni Fonksiyonu Çağır
            updateRandomStats(myScore);
            console.log("Random mod istatistiği kaydedildi:", myScore);
        }
    }
}
// showResults bitişi

// ==========================================
// GRID ÇİZİM FONKSİYONU (GÜNCELLENMİŞ)
// ==========================================

function renderGrid(gridData, elementId) {
    const gridElement = document.getElementById(elementId);
    if (!gridElement) return;

    gridElement.innerHTML = ''; 
    
    const isMyGrid = (elementId === 'myGrid');
    const isClickable = isMyGrid && placementMode; 

    gridData.forEach((letter, index) => {
        const cell = document.createElement('div');
        cell.classList.add('cell');
        
        // 1. Varsayılan olarak veritabanındaki (kesinleşmiş) harfi yaz
        cell.textContent = letter || ''; 
        
        // 2. Seçim (Draft) Görselleştirmesi
        if (isMyGrid && index === selectedDraftIndex) {
            cell.classList.add('selected-draft');

            // --- YENİ EKLENEN KISIM: HARFİ GÖSTER ---
            // Eğer hücre henüz boşsa (onaylanmamışsa), seçili harfi içinde göster.
            if (letter === '') {
                // A. Joker harfi seçili mi? (25. Tur)
                if (typeof myFinalLetter !== 'undefined' && myFinalLetter) {
                    cell.textContent = myFinalLetter;
                } 
                // B. Değilse, normal turdaki harfi ekrandaki kutudan al
                else {
                    const display = document.getElementById('randomLetterDisplay');
                    // Kutunun içinde Alfabe Seçicisi yoksa (yani tek harf varsa)
                    if (display && !display.querySelector('.alphabet-wrapper')) {
                        const visibleLetter = display.textContent.trim();
                        // Sadece tek karakterse (örn: "A") hücreye yaz
                        if (visibleLetter.length === 1) {
                            cell.textContent = visibleLetter;
                        }
                    }
                }
            }
        }
        
        // 3. Tıklanabilirlik Kontrolü
        // Hücre boşsa VEYA zaten o an seçtiğimiz hücreyse (tekrar tıklayıp onaylamak için)
        const shouldBeClickable = isClickable && (letter === '' || index === selectedDraftIndex);
        
        if (shouldBeClickable) {
            cell.classList.add('clickable');
            // Hücreye tıklanınca handleCellClick çalışsın
            cell.onclick = () => handleCellClick(index);
        } else {
            cell.classList.remove('clickable');
            cell.onclick = null;
        }

        gridElement.appendChild(cell);
    });
}

// ==========================================
// YENİ FONKSİYON: SONUÇ GRIDINI ÇİZME (GÜVENLİ VE RENK KODLAMALI)
// ==========================================

function renderFinalScoreGrid(gridData, elementId, rowScores, colScores) {
    const gridElement = document.getElementById(elementId);
    if (!gridElement) return;

    // Grid yapısını 6x6 olarak ayarla
    gridElement.style.gridTemplateColumns = 'repeat(6, 1fr)';
    gridElement.style.gridTemplateRows = 'repeat(6, 1fr)';

    // İçeriği temizle
    gridElement.innerHTML = '';
    
    // Yardımcı fonksiyon: Puana göre CSS sınıfı döndürür
    const getScoreClass = (score) => {
        if (score >= 15) return 'score-15';
        if (score >= 9) return 'score-9';
        if (score >= 7) return 'score-7';
        if (score >= 5) return 'score-5';
        if (score >= 4) return 'score-4';
        if (score >= 2) return 'score-2';
        if (score === 1) return 'score-1';
        if (score === 0) return 'score-0';
        return '';
    };

    // 5x5 harf hücresi ve 5x1 satır puanı hücresi oluştur
    for (let i = 0; i < 25; i++) {
        const cell = document.createElement('div');
        cell.classList.add('cell');
        cell.textContent = gridData[i] || '';      
        gridElement.appendChild(cell);

        // Satır sonu (i=4, 9, 14, 19, 24)
        if ((i + 1) % 5 === 0) {
            const rowIndex = Math.floor(i / 5);
            const scoreCell = document.createElement('div');
            const score = rowScores[rowIndex];
            
            // --- KRİTİK GÜVENLİK DÜZELTMESİ ---
            // Sınıf listesini oluştur, boş stringleri filtrele
            const classes = ['cell', 'score-cell-row', getScoreClass(score)].filter(Boolean);
            scoreCell.classList.add(...classes);
            // ----------------------------------
            
            scoreCell.textContent = score;
            gridElement.appendChild(scoreCell);
        }
    }
    
    // 5x1 kolon puanı hücresi oluştur
    // Hata izi bu forEach döngüsünü işaret ediyor olabilir.
    colScores.forEach(score => {
        const scoreCell = document.createElement('div');
        
        // --- KRİTİK GÜVENLİK DÜZELTMESİ ---
        // Sınıf listesini oluştur, boş stringleri filtrele
        const classes = ['cell', 'score-cell-col', getScoreClass(score)].filter(Boolean);
        scoreCell.classList.add(...classes);
        // ----------------------------------

        scoreCell.textContent = score;
        gridElement.appendChild(scoreCell);
    });

    // Köşe hücresi (Boş ve şeffaf kalacak)
    const cornerCell = document.createElement('div');
    cornerCell.classList.add('cell', 'empty-corner');
    gridElement.appendChild(cornerCell);
}

// ==========================================
// KONTROL FONKSİYONLARI
// ==========================================

function disableControls() {
    const letterInput = document.getElementById('letterInput');
    const actionButton = document.getElementById('submitLetterButton');
    if (letterInput) letterInput.disabled = true;
    if (actionButton) actionButton.disabled = true;
}

function enableControls(isLetterSelectionMode = true) {
    const letterInput = document.getElementById('letterInput');
    const actionButton = document.getElementById('submitLetterButton');
    
    if (letterInput) {
        letterInput.disabled = !isLetterSelectionMode;
        if (isLetterSelectionMode) {
            letterInput.focus();
        }
    }
    if (actionButton) {
        actionButton.disabled = false;
        actionButton.textContent = isLetterSelectionMode ? "SEÇ" : "BEKLE";
    }
}

// ==========================================
// KLASİK MOD HARF SEÇİM FONKSİYONLARI
// ==========================================

// 1. Alfabeyi Ekrana Çiz
function renderClassicAlphabet() {
    const container = document.getElementById('classicAlphabetContainer');
    if (!container) return;
    
    container.innerHTML = ''; // Temizle
    const alphabet = "ABCÇDEFGĞHİIJKLMNOÖPRSŞTUÜVYZ";
    
    alphabet.split('').forEach(char => {
        const btn = document.createElement('div');
        btn.classList.add('alpha-btn'); // CSS'deki mavi buton stili
        btn.textContent = char;
        
        // Tıklama Olayı
        btn.onclick = () => selectClassicLetter(char, btn);
        
        container.appendChild(btn);
    });
}

// 2. Harfe Tıklandığında Seçim Yap
function selectClassicLetter(char, btnElement) {
    selectedClassicLetter = char;
    
    // Görsel vurgu: Önce hepsinden 'selected' sınıfını kaldır
    const allBtns = document.querySelectorAll('#classicAlphabetContainer .alpha-btn');
    allBtns.forEach(b => b.classList.remove('selected'));
    
    // Tıklanana ekle
    btnElement.classList.add('selected');
    
    // Onay butonunu aktif et
    const confirmBtn = document.getElementById('confirmLetterBtn');
    if (confirmBtn) {
        confirmBtn.disabled = false;
        confirmBtn.textContent = `"${char}" HARFİNİ GÖNDER`;
        confirmBtn.style.backgroundColor = "#28a745"; // Yeşil renk
        confirmBtn.style.color = "white";
    }
}

// 3. Onay Butonuna Tıklanınca Gönder
function submitClassicLetter() {
    if (!selectedClassicLetter) return;
    
    // Mevcut submitLetter fonksiyonunu çağır (veya direkt veritabanı kodu)
    // Eğer kodunuzda "submitLetter()" fonksiyonu varsa onu kullanıyoruz:
    submitLetter(selectedClassicLetter); 
    
    // Temizlik
    selectedClassicLetter = null;
    document.getElementById('classicLetterSelectionArea').classList.add('hidden');
}

// ==========================================
// BİLGİ PENCERESİ (MODAL) YÖNETİMİ
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    const modal = document.getElementById("howToPlayModal");
    const btn = document.getElementById("howToPlayBtn");
    const closeSpan = document.querySelector(".close-modal");

    if (btn && modal && closeSpan) {
        // Butona basınca aç
        btn.onclick = (e) => {
            e.preventDefault(); // Form submit olmasın diye
            modal.classList.remove("hidden");
            modal.style.display = "flex"; // CSS çakışması olursa garanti olsun
        };

        // Çarpıya basınca kapat
        closeSpan.onclick = () => {
            modal.classList.add("hidden");
            modal.style.display = "none";
        };

        // Boşluğa basınca kapat
        window.onclick = (event) => {
            if (event.target === modal) {
                modal.classList.add("hidden");
                modal.style.display = "none";
            }
        };
    }
  });

// ==========================================
// DARK MODE YÖNETİMİ
// ==========================================

function toggleDarkMode() {
    const body = document.body;
    const btn = document.getElementById('darkModeBtn');
    
    // Class'ı aç/kapa
    body.classList.toggle('dark-mode');
    
    // Durumu kontrol et
    const isDark = body.classList.contains('dark-mode');
    
    // İkonu değiştir
    if (btn) btn.textContent = isDark ? '☀️' : '🌙';
    
    // Tercihi kaydet
    localStorage.setItem('theme', isDark ? 'dark' : 'light');
}

// Sayfa Yüklendiğinde Tercihi Hatırla
window.addEventListener('DOMContentLoaded', () => {
    const savedTheme = localStorage.getItem('theme');
    const btn = document.getElementById('darkModeBtn');
    
    // Eğer daha önce dark mode seçildiyse
    if (savedTheme === 'dark') {
        document.body.classList.add('dark-mode');
        if (btn) btn.textContent = '☀️';
    }
});

async function fetchDefinition(word) {
    const modal = document.getElementById('definitionModal');
    const title = document.getElementById('defTitle');
    const body = document.getElementById('definitionBody');

    title.textContent = word.toUpperCase('tr');
    body.innerHTML = "<i>Searching...</i>";
    modal.style.display = "flex";

    // BURAYA KENDİ GOOGLE SCRIPT URL'Nİ YAPIŞTIR
    const scriptUrl = "https://script.google.com/macros/s/AKfycbyHywT-EMNS7J1eOGbALgi2TGNF-uuM9wFpsVk12gn1_Lwwz0bq6AgY7m9EuAHSKXlP/exec"; 

    try {
        const response = await fetch(`${scriptUrl}?word=${encodeURIComponent(word.toLowerCase('tr'))}`);
        const data = await response.json();

        if (data && data[0] && data[0].anlamlarListe) {
            let anlam = data[0].anlamlarListe[0].anlam;
            body.innerHTML = `<p>${anlam}</p>`;
        } else {
            body.innerHTML = "Definition not found.";
        }
    } catch (error) {
        body.innerHTML = "Error connecting to dictionary.";
    }
}

function closeDefinition() {
    const modal = document.getElementById('definitionModal');
    if (modal) {
        modal.style.display = "none";
        // İçeriği temizleyelim ki bir sonraki aramada eski yazı görünmesin
        document.getElementById('definitionBody').innerHTML = "";
    }
}

// Boşluğa tıklandığında kapanma özelliği (UX için önemli)
window.addEventListener('click', function(event) {
    const modal = document.getElementById('definitionModal');
    if (event.target === modal) {
        closeDefinition();
    }
});

function updateLetterStats(sequence, moveNumber) {
    if (!sequence || sequence.length === 0) return;

    // moveNumber 1'den başlar. 
    // Örneğin 1. hamledeysek, sequence[0]'ı yerleştiriyoruzdur. 
    // Kalan harfler sequence[1] ve sonrası (index moveNumber ve sonrası)
    // Ancak 25. hamle (joker) dizi dışıdır, bu yüzden kontrol ekliyoruz.
    const remainingPool = (moveNumber <= 24) ? sequence.slice(moveNumber) : [];

    let vCount = 0;
    let cCount = 0;

    remainingPool.forEach(char => {
        if (ALL_VOWELS.includes(char.toUpperCase())) {
            vCount++;
        } else {
            cCount++;
        }
    });

    const vEl = document.getElementById('vowel-count');
    const cEl = document.getElementById('consonant-count');
    
    if (vEl && cEl) {
        vEl.textContent = vCount;
        cEl.textContent = cCount;
    }
}

// ==========================================
// DAILY CHALLENGE IMPLEMENTATION
// Add these functions to your script.js
// ==========================================

// Constants
const GAME_LAUNCH_DATE = new Date('2025-01-01'); // Set your game launch date

// ==========================================
// 1. GENERATE DAILY SEED AND SEQUENCE
// ==========================================

function getDailyChallengeNumber(dateStr) {
    // Calculate days since launch
    const targetDate = new Date(dateStr);
    const diffTime = targetDate - GAME_LAUNCH_DATE;
    const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
    return diffDays + 1; // Start from Challenge #1
}

function generateDailySequence(dateStr) {
    // Use date as seed for consistent random generation
    const seed = dateStr.split('-').join(''); // "2025-01-15" -> "20250115"
    let random = parseInt(seed);
    
    // Seeded random function
    const seededRandom = () => {
        random = (random * 9301 + 49297) % 233280;
        return random / 233280;
    };
    
    // Build letter pool with weights
    let vowelPool = [];
    let consonantPool = [];
    
    for (let [letter, count] of Object.entries(LETTER_POOL_CONFIG)) {
        for (let i = 0; i < count; i++) {
            if (VOWELS.includes(letter)) {
                vowelPool.push(letter);
            } else {
                consonantPool.push(letter);
            }
        }
    }
    
    // Shuffle with seeded random
    const seededShuffle = (array) => {
        for (let i = array.length - 1; i > 0; i--) {
            const j = Math.floor(seededRandom() * (i + 1));
            [array[i], array[j]] = [array[j], array[i]];
        }
    };
    
    seededShuffle(vowelPool);
    seededShuffle(consonantPool);
    
    // Take 12 vowels and 12 consonants
    let finalSequence = [];
    finalSequence.push(...vowelPool.slice(0, 12));
    finalSequence.push(...consonantPool.slice(0, 12));
    
    seededShuffle(finalSequence);
    
    return finalSequence;
}

// ==========================================
// 2. CHECK IF USER ALREADY PLAYED TODAY
// ==========================================

function hasPlayedToday(dateStr) {
    const userId = getMyStatsId();
    const playedDates = JSON.parse(localStorage.getItem('daily_played_dates') || '{}');
    return playedDates[userId] && playedDates[userId][dateStr];
}

function markDayAsPlayed(dateStr) {
    const userId = getMyStatsId();
    const playedDates = JSON.parse(localStorage.getItem('daily_played_dates') || '{}');
    
    if (!playedDates[userId]) {
        playedDates[userId] = {};
    }
    
    playedDates[userId][dateStr] = true;
    localStorage.setItem('daily_played_dates', JSON.stringify(playedDates));
}

// ==========================================
// 3. START DAILY CHALLENGE
// ==========================================

async function startDailyGame(customDate = null) {
    // Determine which date to play
    const today = new Date();
    const dateStr = customDate || today.toISOString().split('T')[0]; // "YYYY-MM-DD"
    
    // Check if already played
    if (!customDate && hasPlayedToday(dateStr)) {
        const confirmed = confirm(
            "Bu günün challenge'ını zaten oynadınız!\n" +
            "Farklı bir tarih seçmek ister misiniz?"
        );
        
        if (confirmed) {
            showDatePicker();
            return;
        } else {
            return;
        }
    }
    
    // Generate game code
    const code = 'DAILY_' + dateStr.replace(/-/g, '');
    myPlayerId = 'PlayerA';
    currentGameId = code;
    
    document.getElementById('lobbyStatus').textContent = "Günlük challenge hazırlanıyor...";
    
    // Generate sequence
    const sequence = generateDailySequence(dateStr);
    const challengeNum = getDailyChallengeNumber(dateStr);
    
    try {
        // Check if this daily challenge already exists
        const existingDoc = await db.collection('games').doc(code).get();
        
        if (!existingDoc.exists) {
            // Create new daily challenge
            await db.collection('games').doc(code).set({
                status: 'active',
                isSinglePlayer: true,
                isDailyChallenge: true,
                dailyDate: dateStr,
                challengeNumber: challengeNum,
                turnOwner: 'PlayerA',
                moveNumber: 1,
                
                gameMode: 'DAILY',
                letterSequence: sequence,
                currentLetter: sequence[0],
                
                gridA: Array(25).fill(''),
                gridB: Array(25).fill(''),
                createdAt: firebase.firestore.FieldValue.serverTimestamp()
            });
        }
        
        // Setup UI
        setupGameUI(code);
        
        // Hide multiplayer elements
        const opponentSection = document.getElementById('opponentSection');
        if (opponentSection) opponentSection.style.display = 'none';
        
        const turnIndicator = document.getElementById('turnIndicator');
        if (turnIndicator) turnIndicator.style.display = 'none';
        
        const codeDisplay = document.getElementById('gameCodeDisplay');
        if (codeDisplay && codeDisplay.parentElement) {
            codeDisplay.parentElement.style.display = 'none';
        }
        
        const roleDisplay = document.getElementById('myPlayerRole');
        if (roleDisplay && roleDisplay.parentElement) {
            roleDisplay.parentElement.style.display = 'none';
        }
        
        // Show daily info panel
        const dailyPanel = document.getElementById('dailyInfoPanel');
        if (dailyPanel) {
            dailyPanel.classList.remove('hidden');
            
            // Format date in Turkish
            const dateObj = new Date(dateStr + 'T12:00:00');
            const options = { day: 'numeric', month: 'long', year: 'numeric' };
            const formattedDate = dateObj.toLocaleDateString('tr-TR', options);
            
            document.getElementById('dailyDateText').textContent = formattedDate;
            document.getElementById('dailyGameNum').textContent = `Challenge #${challengeNum}`;
        }
        
        // Start listening
        listenToGame();
        
    } catch (error) {
        console.error("Daily challenge error:", error);
        document.getElementById('lobbyStatus').textContent = "Challenge başlatılamadı.";
    }
}

// ==========================================
// 4. DATE PICKER FOR ARCHIVE
// ==========================================

function showDatePicker() {
    const today = new Date();
    const minDate = GAME_LAUNCH_DATE.toISOString().split('T')[0];
    const maxDate = today.toISOString().split('T')[0];
    
    const dateInput = document.createElement('input');
    dateInput.type = 'date';
    dateInput.min = minDate;
    dateInput.max = maxDate;
    dateInput.value = maxDate;
    
    const modal = document.createElement('div');
    modal.className = 'modal';
    modal.style.display = 'flex';
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 400px; text-align: center;">
            <h2 style="color: #8e44ad;">📅 TARİH SEÇ</h2>
            <p style="color: #7f8c8d; margin-bottom: 20px;">
                Geçmiş bir challenge'ı oynamak için tarih seçin
            </p>
            <div id="datePickerContainer" style="margin: 20px 0;"></div>
            <div style="display: flex; gap: 10px; justify-content: center;">
                <button onclick="this.closest('.modal').remove()" 
                        style="background: #95a5a6; padding: 10px 20px; border: none; 
                               border-radius: 6px; color: white; cursor: pointer;">
                    İPTAL
                </button>
                <button id="confirmDateBtn" 
                        style="background: #8e44ad; padding: 10px 20px; border: none; 
                               border-radius: 6px; color: white; cursor: pointer;">
                    OYNA
                </button>
            </div>
        </div>
    `;
    
    document.body.appendChild(modal);
    document.getElementById('datePickerContainer').appendChild(dateInput);
    
    document.getElementById('confirmDateBtn').onclick = () => {
        const selectedDate = dateInput.value;
        if (selectedDate) {
            modal.remove();
            startDailyGame(selectedDate);
        }
    };
    
    modal.onclick = (e) => {
        if (e.target === modal) modal.remove();
    };
}

// ==========================================
// 5. SUBMIT DAILY SCORE
// ==========================================

async function submitDailyScore(score, dateStr, challengeNum) {
    const userId = getMyStatsId();
    
    // Determine score bucket
    let bucket;
    if (score >= 100) bucket = "100+";
    else if (score >= 90) bucket = "90-99";
    else if (score >= 80) bucket = "80-89";
    else if (score >= 70) bucket = "70-79";
    else if (score >= 60) bucket = "60-69";
    else if (score >= 50) bucket = "50-59";
    else bucket = "0-49";
    
    const challengeRef = db.collection('dailyChallenges').doc(dateStr);
    
    try {
        await db.runTransaction(async (transaction) => {
            const doc = await transaction.get(challengeRef);
            
            let data = doc.exists ? doc.data() : {
                date: dateStr,
                challengeNumber: challengeNum,
                highestScore: 0,
                totalPlayers: 0,
                scoreDistribution: {
                    "100+": 0, "90-99": 0, "80-89": 0,
                    "70-79": 0, "60-69": 0, "50-59": 0, "0-49": 0
                }
            };
            
            // Update highest score
            if (score > data.highestScore) {
                data.highestScore = score;
            }
            
            // Check if user already submitted
            const userScoreRef = db.collection('userDailyScores').doc(`${userId}_${dateStr}`);
            const userDoc = await transaction.get(userScoreRef);
            
            if (!userDoc.exists) {
                // New submission
                data.totalPlayers += 1;
                data.scoreDistribution[bucket] += 1;
                
                // Save user score
                transaction.set(userScoreRef, {
                    userId: userId,
                    date: dateStr,
                    score: score,
                    timestamp: firebase.firestore.FieldValue.serverTimestamp()
                });
            }
            
            transaction.set(challengeRef, data);
        });
        
        // Mark as played locally
        markDayAsPlayed(dateStr);
        
    } catch (error) {
        console.error("Score submission error:", error);
    }
}

// ==========================================
// 6. SHOW DAILY RESULTS
// ==========================================

async function showDailyResults(data) {
    const result = calculateScore(data.gridA);
    const dateStr = data.dailyDate;
    const challengeNum = data.challengeNumber;
    
    // Submit score
    await submitDailyScore(result.score, dateStr, challengeNum);
    
    // Get distribution data
    const challengeDoc = await db.collection('dailyChallenges').doc(dateStr).get();
    const challengeData = challengeDoc.data();
    
    // Switch panels
    document.getElementById('lobbyPanel').classList.add('hidden');
    document.getElementById('gamePanel').classList.add('hidden');
    document.getElementById('gameOverPanel').classList.remove('hidden');
    
    // Hide normal game over content
    const resultCards = document.querySelectorAll('.result-card');
    resultCards.forEach(card => card.style.display = 'none');
    
    const finalMsg = document.getElementById('finalResultMsg');
    if (finalMsg) finalMsg.style.display = 'none';
    
    // Show daily result panel
    const dailyResultPanel = document.getElementById('dailyResultPanel');
    if (dailyResultPanel) {
        dailyResultPanel.classList.remove('hidden');
        dailyResultPanel.style.display = 'block';
        
        // Update stats
        document.getElementById('resScore').textContent = result.score;
        document.getElementById('resTopScore').textContent = challengeData.highestScore;
        
        // Calculate rank
        const dist = challengeData.scoreDistribution;
        let playersAbove = 0;
        
        if (result.score >= 100) playersAbove = 0;
        else if (result.score >= 90) playersAbove = dist["100+"];
        else if (result.score >= 80) playersAbove = dist["100+"] + dist["90-99"];
        else if (result.score >= 70) playersAbove = dist["100+"] + dist["90-99"] + dist["80-89"];
        else if (result.score >= 60) playersAbove = dist["100+"] + dist["90-99"] + dist["80-89"] + dist["70-79"];
        else if (result.score >= 50) playersAbove = dist["100+"] + dist["90-99"] + dist["80-89"] + dist["70-79"] + dist["60-69"];
        else playersAbove = challengeData.totalPlayers - dist["0-49"];
        
        const rank = playersAbove + 1;
        document.getElementById('resRank').textContent = `#${rank}`;
        
        // Create distribution display
        createDistributionDisplay(challengeData, result.score);
    }
    
    // Show grid and words in a custom area
    createDailyScoreCard(data.gridA, result);
    
    // Setup action buttons
    setupDailyActionButtons(dateStr, challengeNum, result.score);
    
    // Close listener
    if (unsubscribe) {
        unsubscribe();
        unsubscribe = null;
    }
}

// ==========================================
// 7. CREATE DISTRIBUTION DISPLAY
// ==========================================

function createDistributionDisplay(challengeData, userScore) {
    const container = document.getElementById('dailyResultPanel');
    if (!container) return;
    
    // Remove existing distribution if any
    const existing = container.querySelector('.score-distribution-container');
    if (existing) existing.remove();
    
    const dist = challengeData.scoreDistribution;
    const total = challengeData.totalPlayers || 1;
    
    const distContainer = document.createElement('div');
    distContainer.className = 'score-distribution-container';
    distContainer.style.cssText = `
        background: white;
        border-radius: 12px;
        padding: 20px;
        margin: 20px 0;
        box-shadow: 0 2px 8px rgba(0,0,0,0.1);
    `;
    
    const title = document.createElement('h3');
    title.textContent = '📊 PUAN DAĞILIMI';
    title.style.cssText = 'text-align: center; color: #2c3e50; margin-bottom: 15px;';
    distContainer.appendChild(title);
    
    const ranges = [
        { key: "100+", label: "100+ puan", min: 100, max: 999 },
        { key: "90-99", label: "90-99 puan", min: 90, max: 99 },
        { key: "80-89", label: "80-89 puan", min: 80, max: 89 },
        { key: "70-79", label: "70-79 puan", min: 70, max: 79 },
        { key: "60-69", label: "60-69 puan", min: 60, max: 69 },
        { key: "50-59", label: "50-59 puan", min: 50, max: 59 },
        { key: "0-49", label: "0-49 puan", min: 0, max: 49 }
    ];
    
    ranges.forEach(range => {
        const count = dist[range.key] || 0;
        const percentage = ((count / total) * 100).toFixed(1);
        const isUserRange = (userScore >= range.min && userScore <= range.max);
        
        const row = document.createElement('div');
        row.style.cssText = `
            display: flex;
            align-items: center;
            margin: 8px 0;
            padding: 8px;
            border-radius: 6px;
            ${isUserRange ? 'background: #e8f8f5; border: 2px solid #2ecc71;' : ''}
        `;
        
        const label = document.createElement('div');
        label.textContent = range.label + (isUserRange ? ' ← SEN' : '');
        label.style.cssText = `
            width: 120px;
            font-weight: ${isUserRange ? 'bold' : 'normal'};
            color: ${isUserRange ? '#2ecc71' : '#2c3e50'};
        `;
        
        const barContainer = document.createElement('div');
        barContainer.style.cssText = `
            flex: 1;
            background: #ecf0f1;
            height: 24px;
            border-radius: 4px;
            overflow: hidden;
            margin: 0 10px;
        `;
        
        const bar = document.createElement('div');
        bar.style.cssText = `
            width: ${percentage}%;
            height: 100%;
            background: ${isUserRange ? '#2ecc71' : '#3498db'};
            transition: width 0.5s ease;
        `;
        barContainer.appendChild(bar);
        
        const percentText = document.createElement('div');
        percentText.textContent = `${percentage}%`;
        percentText.style.cssText = `
            width: 50px;
            text-align: right;
            font-weight: bold;
            color: #7f8c8d;
        `;
        
        row.appendChild(label);
        row.appendChild(barContainer);
        row.appendChild(percentText);
        distContainer.appendChild(row);
    });
    
    const totalText = document.createElement('div');
    totalText.textContent = `Toplam Oyuncu: ${total.toLocaleString('tr-TR')}`;
    totalText.style.cssText = 'text-align: center; margin-top: 15px; color: #7f8c8d; font-size: 0.9rem;';
    distContainer.appendChild(totalText);
    
    // Insert after summary card
    const summaryCard = container.querySelector('.summary-card');
    if (summaryCard) {
        summaryCard.after(distContainer);
    } else {
        container.appendChild(distContainer);
    }
}

// ==========================================
// 8. CREATE DAILY SCORE CARD
// ==========================================

function createDailyScoreCard(gridData, result) {
    const container = document.getElementById('dailyResultPanel');
    if (!container) return;
    
    // Remove existing if any
    const existing = container.querySelector('.daily-grid-display');
    if (existing) existing.remove();
    
    const gridDisplay = document.createElement('div');
    gridDisplay.className = 'daily-grid-display';
    gridDisplay.style.cssText = `
        background: white;
        border-radius: 12px;
        padding: 20px;
        margin: 20px 0;
        box-shadow: 0 2px 8px rgba(0,0,0,0.1);
        display: flex;
        gap: 20px;
        justify-content: center;
        flex-wrap: wrap;
    `;
    
    // Grid section
    const gridSection = document.createElement('div');
    gridSection.style.cssText = 'flex: 0 0 auto;';
    
    const gridTitle = document.createElement('h4');
    gridTitle.textContent = 'GRİDİN';
    gridTitle.style.cssText = 'text-align: center; color: #2c3e50; margin-bottom: 10px;';
    gridSection.appendChild(gridTitle);
    
    const gridEl = document.createElement('div');
    gridEl.className = 'grid mini-grid';
    gridEl.id = 'dailyFinalGrid';
    gridSection.appendChild(gridEl);
    
    renderFinalScoreGrid(gridData, 'dailyFinalGrid', result.rowScores, result.colScores);
    
    // Words section
    const wordsSection = document.createElement('div');
    wordsSection.style.cssText = 'flex: 1; min-width: 200px;';
    
    const wordsTitle = document.createElement('h4');
    wordsTitle.textContent = 'BULUNAN KELİMELER';
    wordsTitle.style.cssText = 'color: #2c3e50; margin-bottom: 10px;';
    wordsSection.appendChild(wordsTitle);
    
    const wordsList = document.createElement('ul');
    wordsList.className = 'word-list';
    wordsList.style.cssText = `
        list-style: none;
        padding: 10px;
        max-height: 300px;
        overflow-y: auto;
        background: #f8f9fa;
        border-radius: 8px;
    `;
    
    wordsList.innerHTML = result.words.length > 0 
        ? result.words.map(w => `<li onclick="fetchDefinition('${w}')" style="cursor: pointer; padding: 5px; margin: 3px 0; background: white; border-radius: 4px;">${w}</li>`).join('') 
        : '<li>Kelime bulunamadı</li>';
    
    wordsSection.appendChild(wordsList);
    
    gridDisplay.appendChild(gridSection);
    gridDisplay.appendChild(wordsSection);
    
    // Insert before summary actions
    const summaryActions = container.querySelector('.summary-actions');
    if (summaryActions) {
        summaryActions.before(gridDisplay);
    } else {
        container.appendChild(gridDisplay);
    }
}

// ==========================================
// 9. SETUP ACTION BUTTONS
// ==========================================

function setupDailyActionButtons(dateStr, challengeNum, score) {
    const summaryActions = document.querySelector('.summary-actions');
    if (!summaryActions) return;
    
    summaryActions.innerHTML = '';
    
    // Share button
    const shareBtn = document.createElement('button');
    shareBtn.className = 'share-btn';
    shareBtn.textContent = 'SONUCU PAYLAŞ 📤';
    shareBtn.onclick = () => shareDailyResult(dateStr, challengeNum, score);
    
    // Archive button
    const archiveBtn = document.createElement('button');
    archiveBtn.className = 'archive-btn';
    archiveBtn.textContent = 'BAŞKA TARİH SEÇ 📅';
    archiveBtn.onclick = showDatePicker;
    
    // Home button
    const homeBtn = document.createElement('button');
    homeBtn.className = 'archive-btn';
    homeBtn.style.background = '#2ecc71';
    homeBtn.textContent = 'ANA MENÜ 🏠';
    homeBtn.onclick = () => window.location.reload();
    
    summaryActions.appendChild(shareBtn);
    summaryActions.appendChild(archiveBtn);
    summaryActions.appendChild(homeBtn);
}

// ==========================================
// 10. SHARE RESULT
// ==========================================

function shareDailyResult(dateStr, challengeNum, score) {
    const dateObj = new Date(dateStr + 'T12:00:00');
    const options = { day: 'numeric', month: 'long' };
    const formattedDate = dateObj.toLocaleDateString('tr-TR', options);
    
    const shareText = `Kelimelik 5x5 - Challenge #${challengeNum}
${formattedDate}

🎯 Puanım: ${score}
🏆 Günün Rekoru: [Sitede gör]

Hemen oyna: https://kelimelik5x5.com`;
    
    if (navigator.share) {
        navigator.share({
            title: 'Kelimelik 5x5',
            text: shareText
        }).catch(() => {});
    } else {
        // Fallback: Copy to clipboard
        navigator.clipboard.writeText(shareText).then(() => {
            alert('Sonuç panoya kopyalandı! 📋');
        }).catch(() => {
            // Manual copy
            const textarea = document.createElement('textarea');
            textarea.value = shareText;
            document.body.appendChild(textarea);
            textarea.select();
            document.execCommand('copy');
            document.body.removeChild(textarea);
            alert('Sonuç panoya kopyalandı! 📋');
        });
    }
}

// ==========================================
// 11. UPDATE listenToGame() FUNCTION
// Add this to the existing listenToGame function
// ==========================================

// In listenToGame(), add this check at the end where status === 'finished':

/*
else if (data.status === 'finished') {
    if (data.isDailyChallenge) {
        showDailyResults(data);  // <-- ADD THIS
    } else {
        showResults(data);
    }
}
*/

// ==========================================
// 12. THEME TOGGLE FOR DAILY PANEL
// ==========================================

// Add this to your existing theme toggle
document.addEventListener('DOMContentLoaded', () => {
    const dailyThemeBtn = document.getElementById('themeToggle');
    if (dailyThemeBtn) {
        dailyThemeBtn.onclick = toggleDarkMode;
    }
    
    // Setup info and stats buttons in daily panel
    const dailyInfoBtns = document.querySelectorAll('.daily-icon-btn');
    dailyInfoBtns.forEach(btn => {
        const title = btn.getAttribute('title');
        if (title === 'Nasıl Oynanır?') {
            btn.onclick = window.openInfoModal;
        } else if (title === 'İstatistikler') {
            btn.onclick = window.openStatsModal;
        }
    });
});


